       ...
       Else
          ...
          Select COUNT(*) qtde,
            ...
            from T_CON_CONHECIMENTO c,
                 T_CON_CALCCONHECIMENTO CA,
                 T_SLF_RECCUST RC
           -- Sirlano 12/08/2020 mudando clausula in 
  ...
  End if ;  
      ...
      exception when others then
       ...
       loop
            ...
            if vSaque = '1' then /* JOAO - 14/10/2021 - CH.232829 */
               ...
               P_LISTADADOS(i).linha('CABEC5')     := 'EMISSAO - ' || to_char(tpLinhaValeFrete.Con_Valefrete_Datacadastro,'dd/mm/yyyy') || ' EMISSOR - ' || trim(tpLinhaValeFrete.Con_Valefrete_Emissor);
            end if; 
...
*/              
            ...
            -- Iguala o iparou com i2 que ele sera usado para as proximas linhas
            -- o i2 e para controlar a quantidade de linha das frases
            iParou := i2 ;
            i2 := 0;
...
--               P_LISTADADOS(i).linha('CONDESP' || lpad(iParou+1,1,'0') ) := '2.O contratado, neste ato, concorda com os valores negociados, aceitando o frete pago de acordo com as especificações deste transporte quanto à quantidade, peso e volumes';
            ...
            --Não pode Voltar o iParou tem que dar sequencia nele
            -- Sirlano 06/10/2020
--            iParou := i2 + 1;
            i2 := 0;
...
--           i:= -1;  
  ...
  procedure  SP_GLB_RETORNAPCL(P_XMLIN    In VARCHAR2,
    ...
    as   
     ...
     vQtdCopias tdvadm.t_usu_aplicacaoperfil.usu_aplicacaoperfil_parat%type;
   Begin
...
ElsIF substr(P_XMLIN,1,10) = 'VALEFRETEC' Then
   ...
   vXmlin := vXmlin || '      <PCL>VALEFRETE</PCL> ';
   vXmlin := vXmlin || '      <PCLVersao>' || c_VersaoValeFrete || '</PCLVersao> ';
   vXmlin := vXmlin || '      <Codigo>624306</Codigo> ';/*850816*/  /*015831*/
   vXmlin := vXmlin || '      <Serie>A1</Serie> ';
   vXmlin := vXmlin || '      <Rota>011</Rota> ';/*021*/            /*198*/
   vXmlin := vXmlin || '      <Saque>2</Saque> ';                   /*1*/
   vXmlin := vXmlin || '      <Ciclo></Ciclo> ';  
   vXmlin := vXmlin || '      <DiarioBordo>S</DiarioBordo> ';
   vXmlin := vXmlin || '      <UsuarioTDV>jsantos</UsuarioTDV> ';
   vXmlin := vXmlin || '      <RotaUsuarioTDV>010</RotaUsuarioTDV> ';
...
ElsIF P_XMLIN = 'GERCOLETA' Then
   ...
   vXmlin := vXmlin || '      <Coleta>061225</Coleta> ';
   vXmlin := vXmlin || '      <Ciclo>005</Ciclo> ';
   vXmlin := vXmlin || '      <UsuarioTDV>jsantos</UsuarioTDV> ';
   vXmlin := vXmlin || '      <RotaUsuarioTDV>010</RotaUsuarioTDV> ';
          ...
          loop
              ...
              for C_LINHA in (select pcl.glb_pcllinha_campo,
                ...
                loop
                  ...
                  -- Thiago -05/01/2021 - If para colocar quantidade de cópias no conteúdo do pcl  
                  if vPCL = 'GERCOLETA' Then
                    -- Thiago - 05/01/2021 - Variável para pegar quantidades de cópias do parâmetro
                    vQtdCopias := plistaparams('QTDCOPIAS').Texto;
                  
                     vComando := REPLACE(vComando, 'CC', vQtdCopias);
                  End if;   
   ...
   if (vPCL = 'DIARIOBORD') and (P_STATUS = pkg_glb_common.Status_Nomal) then
       Begin
        ...
        Update t_con_valefrete v
           set v.con_valefrete_diariobordo = 'N'
...
*/
 ...
 -- Declaro todas as variáveis que serão utilizadas.
  ...
  -- Mudei o tamnha da placa para comportar a segunda carreta quando for o caso
  -- Sirlano / Bruno
  -- 08/11/2021
  vPlacaCarreta   varchar2(15); --tdvadm.t_arm_coleta_motorista.arm_coleta_motorista_placa%type;
  ...
  vDtProgramacao tdvadm.t_arm_coleta.arm_coleta_dtprogramacao%type;
  begin
  ...
  End if ;
       ...
       -- Thiago - 08/04/2021 - Inclusão da Data de programação no campo de observação   
       vObsCol := vObsCol || ' ' || 'Data Programacao ' || vDtProgramacao;
       
       -- Vinicius Dantas 25/01/2022 - Inclusao da observação quando for imp/exp
       begin
            select count(*)
              into vcount
              from tdvadm.t_arm_coletaimpexp ce
             where ce.arm_coleta_ncompra = vColeta
               and ce.arm_coleta_ciclo = vCiclo;
               
            if vcount > 0 then
            
               for p_cursor in(select cie.arm_coletaimpexp_direserva,
                                      cie.arm_coletaimpexp_refcliente,
                                      cie.arm_coletaimpexp_refdespach,
                                      cie.arm_coletaimpexp_navio,
                                 ...
                                 from tdvadm.t_arm_coletaimpexp cie,
                                      tdvadm.t_arm_coleta c
                                where c.arm_coleta_ncompra = cie.arm_coleta_ncompra
                                  and c.arm_coleta_ciclo   = cie.arm_coleta_ciclo
                                  and c.arm_coleta_ncompra = vColeta
                                  and c.arm_coleta_ciclo   = vCiclo)
              
               loop
                 if p_cursor.arm_coleta_normalimpexp = 'N' then
                    vObsCol := vObsCol;
                    
                 elsif p_cursor.arm_coleta_normalimpexp = 'E' then
                    vObsCol := vObsCol ||
                               'RESERV-' || p_cursor.arm_coletaimpexp_direserva || ' RF CLI-' || p_cursor.arm_coletaimpexp_refcliente || ' RF DESP-' || p_cursor.arm_coletaimpexp_refdespach || ' NAVIO-' || p_cursor.arm_coletaimpexp_navio ||
                               'CONT-' || p_cursor.arm_coleta_containertp || 'PES-' || p_cursor.arm_coleta_containercod || 'COLETA-' || p_cursor.arm_coleta_terminalcol || 'ENTREGA-' || p_cursor.arm_coleta_terminalent || 'PORTO-' || p_cursor.arm_coleta_porto;
                    
                 elsif p_cursor.arm_coleta_normalimpexp = 'I' then
                     vObsCol := vObsCol || 
                                'DI-' || p_cursor.arm_coletaimpexp_direserva || ' RF CLI-' || p_cursor.arm_coletaimpexp_refcliente || ' RF DESP-' || p_cursor.arm_coletaimpexp_refdespach || ' NAVIO-' || p_cursor.arm_coletaimpexp_navio ||
                                'CONT-' || p_cursor.arm_coleta_containertp || 'PES-' || p_cursor.arm_coleta_containercod || 'COLETA-' || p_cursor.arm_coleta_terminalcol || 'ENTREGA-' || p_cursor.arm_coleta_terminalent || 'PORTO-' || p_cursor.arm_coleta_porto;
       ...
       -- Thiago - 08/04/2021 - Inclusão da CIOT vindo t_vgm_vgciot
       if (NVL(vCiot, 0) = 0) then
         Begin 
           select 
               Distinct
                 col.arm_armazem_codigo,
                 vgm.glb_rota_codigo,
                 col.arm_coleta_dtsolicitacao,
                 nvl(ciot.vgm_vgciot_numero,'0'),
                 col.arm_coleta_fonesolic,
                 col.arm_coleta_emailsolic,
                 col.arm_coleta_obs,
                 col.arm_coleta_dtprogramacao                
              into vArmazem,
                   vRota,
                   vDataEmissao,
                   vCiot,
             ...
             from 
               ...
               t_car_veiculo veic,
               t_car_carreteiro carret,
               t_con_vfreteciot vfc
             Where
               0=0
               And col.arm_coleta_ncompra = motor.arm_coleta_ncompra
               And col.arm_coleta_ciclo = motor.arm_coleta_ciclo 
               And motor.con_freteoper_id = seq.uti_sequencia_codigo
               And motor.con_freteoper_rota = seq.uti_sequencia_rota
               And seq.uti_sequencia_rota = vgm.con_freteoper_rota
               And seq.uti_sequencia_codigo = vgm.con_freteoper_id
               And vgm.glb_rota_codigo = ciot.glb_rota_codigo
               And vgm.vgm_viagem_codigo = ciot.vgm_viagem_codigo
               and ciot.vgm_vgciot_numero = vfc.con_vfreteciot_numero(+)
               And motor.arm_coleta_motorista_placa = veic.car_veiculo_placa
               And motor.arm_coleta_motorista_placasaqu = veic.car_veiculo_saque
               And veic.car_proprietario_cgccpfcodigo = prop.car_proprietario_cgccpfcodigo        
               And motor.arm_coleta_carreteiro = carret.car_carreteiro_cpfcodigo
     ...
     for c_msg in (select rpad(b.arm_coleta_ncompra_volume,14,' ')    volume,
                          ...
                          rpad(b.arm_coletancompra_mercadoria || ' ' ||
                               'L: ' || nvl(b.arm_coleta_ncompra_largura,0) || ' x ' ||
                               'A: ' || nvl(b.arm_coleta_ncompra_altura,0) || ' x ' ||
                               'C: ' || nvl(b.arm_coleta_ncompra_comp,0),
                               84,' ') mercadoria,
                          rpad(replace(TRIM(to_char(b.arm_coleta_ncompra_peso,'999999.000')),'.',','),10,' ')      peso,
                          NVL(A.ARM_COLETAORIGEM_COD, 0) origemcoleta,
                          nvl(b.arm_coleta_ncompra_largura,0) mercadoria_largura,  
                          nvl(b.arm_coleta_ncompra_altura,0)  mercadoria_altura,   
                          nvl(b.arm_coleta_ncompra_comp,0)    mercadoria_comprimento
       ...
       */       
         ...
         If c_msg.origemcoleta = 4 Then -- Se for origem Extranet
           
           begin             
              select max(n.arm_nota_numero)
                into vNota
              from tdvadm.t_arm_nota n
              where n.arm_coleta_ncompra = vColeta
                and n.arm_coleta_ciclo = vCiclo; 
                
           exception when others then
             begin
               
                 select  substr(trim(c.arm_coleta_descnfs),1,9) 
                   into vNota
           ...
           begin
              select nvl(an.col_asn_numeronfe,'0')
               ...
               where an.arm_coleta_ncompra = vColeta
                 and an.arm_coleta_ciclo   = vCiclo
                 and an.col_asn_id = (select max (ab.col_asn_id)
                                             from tdvadm.t_col_asn ab
                                            where ab.arm_coleta_ncompra = an.arm_coleta_ncompra
                                              and ab.arm_coleta_ciclo   = an.arm_coleta_ciclo); 
           exception when others then
             vNota := '0';
           end;
         end if;
        ...
        --Verifica se é frota pelo CPF   
         select count (*)
          ...
          where ve.frt_motorista_cpf = vCPFMOT
            -- 23/12/2020
            -- Sirlano - Faltou as criticas abaixo para o FROTA
            and ve.frt_motorista_dtdesligamento is null
            and nvl(ve.frt_motorista_comissao,'N') = 'S';
          
         --Se for frota, pego os dados nas tabelas de frota
          if vContador > 0 then
            
          --Alimento variaveis dados da placa
          -- DESATIVADA em 08/11/2021
          -- Bruno / Sirlano
          -- ESta pegando a placa da carreta de forma errada.
...
*/               
             ...
             vPlacaCarreta := null;
             for c_msg in (select ve.frt_veiculo_placa   vPlacaCarreta
                           from tdvadm.t_frt_veiculo    ve,
                                tdvadm.t_frt_marmodveic ma,
                                tdvadm.t_frt_tpveiculo  tpv,
                                tdvadm.t_frt_conteng    cg
                           where ve.frt_marmodveic_codigo = ma.frt_marmodveic_codigo
                             and ma.frt_tpveiculo_codigo = tpv.frt_tpveiculo_codigo
                             and (ve.frt_veiculo_placa     = vPlaca
                                  or cg.frt_conjveiculo_codigo = vPlaca)
                             and tpv.frt_tpveiculo_tracao = 'N'
                             and ve.frt_veiculo_codigo    = cg.frt_veiculo_codigo)
             Loop
                If vPlacaCarreta is null  Then
                   vPlacaCarreta       := c_msg.vPlacaCarreta;
                Else
                   vPlacaCarreta := vPlacaCarreta || '/' || c_msg.vPlacaCarreta;
                End If;
             End Loop; 
             ...
             vUFPlaca     := null; 
             vFrota       := null; 
             for c_msg in (select ve.frt_veiculo_placa          vPlaca,
                                  ma.frt_marmodveic_marca    vMarcaModelo,
                                  ve.frt_veiculo_placacidade vCidadePlaca,
                                  ve.glb_estado_codigo       vUFPlaca,
                                  cg.frt_conjveiculo_codigo  vFrota
                           from tdvadm.t_frt_veiculo    ve,
                                tdvadm.t_frt_marmodveic ma,
                                tdvadm.t_frt_tpveiculo  tpv,
                                tdvadm.t_frt_conteng    cg
             ...
             Loop
                vPlaca       := c_msg.vplaca;
                vMarcaModelo := c_msg.vmarcamodelo;
                vCidadePlaca := c_msg.vcidadeplaca;
                vUFPlaca     := c_msg.vufplaca;
                vFrota       := c_msg.vfrota;
          ...
          -- Alimento as váriaveis pegando da base de carreteiro se não for frota, essa tabela também serve para agregados.
              -- 12/11/2021
              -- Sirlano 
              -- tratando erro de registro nao encontrado
             Begin
             ...
             Exception
               When NO_DATA_FOUND Then
                 raise_application_error(-20111,chr(10) || '*********************************************' || 
                                                chr(10) ||'Veiculo com Problema, verifique ' || vPlaca ||'-'||vPlacaSQ ||
                                                chr(10) || '*********************************************' );
               End ;
         --Pego os dados do motorista baseado no último saque da ficha.       
             ...
             Exception
               When NO_DATA_FOUND Then
                 raise_application_error(-20111,chr(10) || '*********************************************' || 
                                                chr(10) ||'Motorista com Problema, verifique ' || vCPFMOT ||
                                                chr(10) || '*********************************************' );
               End ;
