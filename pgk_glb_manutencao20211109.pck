...
--Essa procedure e executada atraves do job n? , uma vez por dia, as 00hs                                               --
--------------------------------------------------------------------------------------------------------------------------
...
-- Procedure utilizada para Cancelar Vales de Frete, emitidos e n?o impressos dentro do prazo.
...
-- Procedure que atualiza as tabelas do Eclick, s?o rodadas pelo sp_manutencao
procedure sp_Atualizado_eclick;

 

--Procedure que sera executada uma vez por hora.
...
-- CRIADO PARAMETRO CONTENDO A LISTA DOS GRUPOS, CASO NAO SEJA ENCONTRADO, USARA ESTE DEFAULT
...
*/
-- Sirlano 03/02/2020
-- PARAMETRO DESATIVADO,Trocado pela TABELA TDVADM.T_SLF_CLIREGRASEMAIL
...
-- Essa proceure e executada pelo Job 2579, que esta programado para passar uma vez por dia, as 00:00 Trunc(sysdate),         --         
...
PROCEDURE SP_MANUTENCAO(PDATAEXEC  IN DATE) IS
 --Variaveis utilizadas para executar as procedure de manutenc?o
 vStatus char(01);
 ...
 --Variavel utilizada apenas para administrar as pessoas que receber?o e-mail de confirmac?o do procedimento.
 vDestinoEmails tdvadm.t_uti_infomail.uti_infomail_endmaildestinatar%type;
 vAuxiliar number;
BEGIN
 --Inicializo as variaveis que ser?o utilizadas nessa procedure
 vStatus := '';
 vMessage := '';
 vDestinoEmails := '';
...
update tdvadm.t_car_proprietario p
  set p.car_proprietario_optsimples = (select decode(nvl(substr(c.omn_consulta_situacao,1,3),'N?O'),'N?O','N','S')
...
where p.car_proprietario_cgccpfcodigo in (select distinct c.car_proprietario_cgccpfcodigo 
                                          ...
                                          where c.car_proprietario_cgccpfcodigo = p2.car_proprietario_cgccpfcodigo
                                            ...
                                            and c.omn_consulta_id = (select max(c1.omn_consulta_id)
                                                                  from tdvadm.t_omn_consulta c1
                                                                  where c1.car_proprietario_cgccpfcodigo = c.car_proprietario_cgccpfcodigo)
                                            and p2.car_proprietario_optsimples <> decode(nvl(substr(c.omn_consulta_situacao,1,3),'N?O'),'N?O','N','S'));
...
commit;                                           
  ...
  /***********************************************************************************************************************************/
  /*  Rotina inserir uma ocorrencia nas coletas n?o usadas.
...
--       End If;
  ...
  /***********************************************************************************************************************************/
  begin
    --define os enderecos que recebera mensagem de confirmac?o.
    ...
    --Envio o e-mail com a confirmac?o da operac?o.
  ...
  Exception
    --Caso ocorra algum erro que impessa a execuc?o da procedure envia o e-mail avisando
    when others then
    wservice.pkg_glb_email.SP_ENVIAEMAIL('Erro na rotina executada pelo Lixeiro',
                                        'A rotina "sp_acum_remetentecolocacao", n?o foi executada corretamente. ' || chr(13) || sqlerrm || chr(13) || chr(13) ||    
                                        'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es',
                                        'aut-e@dellavolpe.com.br',
                                        Trim(vDestinoEmails));
  end; 
  ...
  /***********************************************************************************************************************************/
  /* Primeiro procedimento sera a atualizac?o do Flag de DescBonus.                                                                  */
  /* Essa ac?o foi solicitado pelo Sirlano, por e-mail "27/03/2012 14:17"                                                            */
  ...
  Exception
    ...
    when others then
      wservice.pkg_glb_email.SP_ENVIAEMAIL('Erro na rotina executada pelo Lixeiro',
                                           'A rotina "pgk_glb_manutencao.sp_atz_DescBonusVF", n?o foi executada corretamente. ' || chr(13) || sqlerrm || chr(13) || chr(13) || 'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es',
                                           'aut-e@dellavolpe.com.br',
  ...
  /*******************************************************************************************************************************************************************/
  /*  Bloco utilizado para excluir todos os vales, com as seguintes condic?es.                                                                                       */ 
  /*  Vales N?o impressos e imputados no sistema a mais de 5 dias.                                                                                                   */
  /*  Vales Impressos mas n?o entrados no caixa, inputados no sistema a mais de 15 dias.                                                                             */
  /*******************************************************************************************************************************************************************/
  begin
    vDestinoEmails := '';
    ...
    -- VLN = Vales n?o Impressos
    -- VLI = Vales Impressos.
    
    --Executa a procedure que vai excluir os vales n?o impressos.
    tdvadm.pkg_acc_vales.sp_ExcluirVales('VLN', PDATAEXEC, vStatus, vMessage);
    
    --Executo a Procedure que vai excluir os vales impressos e n?o pagos
    ...
    tdvadm.sp_enviamail( 'SEQUENCIAL',
                         ...
                         'Mensagem gerada pela rotina de exclus?o dos vales n?o Impressos (5 dias) : ' || vMessage || chr(13) ||
                         'Mensagem gerada pela rotina de exclus?o dos vales Impressoa e n?o pagos (15 dias): ' || vMessage2 
                        );
    
    
  Exception
    --caso ocorra algum erro durante a execuc?o da procedure de exclus?o de vales.
    when others then 
      tdvadm.sp_enviamail( 'SEQUENCIAL',
                           vDestinoEmails,
                           'Erro na rotina executada pelo Lixeiro',
                           'A rotina "tdvadm.pkg_acc_vales.sp_ExcluirValesF", n?o foi executada corretamente. ' || chr(13) || sqlerrm || chr(13) || chr(13) ||
                           'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es'
                        );
  end;
...
/*******************************************************************************************************************************************************************/
  begin
    ...
    --executo a procedure responsavel por realizar a atualizac?o dos RNTRCs.
    tdvadm.pkg_cfe_validamotor.sp_atualizaConsRntrc( sysdate,
                                                     vStatus,
                                                     vMessage
                                                   );
    
    --Caso o retorno n?o seja Normal, forco um erro.
  ...
  exception
    ...
    when others then
      tdvadm.sp_enviamail( 'SEQUENCIAL',
                           vDestinoEmails,
                           'Erro na rotina executada pelo Lixeiro',
                           'A rotina "tdvadm.pkg_cfe_validamotor.sp_atualizaConsRntrc", n?o foi executada corretamente. ' || chr(13) || 
                           sqlerrm || chr(13) || 
                           chr(13) ||
                           'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es'
                        );
...
/*******************************************************************************************************************************************************************/
/* Bloco utilizado para atribuir permiss?es para o Tracking para novos usuarios "WEB"                                                                              */                 
/*******************************************************************************************************************************************************************/
  -- PARALIZADO as N?o existe mais esta rotina
  -- Sirlano 07/01/2021
  If sysdate >= to_date('01/01/2030','dd/mm/yyyy') Then
  Begin
    vDestinoEmails := '';
    
    --Limpo as variaveis que ser?o utilizadas nesse bloco.
    vStatus := '';
    vMessage := '';
    
    -- Executo a procedure utilizada para atribuir as permiss?es 
    ...
    tdvadm.sp_enviamail( 'SEQUENCIAL',
                         ...
                         'N?o responda esse e-mail, mensagem gerada automaticamente pelo WorkFlow ao executar a rotina: "coleta.pkg_web.SP_SET_PermTracking"' 
                        );
  ...
  Exception
    When Others Then
      ...
      tdvadm.sp_enviamail( 'SEQUENCIAL',
                           Trim(vDestinoEmails),
                           'Erro na rotina executada pelo Lixeiro',
                           'A rotina "coleta.pkg_web.SP_SET_PermTracking", n?o foi executada corretamente. ' || chr(13) || 
                            sqlerrm || chr(13) || 
                            chr(13) ||
                           'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es'
                          );
...
/******************************************************************************/
  Begin
    ...
    tdvadm.sp_enviamail( 'SEQUENCIAL',
                         ...
                         'Status da Rotina: ' || vStatus || chr(13) ||  
                         'Mensagem gerada pela rotina: ' || chr(13) || vMessage || chr(13) || chr(13) ||
                         'N?o responda esse e-mail, mensagem gerada automaticamente pelo WorkFlow ao executar a rotina: "tdvadm.pkg_con_valefrete.SP_RecalculaCusto"' 
                        );
  ...
  Exception
    When Others Then
      ...
      tdvadm.sp_enviamail( 'SEQUENCIAL',
                           Trim(vDestinoEmails),
                           'Erro na rotina executada pelo Lixeiro',
                           'A rotina "tdvadm.pkg_con_valefrete.SP_RecalculaCusto", n?o foi executada corretamente. ' || chr(13) || 
                            sqlerrm || chr(13) || 
                            chr(13) ||
                           'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es'
                          );
...
/******************************************************************************/
/* Klayton                                                                    */
/* Bloco util. para rodas a rotina de Exclus?o de CTRC na serie XXX > 10 Dias */                 
...
/******************************************************************************/
  Begin
    ...
    tdvadm.sp_enviamail( 'SEQUENCIAL',
                         ...
                         'N?o responda esse e-mail, mensagem gerada automaticamente pelo sistema' 
                        );
  ...
  Exception
    When Others Then
      ...
      tdvadm.sp_enviamail( 'SEQUENCIAL',
                           ...
                           'A rotina "tdvadm.sp_limpa_conhec_xxx", n?o foi executada corretamente. ' || chr(13) || 
                            sqlerrm || chr(13) ||chr(13) ||'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es'
                          );
  ...
  /*****************************************************************************************************/
  /* ROBERTO                                                                                           */
  /* Bloco util. para rodar a rotina de Exclus?o de Vales de frete com emiss?o > 5 dias e n?o impressos*/                 
  /* Impementada em 10/09/2013                                                                         */
  /*****************************************************************************************************/
    begin
    --define os enderecos que recebera mensagem de confirmac?o ou erro.
    vDestinoEmails := '';
    vDestinoEmails := vDestinoEmails || 'ricardo.carvalho@dellavolpe.com.br;sirlano.drumond@dellavolpe.com.br';
    vDestinoEmails := vDestinoEmails || 'grp.hd@dellavolpe.com.br;';
...
--     where l."data" <= trunc(sysdate-30);
    ...
    --Envio o e-mail com a confirmac?o da operac?o.
    wservice.pkg_glb_email.SP_ENVIAEMAIL('Rotina executada pelo Lixeiro - Vale frete n?o impresso!',
   ...
   Exception
    ...
    when others then
      wservice.pkg_glb_email.SP_ENVIAEMAIL('Erro na rotina executada pelo Lixeiro',
                                           'A rotina "pgk_glb_manutencao.sp_lmp_semimpressaovf", n?o foi executada corretamente. ' || chr(13) || sqlerrm || chr(13) || chr(13) || 'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es',
                                           'aut-e@dellavolpe.com.br',
                                           Trim(vDestinoEmails));
   end;  
   ...
   exception
     When OTHERS Then
        dbms_output.put_line('ERRO - ' || chr(10) || dbms_utility.format_call_stack);
END SP_MANUTENCAO;
...
/******************************************************************************/
  Begin
    ...
    tdvadm.sp_enviamail( 'SEQUENCIAL',
                         ...
                         'Status da Rotina: ' || vStatus || chr(13) ||  
                         'Mensagem gerada pela rotina: ' || chr(13) || vMessage || chr(13) || chr(13) ||
                         'N?o responda esse e-mail, mensagem gerada automaticamente pelo WorkFlow ao executar a rotina: "tdvadm.pkg_ope_frota.SP_Set_Motoristas"' 
                        );
  Exception
    When Others Then
      --Envio o e-mail com a confirmac?o da operac?o.
      tdvadm.sp_enviamail( 'SEQUENCIAL',
                           Trim(vDestinoEmails),
                           'Erro na rotina executada pelo Lixeiro',
                           'A rotina "' || vMessage2 || '", n?o foi executada corretamente. ' || chr(13) || 
                            sqlerrm || chr(13) || 
                            chr(13) ||
                           'Certifique-se que n?o terei o mesmo erro amanh?, pois um erro no Lixeiro podera prejudicar outras operac?es'
                          );
  ...
  Begin
    ...
    tdvadm.sp_enviamail( 'SEQUENCIAL',
                         ...
                         'Status da Rotina: ' || vStatus || chr(13) ||  
                         'Mensagem gerada pela rotina: ' || chr(13) || vMessage || chr(13) || chr(13) ||
                         'N?o responda esse e-mail, mensagem gerada automaticamente pelo WorkFlow ao executar a rotina: "tdvadm.pkg_ope_frota.sp_set_cadastrogeral"' 
                        );
...
--                         'Status da Rotina: ' || vStatus || chr(13) ||  
--                         'Mensagem gerada pela rotina: ' || chr(13) || vMessage || chr(13) || chr(13) ||
                         'N?o responda esse e-mail, mensagem gerada automaticamente pelo WorkFlow ao executar a rotina: "' || vMessage2 || '"' 
                        );
...
--------------------------------------------------------------------------------------------------------------------------------     
-- Func?o utilizada para excluir todas as notas fiscais lancadas no sistema com a solicitac?o 55899
--------------------------------------------------------------------------------------------------------------------------------     
Function fni_Excluir_NotasCarreg( pNota_Seq  tdvadm.t_arm_nota.arm_nota_sequencia%Type ) Return Varchar2 Is
  --variavel que sera utilizada no retorno da func?o
  vRetorno  Varchar2(32000);
  
  --Variavel que sera utilizada para guardar o codigo do carregamento da embalagem

  
  --Variaveis utilizada para recuperar dados da nota fiscal
...
--------------------------------------------------------------------------------------------------------------------------------     
-- Func?o utilizada para retornar a mensagem a ser gravada nas condicoes epeciais do vale de frete apos cancelamento 
--------------------------------------------------------------------------------------------------------------------------------     
Function fni_Cond_Espec( p_Cond tdvadm.t_con_valefrete.con_valefrete_condespeciais%Type, p_Msg char ) Return Varchar2 Is
  ...
  --variavel que contem o tamanho maximo que o retorno da funcao pode ter
  vLimite  integer;
...
--------------------------------------------------------------------------------------------------------------------------
--Procedure utilizada para atualizar o Flag de DescBonus na tabela de Vale de Frete.                                    --
-- Procedure sera executada pelo Lixeiro, ate o frontEnd do Vale de Frete cobrir essa situac?o                          --
--------------------------------------------------------------------------------------------------------------------------
procedure sp_atz_DescBonusVF( pStatus out char,
                              pMessage out varchar2
                            ) is
 --Variavel utilizada apenas para contar a quantia de vale que ser?o atualizadas
 vCount integer;
 
 --Variavel utilizada para facilitar a criac?o do log
...
begin
  ...
  vLogOperacao.Log_System_Campochave := 'Atualizac?o de Flag Bonus';
  
  
  Begin
    begin
      --Abro um cursor com os Vales de Frete que ser?o atualizados.
      for vCursor in ( select 
                       ...
                       where 0 = 0
                         ...
                         or vf.con_valefrete_condespeciais like '%RECEBER COMISS?O S/BONUS DE R$%')
                      ...
                      loop
                         --Escrevo a hora do inicio da operac?o e inicio a mensagem do log
                        ...
                        --incremento a variavel contadora.
                        vCount := vCount +1;
                        begin
                          ...
                          --comito o update para ter certeza que n?o vai ocorrer nenhum erro.
                          commit;
                          
                          --Defino o logo como informativo, e n?o adiciono "OK" como observac?o.
                          vLogOperacao.Log_Tipo_Codigo := 'I';
                          vLogOperacao.Log_System_Message :=   vLogOperacao.Log_System_Message || '|' || 'OK' || '|';
                        Exception
                          --Caso ocorra algum erro durante a atualizac?o registra o log de erro.
                          when others then
                            --Descrevo o log com erro, e defino uma observac?o.
...
--------------------------------------------------------------------------------------------------------------------------
-- Procedure utilizada para Cancelar Vales de Frete, emitidos e n?o impressos dentro do prazo.                          --
-- Procedure so sera executada pelo Lixeiro - Roberto Pariz - 26/07/2013                                                --
--------------------------------------------------------------------------------------------------------------------------
procedure sp_lmp_SemImpressaoVF( pStatus out char,
                                 pMessage out varchar2
                               ) is
 --Variavel utilizada apenas para contar a quantia de vales de frete que ser?o cancelados
 vCount integer;
      ...
      for vCursor in ( select 
                      ...
                      loop
                        ...
                        begin
                          ...
                          vCond := fni_Cond_Espec(vCursor.Con_Valefrete_Condespeciais,'Cancelado Pelo Sistema! - Prazo para impress?o excedido!');
...
--------------------------------------------------------------------------------------------------------------------------------
-- PROCEDURE QUE SERA DE MANUTENC?O QUE SERA RODADA DE HORA EM HORA.                                                          --
--------------------------------------------------------------------------------------------------------------------------------
procedure SP_MANUTENCAO_HORA is
 ...
 --Variaveis que ser?o utilizadas para controlar o tempo do processo.
 vInicio date;
 vDia  integer;
 vHora integer;
...
commit;
 ...
 ------------------------------------------------------------------------------------------------
 -- ROTINA UTILIZADA PARA INFORMAR EQUIPAMENTOS DA EMPRESA SASCAR, QUE N?O RECEBERAM SINAL DE  --
 ...
 begin
    ...
    if ( ( vDia in (1, 7) ) or  ( (vHora < 8 ) or ( vHora > 18 ) ) )  then
      --lanco a excec?o de n?o operc?o.
      raise vEx_Executa;
    end if;
   ...
   --Relac?o dos enderecos que receber?o mensagens.
   vDestinoEmails:= '';
...
--   vDestinoEmails:= vDestinoEmails || 'tdv.siimg@dellavolpe.com.br';
   ...
   if (tdvadm.pkg_scr_integracao.FNP_Mct_SemSinal(vMessage)) then
     -- Defino cabecalho da mensagen caso a func?o seja executada corretamente.
     vMessage := 'ABAIXO RELAC?O DOS EQUIPAMENTOS DA SASCAR QUE N?O RECEBER?O SINAIS A MAIS DE 5 HORAS.' || chr(10)|| chr(10)
                 || vMessage || chr(13) || chr(13) 
                 || 'rotina executada: tdvadm.pkg_scr_integracao.fnp_mct_semsinal' || chr(13) 
                 || 'E-MAIL ENVIADO ATRAVES DE PROCESSO AUTOMATICO WORKFLOW DELLAVOLPE.'|| chr(13)|| 'FAVOR N?O RESPONDA ESSE E-MAIL.' 
                 || chr(13) || chr(13) 
 ...
 Exception
   --caso n?o seja necessaria a execuc?o da rotina
   when vEx_Executa then  
     --Simplesmente limpo a variavel de control
     vControl := 0;
 end;
...
-- Sirlano 03/02/2020
-- PARAMETRO DESATIVO, USANDO TABELA T_SLF_CLIREGRASEMAIL
--   Begin
...
--    end;
     ...
     -- 03/02/2019
     -- Colocado mais tres quebras para fazer o Romanerio
     -- GRUPO / CNPJ / CONTRATO
      for vCursor in (select distinct trunc(vf.con_valefrete_datacadastro) cadvalefrete,
                    ...
                    --       c.con_conhecimento_dtembarque embcte,
                      ...
                      from tdvadm.t_con_conhecimento c,
                           ...
                           tdvadm.t_con_valefrete vf
                      where 0 = 0
                        --and c.con_conhecimento_codigo = 'XXXXXX'
                        and cl.glb_grupoeconomico_codigo = cld.glb_grupoeconomico_codigo
                        ...
                        and vf.con_valefrete_status is null
                        and substr(vfc.glb_rota_codigovalefrete,1,1) <> '9'
--                       alterada para 01/07/2020, existiam varios vales de fretes antigos 
--                       Sirlano / Alison 07/07/2020
--                        and trunc(vf.con_valefrete_datacadastro) > '08/12/2015'
--                        and trunc(vf.con_valefrete_datacadastro) > '01/07/2020'
...
--                        and vfc.con_valefrete_codigo = '033330'
--                        and vfc.glb_rota_codigovalefrete = '530'
                        and vfc.con_valefrete_saque = (select min(vf1.con_valefrete_saque)
                                                       ...
                                                       where vf1.con_conhecimento_codigo = vf.con_conhecimento_codigo
                                                         ...
                                                         and nvl(vf1.con_valefrete_impresso,'N') = 'S'
                                                         and vf1.con_valefrete_status is null)
                       -- and vfc.con_vfreteconhec_transfchekin <> 'Sim' (tratando o nulo 10/08/2021)
                        and nvl(vfc.con_vfreteconhec_transfchekin,'N?o') <> 'Sim'
                        and cl.glb_grupoeconomico_codigo in (select ce.glb_grupoeconomico_codigo
                                                             from tdvadm.t_slf_cliregrasemail ce)
                        and 0 < (select count(*)
                                 from rmadm.t_glb_benasserec br
                                 --MSG=LISTNOTA;MINUTA=0421699131;GRUPO=0567;CNPJ=60894730006308;CONTRATO=C7000067442
                                 where br.glb_benasserec_assunto LIKE '%CONTRATO%'
                                   AND br.glb_benasserec_assunto LIKE 'MSG=LISTNOTA;MINUTA=%'
                                   and br.glb_benasserec_status in ('OK','IN'))
                        
--                        and instr(vListaRomaneio,cl.glb_grupoeconomico_codigo) > 0
                        and 0 = (select count(*)
                                 from wservice.t_glb_emailpend ep
                                 --MSG=LISTNOTA;MINUTA=0421699131;GRUPO=0567;CNPJ=60894730006308;CONTRATO=C7000067442
                                 where substr(ep.glb_emailpend_assunto,1,80) = substr('MSG=LISTNOTA;MINUTA=' || vfc.con_valefrete_codigo || vfc.glb_rota_codigovalefrete ||  vfc.con_valefrete_saque ||';GRUPO='||cl.glb_grupoeconomico_codigo || ';CNPJ='||trim(c.glb_cliente_cgccpfsacado)||';CONTRATO='||trim(tdvadm.pkg_slf_utilitarios.fn_retorna_contratoCod(c.con_conhecimento_codigo,c.con_conhecimento_serie,c.glb_rota_codigo)),1,80))

                        and 0 = (select count(*)
                                 from rmadm.t_glb_benasserec br
                                 --MSG=LISTNOTA;MINUTA=0421699131;GRUPO=0567;CNPJ=60894730006308;CONTRATO=C7000067442
                                 where substr(br.glb_benasserec_assunto,1,80) = substr('MSG=LISTNOTA;MINUTA=' || vfc.con_valefrete_codigo || vfc.glb_rota_codigovalefrete ||  vfc.con_valefrete_saque ||';GRUPO='||cl.glb_grupoeconomico_codigo || ';CNPJ='||trim(c.glb_cliente_cgccpfsacado)||';CONTRATO='||trim(tdvadm.pkg_slf_utilitarios.fn_retorna_contratoCod(c.con_conhecimento_codigo,c.con_conhecimento_serie,c.glb_rota_codigo)),1,80)
                                   and br.glb_benasserec_status in ('OK','IN'))
                      order by 1)
      loop
