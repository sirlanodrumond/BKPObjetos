 ...
 ****************************************************************************************************/
  ...
  cCNPJVPEDPAMCARY   Constant char(20) := '12815827000132';


  cAlterar           Constant char(2) := '01';
  cAtualizaSaldo     Constant char(2) := '02';
  cCancela           Constant char(2) := '03';
  cCancelando        Constant char(2) := '04';
  cCancelarEd        Constant char(2) := '05';
  cConcluindoCriando Constant char(2) := '06';
  cConsulta          Constant char(2) := '07';
  cCriando           Constant char(2) := '08';
  cExcluir           Constant char(2) := '09';
  cExcluirCTe        Constant char(2) := '10';
  cGravaImg          Constant char(2) := '11';
  cImprimir          Constant char(2) := '12';
  cImprimirOk        Constant char(2) := '13';
  cInclui            Constant char(2) := '14';
  cIncluirCTe        Constant char(2) := '15';
  cRetiraImpressao   Constant char(2) := '16';
  cRetTab            Constant char(2) := '17';
  cSaldoCarreito     Constant char(2) := '18';
  cSalvar            Constant char(2) := '19';
  cCarregandoDireto  Constant char(2) := '20';
 ...
 Type TpMODELO  is RECORD (CAMPO1         char(10),
   ...
   function fn_vld_PagaPedagio(pCategoria in tdvadm.t_con_catvalefrete.con_catvalefrete_codigo%type)
     return Char;
...
Procedure sp_con_validaPedagio(pVFCodigo  in tdvadm.t_con_valefrete.con_conhecimento_codigo%type,
                               pVFSerie   in tdvadm.t_con_valefrete.con_conhecimento_serie%type,
                               pVFRota    in tdvadm.t_con_valefrete.glb_rota_codigo%type,
                               pVFSaque   in tdvadm.t_con_valefrete.con_valefrete_saque%type,
                               pAplicacao in tdvadm.t_usu_aplicacao.usu_aplicacao_codigo%type default 'comvlfrete',
                               P_STATUS   out char,
                               P_MESSAGE  out varchar2);
...
CREATE OR REPLACE PACKAGE BODY PKG_CON_VALEFRETE AS
   ...
   Begin
      ...
      End ;
        ...
        vVALORRESTANTE := vMultaValeFrete;
          ...
          -- Sirlano 13/12/2021
          -- Troquei TRIM e RPAD por SUBSTR
          where substr(cc.docref,1,10) = P_CON_VALEFRETE || P_CON_ROTA || P_CON_SAQUE;
 ...
 --            vInserDebito := 'N';
          ...
          -- Sirlano 13/12/2021
          -- Troquei TRIM e RPAD por SUBSTR
                   where substr(cc.car_Contacorrente_Docref,1,10) = P_CON_VALEFRETE || P_CON_ROTA || P_CON_SAQUE;
          -- Sirlano / Bruno 1=02/02/2022
          -- Trocado o End If para depois do Loop
 ...
 --           commit;
     ...
     Begin
       ...
       P_MESSAGE := '';
             ...
             End If;
              ...
              If ( tpVF.VFAcao in (cImprimir) ) and ( NVL(P_STATUS,'N') <> pkg_glb_common.Status_Erro )   Then
              ...
              If ( tpVF.VFAcao in (cExcluir,
                                   cAlterar,
                                   cCancela) ) and ( P_STATUS <> pkg_glb_common.Status_Erro )   Then
   ...
   PROCEDURE SP_CON_ATUCALCVALEFRETE(P_QUERYSTR      IN OUT  Varchar2,
      ...
      -- Utilizando um valor qualquer para inicializar a variavel, depois e alimentado com valor do paramentro
...
-- VFNumero=nome=VFNumero|tipo=String|valor=895312*VFSerie=nome=VFSerie|tipo=String|valor=A1*VFRota=nome=VFRota|tipo=String|valor=903*VFSaque=nome=VFSaque|tipo=String|valor=1*VFUsuarioTDV=nome=VFUsuarioTDV|tipo=String|valor=bbernardo*VFRotaUsuarioTDV=nome=VFRotaUsuarioTDV|tipo=String|valor=197*VFAplicacaoTDV=nome=VFAplicacaoTDV|tipo=String|valor=comvlfrete*VFVersaoAplicao=nome=VFVersaoAplicao|tipo=String|valor=16.1.14.0*
      ...
      If  nvl(vValeFreteNumero,'Sirlano') <> 'Sirlano' Then
          ...
          vAcaoOld            := trim(TDVADM.fn_querystring(TDVADM.fn_querystring(P_QUERYSTR,'VFAcao','=','*'), 'valor', '=', '|'));
     ...
     Else
          ...
          vAcaoOld            := trim(Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'VFAcao' ));
          vCategoriaVF        := Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'CON_CATVALEFRETE_CODIGO' );
      End If;
      
   if vAcaoold = 'Alterar' Then
      VAcao := '01';
   Elsif VAcaoold = 'AtualizaSaldo' Then
      vAcao := '02';
   Elsif vAcaoold = 'Cancela' Then
      VAcao := '03';
   Elsif VAcaoold = 'Cancelando' Then
      VAcao := '04';
   Elsif VAcaoold = 'CancelarEd' Then
      VAcao := '05';
   Elsif VAcaoold = 'ConcluindoCriando' Then
      VAcao := '06';
   Elsif VAcaoold = 'Consulta' Then
      VAcao := '07';
   Elsif VAcaoold = 'Criando' Then
      VAcao := '08';
   Elsif VAcaoold = 'Excluir' Then
      VAcao := '09';
   Elsif VAcaoold = 'ExcluirCTe' Then
      VAcao := '10';
   Elsif VAcaoold = 'GravaImg' Then
      VAcao := '11';
   Elsif VAcaoold = 'Imprimir' Then
      VAcao := '12';
   Elsif VAcaoold = 'ImprimirOk' Then
      VAcao := '13';
   Elsif VAcaoold = 'Inclui' Then
      VAcao := '14';
   Elsif VAcaoold = 'IncluirCTe' Then
      VAcao := '15';
   Elsif VAcaoold = 'RetiraImpressao' Then
      VAcao := '16';
   Elsif VAcaoold = 'RetTab' Then
      VAcao := '17';
   Elsif VAcaoold = 'SaldoCarreito' Then
      VAcao := '18';
   Elsif VAcaoold = 'Salvar' Then
      VAcao := '19';
   Elsif VAcaoold = 'CarregandoDireto' Then 
      VAcao := '20'; 
   Else
      VAcao := substr(VAcaoold,1,2);
   End If;
   ...
   End if ;
    ...
    begin
     ...
     exception
       ...
       if vAcao not in (cSalvar,cConsulta) Then
          vAcao := cSalvar;
       end if;
...
--                                                                       '18',--'Coleta
                           ...
                           -- Dantas/ Sirlano 06/12/2021
                           -- Não e Frota e não teve cobranca anterior.
                        if ( (vVfFrota = false) and (vGeraTarifa = 'S') ) then
                          
                           -- Verifica se o Valor do Adiantamento suporta retirar as Tarifas.
                           select nvl(cv.con_calcvalefrete_valor,0)
                              into vValorAd
                           from tdvadm.t_con_calcvalefrete cv
                           WHERE cv.CON_CONHECIMENTO_CODIGO        = vValeFreteNumero
                             AND cv.CON_CONHECIMENTO_SERIE         = vValeFreteSerie
                             AND cv.GLB_ROTA_CODIGO                = vValeFreteRota
                             AND cv.CON_VALEFRETE_SAQUE            = vValeFreteSaque
                             and cv.CON_CALCVALEFRETETP_CODIGO = '01'; -- Adiantamento
                           ...
                           WHERE cv.CON_CONHECIMENTO_CODIGO        = vValeFreteNumero
                             ...
                             AND cv.GLB_ROTA_CODIGO                = vValeFreteRota
                             AND cv.CON_VALEFRETE_SAQUE            = vValeFreteSaque
                             and cv.CON_CALCVALEFRETETP_CODIGO = '02'; -- Pedagio
                           ...
                           WHERE cv.CON_CONHECIMENTO_CODIGO        = vValeFreteNumero
                             ...
                             AND cv.GLB_ROTA_CODIGO                = vValeFreteRota
                             AND cv.CON_VALEFRETE_SAQUE            = vValeFreteSaque
                             and cv.CON_CALCVALEFRETETP_CODIGO = '20'; -- Saldo
                            
                          If ( vValorAd <  vValorPED + vValorSaque + vValorTransferencia ) Then
                              If ( vValorAd = vValorPED ) Then
                                  -- Coloco o Adinatamento = a Pedagio + Tarifas
                                  vValorSLD := vValorSLD - ( vValorSaque + vValorTransferencia );
                                  vValorAd  := vValorAd  + ( vValorSaque + vValorTransferencia );
                              ElsIf ( vValorAd < vValorPED ) Then
                                  vValorSLD := vValorSLD - ( vValorPED - vValorAd  ) - ( vValorSaque + vValorTransferencia) ;
                                  vValorAd  := vValorAd  + ( vValorPED - vValorAd  ) + ( vValorSaque + vValorTransferencia);

                              ElsIf ( vValorAd > vValorPED ) Then
                                  vValorSLD := vValorSLD  - ( vValorSaque + vValorTransferencia) ;
                                  vValorAd  := vValorAd   + ( vValorSaque + vValorTransferencia);
                              End If;
                              -- Atualizo o novo Adiantamento e Saldo
                              UPDATE T_CON_CALCVALEFRETE DE
                              ...
                              UPDATE T_CON_CALCVALEFRETE DE
                                set de.con_calcvalefrete_valor = vValorSLD
                              WHERE DE.CON_CONHECIMENTO_CODIGO        = vValeFreteNumero
                                AND DE.CON_CONHECIMENTO_SERIE         = vValeFreteSerie
                                AND DE.GLB_ROTA_CODIGO                = vValeFreteRota
                                AND DE.CON_VALEFRETE_SAQUE            = vValeFreteSaque
 ...
 */
 ...
 /* Variáveis criadas para controlar a exigência de imagem para o CTRC e/ou NF, na criação do valefrete. */
   ...
   vXmlEntrega            clob;
   vXmlEntregaDet         clob;
   vXMLVPPagadores        clob;
   vXMLVPFornecedores     clob;
   vValorDeb              number;
   ...
   tplinhaVfretepedagio   tdvadm.t_con_vfretepedagio%rowtype;
   vValePedagio           char(1);
   vStatus                char(1);
...
--        insert into tdvadm.dropme (x , l) values ('teste03/10' , vC_QRYSTR);
      ...
      If  nvl(tpVF.VFNumero,'Sirlano') <> 'Sirlano' Then
          ...
          tpVF.VFAcaoold        := TDVADM.fn_querystring(TDVADM.fn_querystring(vC_QRYSTR,'VFAcao','=','*'), 'valor', '=', '|');
     ...
     Else
          ...
          tpVF.VFAcaoold                      := trim(Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'VFAcao' ));
...
If tpVF.VFRota in ('060','036') Then
        insert into tdvadm.t_glb_sql values (vC_QRYSTR,
                                             sysdate,
                                             'TESTE SANTOS',
                                             tpVF.VFNumero || '-' || tpVF.VFSerie || '-' || tpVF.VFRota  || '-' || tpVF.VFSaque || '-' || tpVF.VFAcaoold || '-' || tpVF.VFCON_CATVALEFRETE_CODIGO );
        commit;
...
End If;
 ...
 --     End if;
   ...
   if tpVF.VFusuarioTDV = 'jsantos' then
      P_MESSAGE := chr(10) || 'Ac Solicitada ' || tpVF.VFAcaoold || '-' || tpVF.VFCON_CATVALEFRETE_CODIGO || chr(10);
   ...
   if tpVF.VFAcaoold = 'Alterar' Then
      tpVF.VFAcao := '01';
   Elsif tpVF.VFAcaoold = 'AtualizaSaldo' Then
      tpVF.VFAcao := '02';
   Elsif tpVF.VFAcaoold = 'Cancela' Then
      tpVF.VFAcao := '03';
   Elsif tpVF.VFAcaoold = 'Cancelando' Then
      tpVF.VFAcao := '04';
   Elsif tpVF.VFAcaoold = 'CancelarEd' Then
      tpVF.VFAcao := '05';
   Elsif tpVF.VFAcaoold = 'ConcluindoCriando' Then
      tpVF.VFAcao := '06';
   Elsif tpVF.VFAcaoold = 'Consulta' Then
      tpVF.VFAcao := '07';
   Elsif tpVF.VFAcaoold = 'Criando' Then
      tpVF.VFAcao := '08';
   Elsif tpVF.VFAcaoold = 'Excluir' Then
      tpVF.VFAcao := '09';
   Elsif tpVF.VFAcaoold = 'ExcluirCTe' Then
      tpVF.VFAcao := '10';
   Elsif tpVF.VFAcaoold = 'GravaImg' Then
      tpVF.VFAcao := '11';
   Elsif tpVF.VFAcaoold = 'Imprimir' Then
      tpVF.VFAcao := '12';
   Elsif tpVF.VFAcaoold = 'ImprimirOk' Then
      tpVF.VFAcao := '13';
   Elsif tpVF.VFAcaoold = 'Inclui' Then
      tpVF.VFAcao := '14';
   Elsif tpVF.VFAcaoold = 'IncluirCTe' Then
      tpVF.VFAcao := '15';
   Elsif tpVF.VFAcaoold = 'RetiraImpressao' Then
      tpVF.VFAcao := '16';
   Elsif tpVF.VFAcaoold = 'RetTab' Then
      tpVF.VFAcao := '17';
   Elsif tpVF.VFAcaoold = 'SaldoCarreito' Then
      tpVF.VFAcao := '18';
   Elsif tpVF.VFAcaoold = 'Salvar' Then
      tpVF.VFAcao := '19';
   Elsif tpVF.VFAcaoold = 'CarregandoDireto' Then 
      tpVF.VFAcao := '20'; 
   Else
      tpVF.VFAcao := substr(tpVF.VFAcaoold,1,2);
   End If;
   ...
   Begin
       ...
       tpLinhaValeFrete.Con_Valefrete_Pedagio := nvl(tpLinhaValeFrete.Con_Valefrete_Pedagio,0);
   ...
   Exception
    ...
    If ( nvl(plistaparams('ESOCIALCONSULTA').texto,'S') = 'S' ) and 
       ...
       ( tpVF.VFAcao in (cSalvar,cImprimir) )  Then
       If tpVF.v_Proprietario Is not null Then
       ...
       End If;    
    Else
      If ( tpVF.VFAcao in (cSalvar,cImprimir) ) and 
         ( tpVf.v_TpProprietario = 'J' ) Then
         if ( nvl(tpLinhaValeFrete.Con_Valefrete_Inss,0) <> 0 ) or
            ( nvl(tpLinhaValeFrete.Con_Valefrete_Sestsenat,0) <> 0 ) Then
            P_STATUS := 'E';
            P_MESSAGE := P_MESSAGE || ' Pessoa JURIDICA como Desconto do INSS. REFAZER CALCULO !!!' || chr(10);
         End If;
      End If;   
    End If;

-- 11/03/2022 - Sirlano
-- Verificando se tem saldo e nao foi descontado   
   Begin 
      If tpVF.VFAcao in (cImprimir) Then
         If nvl(Pkg_Car_Proprietario.fn_RetornaSaldocc(tpVF.v_Proprietario),0) > 0 Then
            If tpVf.VFCON_CATVALEFRETE_CODIGO  in (CatTUmaViagem,       
               ...
               If nvl(tpVF.VFCON_VALEFRETE_MULTA,0) = 0 Then
                  If instr(nvl(plistaparams('VFRETELIBCC').texto,'000000'),tpVF.VFNumero) = 0 Then 
                     P_STATUS := 'E';
                     P_MESSAGE := P_MESSAGE || 'Proprietario com Saldo em conta corrente.' || chr(10) || 'OBRIGATORIO DESCONTAR' || chr(10);
                     If tpVF.VFRota <> tpVF.VFRotaUsuarioTDV Then
                        P_MESSAGE := P_MESSAGE || 'TROCAR A ROTA DO MENU PARA ' || tpVF.VFRota || chr(10);
                     End If;
                  End If;
               End If;
           End If;    
  ...
  If tpVF.VFCON_VALEFRETE_PLACA <> '' Then
   ...
   -- 15/02/2022 - Sirlano
   -- Trava para nao zerar o Pedagio
   -- Verifica se a Categoria paga Pedagio for S o Zera Pedagio e igual a N
   tpVF.vZeraPedagio := fn_vld_PagaPedagio(tpVF.VFCON_CATVALEFRETE_CODIGO);

   Begin
      tpVF.vPercentReduzPed :=  plistaparams('ALT_PEDVPERCT').numerico1;  
   exception
      When NO_DATA_FOUND Then
         tpVF.vPercentReduzPed := 50;
      End;
    
   tpVF.vPercentReduzPed := nvl(tpVF.vPercentReduzPed,50);
...
-- 23/02/2022
-- Verifica se Existe Vale Pedagio 
-- Gera o arquivo de Retorno
      
      vValePedagio := 'N';
      tpvf.vAuxiliar := 0;
      FOR R_CURSOR in (select --d.vFrete,
                              --d.Rota,
                              --d.saque,
                       ...
                       from (select --a.con_conhecimento_codigo vfrete,
                                    ...
                                    c.glb_cliente_cgccpfsacado sacado,
                                    clsac.glb_cliente_razaosocial razao,
                                   -- c.con_conhecimento_codigo || '-' ||c.con_conhecimento_serie || '-' || c.glb_rota_codigo cte,
                                    ...
                                    max((select vfp.con_vfretepedagio_idresponsavel
                                     from tdvadm.t_con_vfretepedagio vfp
                                     where vfp.con_conhecimento_codigo = a.con_conhecimento_codigo
                                       and vfp.con_conhecimento_serie  = a.con_conhecimento_serie
                                       and vfp.glb_rota_codigo         = a.glb_rota_codigo
                                       and vfp.con_valefrete_saque     = a.con_valefrete_saque)) idresponsavel,
                                    nvl(p.car_proprietario_rntrcdtval,trunc(sysdate)) rntrcdtval,
                                    nvl(p.car_proprietario_classdt,trunc(sysdate)) classdt,
                                    ...
                                    trim(tdvadm.pkg_slf_utilitarios.fn_retorna_contratocod(C.CON_CONHECIMENTO_CODIGO,C.CON_CONHECIMENTO_SERIE,C.GLB_ROTA_CODIGO)) CONTRATO,
                                    SUM(a.con_valefrete_pesocobrado) peso,
                                    count(*) qtde
                             From tdvadm.t_con_valefrete a,
                                  tdvadm.t_con_vfreteconhec vfc,
                                  ...
                                  tdvadm.t_car_proprietario p
                             where 0 = 0
                               and a.con_valefrete_datacadastro >= '01/01/2021'
                               and nvl(a.con_valefrete_status,'N') = 'N'
                               and vfc.arm_armazem_codigo is null
                               and a.con_conhecimento_codigo = tpVF.VFNumero
                               and a.con_conhecimento_serie  = tpVF.VFSerie
                               and a.glb_rota_codigo         = tpVF.VFRota
                               AND A.CON_VALEFRETE_Saque     = tpVF.VFSaque
                               ...
                               and vfc.glb_rota_codigo         = c.glb_rota_codigo
                               and c.glb_cliente_cgccpfsacado  = CLSac.Glb_Cliente_Cgccpfcodigo
                               and a.con_valefrete_placa      = v.car_veiculo_placa (+)
                               and a.con_valefrete_placasaque = v.car_veiculo_saque (+)
                               and v.car_proprietario_cgccpfcodigo = p.car_proprietario_cgccpfcodigo (+)
                             group by --a.con_conhecimento_codigo ,
                                      ...
                                      nvl(p.car_proprietario_rntrcdtval,trunc(sysdate)) ,
                                      nvl(p.car_proprietario_classdt,trunc(sysdate)) ,
                                      nvl(p.car_proprietario_classeqp,'N') ,
                                      nvl(p.car_proprietario_classantt,'ETC') ,
                       ...
                       order by 11,1)
      Loop
          If substr(R_CURSOR.sacado,1,8) = '61139432' Then
             tpVF.vAuxiliarTexto := R_CURSOR.sacado;
          End If;
          If R_CURSOR.ValePed = 'S' Then
             vValePedagio := 'S';
          end If;
             
          tpVF.vAuxiliar := tpVF.vAuxiliar + 1;
          vXMLVPPagadores := vXMLVPPagadores || '<row num=#'   || to_char(tpVF.vAuxiliar) ||  '#>';
          --vXMLVPPagadores := vXMLVPPagadores || '<vFrete>'     || R_CURSOR.vFrete         || '</vFrete>';
          --vXMLVPPagadores := vXMLVPPagadores || '<rota>'       || R_CURSOR.rota           || '</rota>';
          --vXMLVPPagadores := vXMLVPPagadores || '<saque>'      || R_CURSOR.saque          || '</saque>';
          vXMLVPPagadores := vXMLVPPagadores || '<contrato>'   || R_CURSOR.contrato       || '</contrato>';
          vXMLVPPagadores := vXMLVPPagadores || '<razao>'      || trim(R_CURSOR.razao)     || '</razao>';
          vXMLVPPagadores := vXMLVPPagadores || '<valeped>'    ||  R_CURSOR.ValePed       || '</valeped>';
          vXMLVPPagadores := vXMLVPPagadores || '<sacado>'     || trim(R_CURSOR.sacado)   || '</sacado>';
          vXMLVPPagadores := vXMLVPPagadores || '<idresponsavel>' || trim(R_CURSOR.idresponsavel)   || '</idresponsavel>';
          --vXMLVPPagadores := vXMLVPPagadores || '<cte>'        || R_CURSOR.cte            || '</cte>';
          vXMLVPPagadores := vXMLVPPagadores || '<pedagioori>' || tdvadm.f_mascara_valor(tpLinhaValeFrete.Con_Valefrete_Pedagiooriginal,12,2) || '</pedagioori>';
          vXMLVPPagadores := vXMLVPPagadores || '<pedagio>'    || tdvadm.f_mascara_valor(tpLinhaValeFrete.Con_Valefrete_Pedagio,12,2)         || '</pedagio>';
          vXMLVPPagadores := vXMLVPPagadores || '<tipomot>'    || R_CURSOR.tipomot        || '</tipomot>';
          vXMLVPPagadores := vXMLVPPagadores || '<placa>'      || R_CURSOR.placa          || '</placa>';
          vXMLVPPagadores := vXMLVPPagadores || '<rntrcdtval>' || R_CURSOR.rntrcdtval     || '</rntrcdtval>';
          vXMLVPPagadores := vXMLVPPagadores || '<classdt>'    || R_CURSOR.classdt        || '</classdt>';
          vXMLVPPagadores := vXMLVPPagadores || '<classeqp>'   || R_CURSOR.classeqp       || '</classeqp>';
          vXMLVPPagadores := vXMLVPPagadores || '<classantt>'  || R_CURSOR.classantt      || '</classantt>';
          vXMLVPPagadores := vXMLVPPagadores || '<cliente>'    || R_CURSOR.cliente        || '</cliente>';
          vXMLVPPagadores := vXMLVPPagadores || '<grupo>'      || R_CURSOR.grupo          || '</grupo>';
          vXMLVPPagadores := vXMLVPPagadores || '<peso>'       || R_CURSOR.Peso           || '</peso>';
          vXMLVPPagadores := vXMLVPPagadores || '<qtde>'       || R_CURSOR.qtde           || '</qtde>';
          vXMLVPPagadores := vXMLVPPagadores || '</row>';

      End Loop;
      
      If tpvf.vAuxiliar = 0 Then
         vValePedagio := 'N';
         FOR R_CURSOR in (select tdvadm.pkg_slf_contrato.FN_VALIDAVALEPEDAGIO(d.sacado,d.CONTRATO,d.grupo) ValePed,
                                 d.contrato,
                                 d.sacado,
                                 d.razao,
                                 d.pedagio,
                                 ...
                                 d.peso,
                                 d.qtde
                          from (select c.arm_coleta_cnpjpagadorferete sacado,
                                       clsac.glb_cliente_razaosocial razao,
                                       a.con_valefrete_pedagio pedagio,
                                       a.glb_tpmotorista_codigo tipomot,
                                       a.con_valefrete_placa placa,
   ...
   --                               and vfc.arm_armazem_codigo is null
                                  ...
                                  and a.con_conhecimento_serie  = tpVF.VFSerie
                                  and a.glb_rota_codigo         = tpVF.VFRota
                                  AND A.CON_VALEFRETE_Saque     = tpVF.VFSaque--                               and a.con_valefrete_pedagio > 0
                                  and a.con_conhecimento_codigo = vfc.con_valefrete_codigo
                                  and a.con_conhecimento_serie  = vfc.con_valefrete_serie
                                  and a.glb_rota_codigo         = vfc.glb_rota_codigovalefrete
                                  and a.con_valefrete_saque     = vfc.con_valefrete_saque
                                  and vfc.arm_coleta_ncompra    = c.arm_coleta_ncompra
                                  and vfc.arm_coleta_ciclo      = c.arm_coleta_ciclo
                                  and c.arm_coleta_cnpjpagadorferete = CLSac.Glb_Cliente_Cgccpfcodigo
                                  and a.con_valefrete_placa      = v.car_veiculo_placa (+)
                                  and a.con_valefrete_placasaque = v.car_veiculo_saque (+)
                                  and v.car_proprietario_cgccpfcodigo = p.car_proprietario_cgccpfcodigo (+)
                                group by c.arm_coleta_cnpjpagadorferete ,
                                         a.con_valefrete_pedagio ,
                                         a.glb_tpmotorista_codigo ,
                                         a.con_valefrete_placa ,
                                         nvl(p.car_proprietario_rntrcdtval,trunc(sysdate)) ,
    ...
    -- 15/02/2022 - Sirlano
    -- Validacao se reduziu mais do que o permitido no pedagio
    If tpVF.VFAcao in (cImprimir) Then
       If ( tpVF.vZeraPedagio = 'N' ) and 
          ( vValePedagio = 'N' ) and
          ( tpVF.vPercentReduzPed < 100 ) and 
          ( nvl(tpLinhaValeFrete.Con_Valefrete_Pedagiooriginal,0) <> 0 ) and 
          ( nvl(tpLinhaValeFrete.Con_Valefrete_Impresso,'N') = 'N' ) and 
          ( nvl(tpLinhaValeFrete.Con_Valefrete_Pedagio,0) <> nvl(tpLinhaValeFrete.Con_Valefrete_Pedagiooriginal,0) ) Then

          select count(*)
             into tpVF.vAuxiliar2
          from tdvadm.t_con_calcvalefrete cvf
          where cvf.con_conhecimento_codigo = tpVF.VFNumero
            and cvf.con_conhecimento_serie = tpVF.VFSerie
            and cvf.glb_rota_codigo = tpVF.VFRota
            and cvf.con_valefrete_saque = tpVF.VFSaque
            and cvf.con_calcvalefretetp_codigo = '02'
            and nvl(cvf.con_calcvalefrete_valor,0) <> 0;
            -- Nao existe pedagio no CIOT
          If tpVF.vAuxiliar2 > 0 Then

             tpVF.vAuxiliar := round(tpLinhaValeFrete.Con_Valefrete_Pedagio / nvl(tpLinhaValeFrete.Con_Valefrete_Pedagiooriginal,0),2); 
             If tpVf.vAuxiliar < round(( tpVF.vPercentReduzPed / 100 ),2) Then
               P_STATUS :=  PKG_GLB_COMMON.Status_Erro;
               P_MESSAGE := P_MESSAGE || 'Pedagio menor do que o Permitido! ' || chr(10) ||
                                         'Ped Original ' || tpLinhaValeFrete.Con_Valefrete_Pedagiooriginal || chr(10) ||
                                         'Ped Atual    ' || tpLinhaValeFrete.Con_Valefrete_Pedagio || chr(10) || 
                                         '% Redução Permitida ' || tpVF.vPercentReduzPed || '%' || chr(10) ||
                                         'Calculado ' || tpVF.vAuxiliar || chr(10) ;
...
--         and vf.glb_rota_codigo <> tpVf.VFRota
    ...
    End If;
     ...
     If tpVF.VFAcao = '00' Then
        tpVF.VFAcao := tpVF.VFAcao; 
     ElsIf tpVF.VFAcao = cExcluirCTe Then
     ...
     ElsIf tpVF.VFAcao = cImprimirOk Then
...
*/  
          ...
          where vf.con_conhecimento_codigo = tpVF.VFNumero
              ...
              and vf.con_valefrete_saque = tpVF.VFSaque;
          --14/02/2022 - Sirlano
          -- Colocar como usado o Veiculo DISP
   ...
   If tpVF.VFAcao in (cCriando,cConcluindoCriando) Then
      tpVF.vAuxiliar := 0;
    ...
    End IF;      
   End If;
        If tpVF.VFAcao = cSaldoCarreito Then
        ...
        ElsIf tpVF.VFAcao In (cRetTab) Then
           P_STATUS  := pkg_glb_common.Status_Nomal;
              if tpVF.VFVersaoAplicao = '13.1.23.1' Then
                 P_MESSAGE := ' ' ; -- Processamento normal!';
              Else
 ...
 --                              And vf.con_valefrete_saque     = tpVF.vUltimoSaqueValido
        ...
        ElsIf tpVF.VFAcao In (cConcluindoCriando) Then
...
*/                 
      ...
      If tpVF.VFAcao Not In (cCriando,
                             cConcluindoCriando,
                             cConsulta,
                             cAtualizaSaldo) Then
...
--                tpVf.vAuxiliarTexto := 'R';
         ...
         if ( tpvf.VFCON_CATVALEFRETE_CODIGO in (CatTEstadia) ) Then
            ...
            where vf.fcf_veiculodisp_codigo = vd.fcf_veiculodisp_codigo
             ...
             if ( V_QTDEIMG < vQtdeEntregas ) or
                ( V_CONTADORREC > 0 
                 or V_CONTADORNAPROV > 0 )  
                 --Sirlano \ Vinicius Rezende
                 --14/12/2021 Foi reativado o contador de imagens não aprovadas Chamado: 250267
                 --So vale para o Caixa se não estiver aprovada
 --              and ( trunc(sysdate) > to_date('12/01/2015','dd/mm/yyyy'))
       ...
       End If;
          ...
          If tpVF.VFAcao = cConcluindoCriando Then
         ...
         If ( tpVF.VFAcao = cSalvar ) Then 
          Begin
            ...
            -- Grava os Pagadores e Fornecedor de pedagio
            update tdvadm.t_con_vfretepedagio x
              set x.Glb_Fornecedor_Cgccpf = Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'vfornecedor' ),
                  x.glb_cliente_cgccpfcodigo = Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'vpagador' ),
                  x.con_vfretepedagio_idresponsavel = Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'idresponsavel' )
            where x.con_conhecimento_codigo = tpVF.VFNumero
              and x.con_conhecimento_serie  = tpVF.VFSerie
              and x.glb_rota_codigo         = tpVF.VFRota
              and x.con_valefrete_saque     = tpVf.VFSaque;
            If ( sql%rowcount = 0 ) and (tpLinhaValeFrete.Con_Valefrete_Pedagio > 0 ) Then

               insert into tdvadm.t_con_vfretepedagio
               select c.con_conhecimento_codigo,
                      c.con_conhecimento_serie,
                      c.glb_rota_codigo,
                      c.con_valefrete_saque,
                      c.con_freteoper_id,
                      c.con_freteoper_rota,
                      c.con_vfreteciot_id,
                      c.con_vfreteciot_idcliente,
                      c.con_vfreteciot_tppagamento,
                      c.con_vfreteciot_flagcancel,
                      c.con_vfreteciot_data,
                      c.con_vfreteciot_flagimprime,
                      c.con_vfreteciot_flagaltera,
                      c.con_vfreteciot_flagprocesal,
                      Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'vfornecedor' ),
                      Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'vpagador' ),
                      Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam( tpVF.vXmlTab,'idresponsavel' )
               from tdvadm.t_con_vfreteciot c
               where c.con_conhecimento_codigo = tpVF.VFNumero
                 and c.con_conhecimento_serie  = tpVF.VFSerie
                 and c.glb_rota_codigo         = tpVF.VFRota
                 and c.con_valefrete_saque     = tpVf.VFSaque;
               if ( sql%rowcount = 0 ) and 
                  ( tpVF.vClassantt = 'ETC' ) and 
                  ( tpVF.vClasseqp = 'N' ) Then
                 tplinhaVfretepedagio.Con_Conhecimento_Codigo := tpVF.VFNumero;
                 tplinhaVfretepedagio.Con_Conhecimento_Serie  := tpVF.VFSerie;
                 tplinhaVfretepedagio.Glb_Rota_Codigo         := tpVF.VFRota;
                 tplinhaVfretepedagio.Con_Valefrete_Saque     := tpVf.VFSaque;
                 Begin
                    insert into tdvadm.t_con_vfretepedagio values tplinhaVfretepedagio;
                    commit;
                 exception
                    When OTHERS Then
                       tplinhaVfretepedagio.Con_Valefrete_Saque := tplinhaVfretepedagio.Con_Valefrete_Saque;
                    End;
               End IF;
               commit;          
            End If;      
         End If;   
          ...
          If ( tpVF.VFAcao = cRetiraImpressao ) and ( tpVF.tpValeFrete.Con_Valefrete_Impresso = 'N' ) Then
             P_STATUS :=  PKG_GLB_COMMON.Status_Erro;
             P_MESSAGE := P_MESSAGE || '03-Vale Não Esta Impresso .' || chr(10);
          ElsIf ( tpVF.VFAcao not in (cRetiraImpressao,cGravaImg )) and ( tpVF.tpValeFrete.Con_Valefrete_Impresso = 'S' ) Then
...
--          End If;
 ...
 --          tpVF.vXmlTab := tpVF.vXmlTab|| '</tables>';
      ...
      If tpVF.VFAcao = '00' Then
         tpVF.VFAcao := tpVF.VFAcao;
      ElsIf tpVF.VFAcao = cCriando Then
      ...
      ElsIf tpVF.VFAcao = cGravaImg Then
 ...
 --                   tpVf.vAuxiliar2 := tpVf.vAuxiliar2
      ...
      ElsIf tpVF.VFAcao in (cSalvar,cImprimir) Then
        ...
        Else
           ...
           if tpVF.vAuxiliar = 1 Then
              ...
              where vfe.con_conhecimento_codigo = tpVF.VFNumero
                ...
                -- 14/04/2022 - Sirlano
                -- Retirado a pedido do Romario e Ray
...
--                    P_STATUS := pkg_glb_common.Status_Erro ;
         ...
         End If;
          ...
          -- Sirlano 13/12/2021
          -- Troquei TRIM e RPAD por SUBSTR
               where substr(cc.car_Contacorrente_Docref,1,10) = tpVF.VFNumero || tpVF.VFRota || tpVF.VFSaque
                 and 0 = (select count(*)
                          from tdvadm.t_con_valefrete vf
                          where vf.con_conhecimento_codigo = tpVF.VFNumero
                            and vf.glb_rota_codigo = tpVF.VFRota
          -- Sirlano 13/12/2021
          -- Inclui a Serie por ter Vales da seria A0 na 180
                            and vf.con_conhecimento_serie = tpVF.VFSerie
                            and vf.con_valefrete_saque = tpVF.VFSaque
                            and nvl(vf.con_valefrete_impresso,'N') = 'S');
              end if;
          ...
          -- Sirlano 13/12/2021
          -- Troquei TRIM e RPAD por SUBSTR
            where substr(cc.docref,1,10) = tpVF.VFNumero || tpVF.VFRota || tpVF.VFSaque
              and cc.car_contacorrente_tplanc = 'C' ;

          Else
          -- Sirlano 15/12/2021
          -- Se o Vale ja esta IMPRESSO, igual o auxiliar ao valor da multa
            tpvf.vAuxiliar := tpVF.VFCON_VALEFRETE_MULTA;
          End If;  
          tpvf.vAuxiliar := nvl(tpvf.vAuxiliar,0);
     ...
     End If;
      ...
      if tpVF.VFAcao = '00' Then
         tpVF.VFAcao := tpVF.VFAcao;
      Elsif tpVF.VFAcao = cRetiraImpressao Then
         if nvl(trim(P_STATUS),'N') <> pkg_glb_common.Status_Erro Then
 ...
 --                          SELECT * FROM T_CON_VALEFRETE
            ...
            End If;
         End If;
      ElsIf tpVF.VFAcao = cSalvar Then
...
-- CAD - Cadastrado
 ...
 --                             tpVF.tpContaCorrente.car_contacorrente_docorigem := tpVF.VFNumero || tpVF.VFSerie || tpVF.VFRota || tpVF.VFSaque;
                      ...
                      End Loop;
          -- Sirlano / Bruno 1=02/02/2022
          -- End If depois do for da inserção do CC
                      End If;
                 End If;
     --            tem que devolver o valor descontado
      ...
      Elsif tpVF.VFAcao = cCancelando Then
        If P_STATUS = tdvadm.pkg_glb_common.Status_Nomal Then
          Begin
           update tdvadm.t_con_valefrete vf
      ...
      Elsif tpVF.VFAcao in (cExcluir,
         ...
         If nvl(trim(P_STATUS),'N') <> PKG_GLB_COMMON.Status_Erro Then
          ...
          -- Sirlano 13/12/2021
          -- Troquei TRIM e RPAD por SUBSTR
                Where substr(Car_Contacorrente_Docref,1,10) = tpVF.VFNumero || tpVF.VFRota || tpVF.VFSaque
                 and 0 = (select count(*)
                          from tdvadm.t_con_valefrete vf
                          where vf.con_conhecimento_codigo = tpVF.VFNumero
                            and vf.glb_rota_codigo = tpVF.VFRota
                ...
                ElsIf  tpVF.VFAcao = cCancela Then
                    update t_con_valefrete vf
                       set vf.con_valefrete_status = 'C'
                    Where VF.CON_CONHECIMENTO_CODIGO = tpVF.VFNumero
                      And VF.CON_CONHECIMENTO_SERIE = tpVF.VFSerie
      ...
      If nvl(trim(P_STATUS),'N') = PKG_GLB_COMMON.Status_Nomal Then
         ...
         If nvl(INSTR(upper(tpVF.tpValeFrete.Con_Valefrete_Obs),'VERSAO'),0) = 0 Then
            ...
            Where VF.CON_CONHECIMENTO_CODIGO = tpVF.VFNumero
              ...
              And VF.GLB_ROTA_CODIGO         = tpVF.VFRota
              And VF.CON_VALEFRETE_SAQUE     = tpVF.VFSaque
              -- Altera somente se nao estiver impresso
              -- Sirlano 04/02/2022
...
--      vXMLVPPagadores := empty_clob;
      tpVF.vAuxiliar := 0;
      tpVF.vAuxiliarTexto := null;

      If tpVF.vAuxiliarTexto is null Then
          tpVF.vAuxiliar := tpVF.vAuxiliar + 1;
          vXMLVPPagadores := vXMLVPPagadores || '<row num=#'   || to_char(tpVF.vAuxiliar) ||  '#>';
          --vXMLVPPagadores := vXMLVPPagadores || '<vFrete></vFrete>';
          --vXMLVPPagadores := vXMLVPPagadores || '<rota></rota>';
          --vXMLVPPagadores := vXMLVPPagadores || '<saque></saque>';
          vXMLVPPagadores := vXMLVPPagadores || '<contrato>999999999</contrato>';
          vXMLVPPagadores := vXMLVPPagadores || '<razao>TRANSPORTES DELLA VOLPE SA COM IND</razao>'; 
          vXMLVPPagadores := vXMLVPPagadores || '<valePed>N</valePed>';
          vXMLVPPagadores := vXMLVPPagadores || '<sacado>61139432000172</sacado>';
          vXMLVPPagadores := vXMLVPPagadores || '<idresponsavel>' || '1                   '   || '</idresponsavel>';
          --vXMLVPPagadores := vXMLVPPagadores || '<cte></cte>';
          vXMLVPPagadores := vXMLVPPagadores || '<pedagioori>' || tdvadm.f_mascara_valor(tpLinhaValeFrete.Con_Valefrete_Pedagiooriginal,12,2) || '</pedagioori>';
          vXMLVPPagadores := vXMLVPPagadores || '<pedagio>'    || tdvadm.f_mascara_valor(tpLinhaValeFrete.Con_Valefrete_Pedagio,12,2)         || '</pedagio>';
          vXMLVPPagadores := vXMLVPPagadores || '<tipomot></tipomot>';
          vXMLVPPagadores := vXMLVPPagadores || '<placa></placa>';
          vXMLVPPagadores := vXMLVPPagadores || '<rntrcdtval></rntrcdtval>';
          vXMLVPPagadores := vXMLVPPagadores || '<classdt></classdt>';
          vXMLVPPagadores := vXMLVPPagadores || '<classeqp></classeqp>';
          vXMLVPPagadores := vXMLVPPagadores || '<classantt></classantt>';
          vXMLVPPagadores := vXMLVPPagadores || '<cliente></cliente>';
          vXMLVPPagadores := vXMLVPPagadores || '<grupo></grupo>';
          vXMLVPPagadores := vXMLVPPagadores || '<peso></peso>';
          vXMLVPPagadores := vXMLVPPagadores || '<qtde></qtde>';
          vXMLVPPagadores := vXMLVPPagadores || '</row>';
      ...
      If tpVF.vAuxiliar = 0 Then
          ...
          vXMLVPPagadores := vXMLVPPagadores || '<row num=#1#>';
          --vXMLVPPagadores := vXMLVPPagadores || '<vFrete></vFrete>';
          --vXMLVPPagadores := vXMLVPPagadores || '<rota></rota>';
          --vXMLVPPagadores := vXMLVPPagadores || '<saque></saque>';
          vXMLVPPagadores := vXMLVPPagadores || '<contrato></contrato>';
          vXMLVPPagadores := vXMLVPPagadores || '<razao></razao>'; 
          vXMLVPPagadores := vXMLVPPagadores || '<valePed></valePed>';
          vXMLVPPagadores := vXMLVPPagadores || '<sacado></sacado>';
          vXMLVPPagadores := vXMLVPPagadores || '<idresponsavel></idresponsavel>';
          --vXMLVPPagadores := vXMLVPPagadores || '<cte></cte>';
          vXMLVPPagadores := vXMLVPPagadores || '<pedagioori></pedagioori>';
          vXMLVPPagadores := vXMLVPPagadores || '<pedagio></pedagio>';
          vXMLVPPagadores := vXMLVPPagadores || '<tipomot></tipomot>';
          vXMLVPPagadores := vXMLVPPagadores || '<placa></placa>';
          vXMLVPPagadores := vXMLVPPagadores || '<rntrcdtval></rntrcdtval>';
          vXMLVPPagadores := vXMLVPPagadores || '<classdt></classdt>';
      ...
      End IF;
       ...
       vXMLVPPagadores := '<table name=#PagadorVP#><rowset>'|| vXMLVPPagadores || '</rowset></table>';

       vXMLVPPagadores := Replace( vXMLVPPagadores, '#', '''' );

/***************************************************/

      vXMLVPFornecedores := empty_clob;
      tpVF.vAuxiliar := 0;
      FOR R_CURSOR in (select f.glb_fornecedor_cgccpf cnpj,
                              f.glb_fornecedor_nome fornecedor
                       from tdvadm.t_glb_fornecedor f
                       where 0 = 0
                         and f.glb_fornecedor_valepedagio = 'S' )
      Loop

          tpVF.vAuxiliar := tpVF.vAuxiliar + 1;
          vXMLVPFornecedores := vXMLVPFornecedores || '<row num=#'   || to_char(tpVF.vAuxiliar)   ||  '#>';
          vXMLVPFornecedores := vXMLVPFornecedores || '<cnpj>'       || trim(R_CURSOR.cnpj)       || '</cnpj>';
          vXMLVPFornecedores := vXMLVPFornecedores || '<fornecedor>' || trim(R_CURSOR.fornecedor) || '</fornecedor>';
          vXMLVPFornecedores := vXMLVPFornecedores || '</row>';
        
      End Loop;
      
      If tpVF.vAuxiliar = 0 Then
        
          vXMLVPFornecedores := vXMLVPFornecedores || '<row num=#1#>';
          vXMLVPFornecedores := vXMLVPFornecedores || '<cnpj></cnpj>';
          vXMLVPFornecedores := vXMLVPFornecedores || '<fornecedor></fornecedor>';
          vXMLVPFornecedores := vXMLVPFornecedores || '</row>';

      End IF;

       vXMLVPFornecedores := '<table name=#FornecedorVP#><rowset>'|| vXMLVPFornecedores || '</rowset></table>';

       vXMLVPFornecedores := Replace( vXMLVPFornecedores, '#', '''' );
    ...
    vXmlImagem := Replace( vXmlImagem, '#', '''' );
     ...
     -- 12/05/2022 - Sirlano
     -- Mudei o conceito de atualizar a tabela de VfretePedagio
     Begin
        select *
          into tplinhaVfretepedagio
        from tdvadm.t_con_vfretepedagio x
        where x.con_conhecimento_codigo = tpVF.VFNumero
          and x.con_conhecimento_serie  = tpVF.VFSerie
          and x.glb_rota_codigo         = tpVF.VFRota
          and x.con_valefrete_saque     = tpVf.VFSaque;
     Exception
        When NO_DATA_FOUND Then
           If (tpLinhaValeFrete.Con_Valefrete_Pedagio > 0 ) Then
                 insert into tdvadm.t_con_vfretepedagio
                 select c.con_conhecimento_codigo,
                        c.con_conhecimento_serie,
                        c.glb_rota_codigo,
                        ...
                        cCNPJVPEDPAMCARY,
                        '61139432000172',
                        trim(to_char(c.con_vfreteciot_id))
                 from tdvadm.t_con_vfreteciot c
                 where c.con_conhecimento_codigo = tpVF.VFNumero
                   and c.con_conhecimento_serie  = tpVF.VFSerie
                   and c.glb_rota_codigo         = tpVF.VFRota
     ...
     tpVF.vXmlTab := tpVF.vXmlTab|| '<vfornecedor>'         || tplinhaVfretepedagio.Glb_Fornecedor_Cgccpf    || '</vfornecedor>';
     tpVF.vXmlTab := tpVF.vXmlTab|| '<vpagador>'            || tplinhaVfretepedagio.Glb_Cliente_Cgccpfcodigo || '</vpagador>';
     tpVF.vXmlTab := tpVF.vXmlTab|| '<idresponsavel>'       || rpad(tplinhaVfretepedagio.Con_Vfretepedagio_Idresponsavel,20) || '</idresponsavel>'; 
     tpVF.vXmlTab := tpVF.vXmlTab|| '<tables>';
     tpVF.vXmlTab := tpVF.vXmlTab|| vXmlImagem;
     tpVF.vXmlTab := tpVF.vXmlTab|| vXmlEntrega;
     tpVF.vXmlTab := tpVF.vXmlTab|| vXmlEntregaDet;
     tpVF.vXmlTab := tpVF.vXmlTab|| vXMLVPFornecedores;
     tpVF.vXmlTab := tpVF.vXmlTab|| vXMLVPPagadores;
     tpVF.vXmlTab := tpVF.vXmlTab|| '</tables>';
     tpVF.vXmlTab := tpVF.vXmlTab|| '</output></parametros>';
   ...
   Begin
     ...
     Begin
        ...
        where vf.con_conhecimento_codigo = p_vfnumero
          ...
          and vf.glb_rota_codigo = p_vfrota
          and vf.con_valefrete_saque = p_vfsaque;
       vTpValefrete.Con_Valefrete_Pedagio := nvl(vTpValefrete.Con_Valefrete_Pedagio,0);
     Exception
       When NO_DATA_FOUND Then
          p_Status := 'E';
...
-- O Sistema pega o Maior que é o 'S'
 ...
 begin
    Begin
       select NVL(MAX(cr.slf_clienteregras_bonus),'S') bonus,NVL(MAx(cr.slf_clienteregras_bonusfrota),'S') bonusfrota,count(*)
          into vDescBonus,
               vDescBonusFrota,
               vcontador
       from tdvadm.t_slf_clienteregras cr
       where 0 = 0
         ...
         and ( nvl(cr.slf_clienteregras_bonus,'N') = 'N'
            or nvl(cr.slf_clienteregras_bonusfrota,'N') = 'N');
         -- 25/05/2022 Sirlano
         -- controle se desconta BONUS para VF de Coleta
         If vcontador = 0 Then

            select max(cr.slf_clienteregras_bonus),
                   max(cr.slf_clienteregras_bonusfrota),
                   count(*)
                 into vDescBonus,
                      vDescBonusFrota,
                      vcontador
            from --tdvadm.t_con_valefrete a,
                 tdvadm.t_con_vfretecoleta vfc,
                 tdvadm.t_arm_coleta co,
                 tdvadm.t_slf_clienteregras cr
            WHERE vfc.con_valefrete_codigo = pValeFreteCod
              AND vfc.con_valefrete_serie  = pValeFrereSerie
              AND vfc.glb_rota_codigovalefrete = pValeFreteRota
              and vfc.con_valefrete_saque = pValeFreteSaque
              and vfc.arm_coleta_ncompra = co.arm_coleta_ncompra
              and vfc.arm_coleta_ciclo = co.arm_coleta_ciclo
              and co.slf_contrato_codigo = cr.slf_contrato_codigo
              and cr.slf_clienteregras_ativo = 'S'
              and cr.slf_clienteregras_vigencia = (select max(cr1.slf_clienteregras_vigencia)
                                                   from tdvadm.t_slf_clienteregras cr1
                                                   where cr1.glb_grupoeconomico_codigo = cr.glb_grupoeconomico_codigo
                                                     ...
                                                     and cr1.slf_contrato_codigo = cr1.slf_contrato_codigo
                                                     and cr1.slf_clienteregras_ativo = 'S'
                                                     and cr1.slf_clienteregras_vigencia <= sysdate);
...
As
  ...
  plistaparams     glbadm.pkg_listas.tlistausuparametros;
  vAuxiliar        number;
  vUsuario         tdvadm.t_con_valefrete.usu_usuario_codalteracao%Type;
  vRota            tdvadm.t_con_valefrete.glb_rota_codigo%type;
  vPedagio         tdvadm.t_con_valefrete.con_valefrete_pedagio%type;
  vPedagioORI      tdvadm.t_con_valefrete.con_valefrete_pedagiooriginal%type;
  vCategoria       tdvadm.t_con_valefrete.con_catvalefrete_codigo%type;
  vImpresso        tdvadm.t_con_valefrete.con_valefrete_impresso%type;
  vPercentReduzPed number;
  vZeraPedagio     char(1);
  vValePedagio     char(1);
begin
   ...
   begin
       Select vf.usu_usuario_codalteracao,
              vf.glb_rota_codigo,
              nvl(vf.con_valefrete_pedagio,0),
              nvl(vf.con_valefrete_pedagiooriginal,0),
              vf.con_catvalefrete_codigo,
              nvl(vf.con_valefrete_impresso,'N')
         into vUsuario,
              vRota,
              vPedagio,
              vPedagioORI,
              ...
              vImpresso
       From tdvadm.t_con_valefrete vf
       where vf.con_conhecimento_codigo = pVFCodigo
         and vf.con_conhecimento_serie  = pVFSerie
         and vf.glb_rota_codigo         = pVFRota
         and vf.con_valefrete_saque     = pVFSaque;
   ...
   If vPedagioORI > 0 Then
  
      If Not glbadm.pkg_listas.fn_get_usuparamtros(pAplicacao,
                                                   vUsuario,
                                                   vRota,
                                                   plistaparams) Then
      ...
      vZeraPedagio := nvl(tdvadm.pkg_con_valefrete.fn_vld_PagaPedagio(vCategoria),'N');

      If ( vZeraPedagio = 'N' ) and 
         ( vPedagioORI <> 0 ) and 
         ( vImpresso = 'N' ) and 
         ( vPedagio <> vPedagioORI ) Then
 
         Begin
            vPercentReduzPed :=  plistaparams('ALT_PEDVPERCT').numerico1;  
         exception
            When NO_DATA_FOUND Then
               vPercentReduzPed := 50;
         End;
         ...
         vAuxiliar := round(vPedagio / vPedagioORI ); 
         If ( vAuxiliar < round(( vPercentReduzPed / 100 ),2) ) and 
            ( vPercentReduzPed < 100 )  then
            
-- 25/02/2022 -- Validando se é vale Pedagio
            vValePedagio := 'N';
            FOR R_CURSOR in (select tdvadm.pkg_slf_contrato.FN_VALIDAVALEPEDAGIO(d.sacado,d.CONTRATO,d.grupo) ValePed,
                                 d.contrato,
                                 d.sacado,
                          ...
                          from (select c.arm_coleta_cnpjpagadorferete sacado,
                                ...
                                where 0 = 0
                                  ...
                                  and nvl(a.con_valefrete_status,'N') = 'N'
   --                               and vfc.arm_armazem_codigo is null
                                  and a.con_conhecimento_codigo = pVFCodigo
                                  and a.con_conhecimento_serie  = pVFSerie
                                  and a.glb_rota_codigo         = pVFRota
                                  AND A.CON_VALEFRETE_Saque     = pVFSaque--                               and a.con_valefrete_pedagio > 0
                                  and a.con_conhecimento_codigo = vfc.con_valefrete_codigo
                                  and a.con_conhecimento_serie  = vfc.con_valefrete_serie
                                  and a.glb_rota_codigo         = vfc.glb_rota_codigovalefrete
                                  and a.con_valefrete_saque     = vfc.con_valefrete_saque
...
--         
            ...
            If ( vValePedagio = 'N' ) Then
            
               P_STATUS :=  PKG_GLB_COMMON.Status_Erro;
               P_MESSAGE := P_MESSAGE || 'Pedagio menor do que o Permitido! ' || chr(10) ||
                                         'Ped Original ' || vPedagioORI || chr(10) ||
                                         'Ped Atual    ' || vPedagio || chr(10) || 
                                         '% Redução Permitida ' ||  vPercentReduzPed || '%' || chr(10) ||
                                         'Calculado ' || vAuxiliar || chr(10) ;
      ...
      Else
         P_STATUS :=  PKG_GLB_COMMON.Status_Nomal;
         P_MESSAGE := P_MESSAGE || 'Pedagio não alterado';
      End If;
  Else
     P_STATUS :=  PKG_GLB_COMMON.Status_Nomal;
     P_MESSAGE := P_MESSAGE || 'Não existe pedagio a ser validado';
  End If;
  
end sp_con_validaPedagio; 
