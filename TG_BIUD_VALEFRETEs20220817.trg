...
DECLARE
  ...
  ******************************************************************************/
  ...
  vBonusContrato  tdvadm.t_slf_clienteregras.slf_clienteregras_bonus%type;
  vBonusFContrato tdvadm.t_slf_clienteregras.slf_clienteregras_bonusfrota%type;
...
*/  
         ...
         Begin                                             
               If UPPER(nvl(vListaparams('ATUALIZA_PEDAGIO').texto, 'S')) = 'S' Then
                  ...
                  (PKG_CFE_FRETE.FN_GET_VALORPED(:new.glb_localidade_codigoori,
                                                 ...
                                                 NVL(:new.glb_localidade_codigopasspor,'99999'),
  ...
  ******************************************************************************/
  IF UPDATING('CON_VALEFRETE_LOTACAO')         or
     ...
     -- 13/12/2021 Sirlano / Vinicius Dantas
     -- Verificando se foi impresso e colocando a data da impressao
     -- Impressao foi retirada
     If nvl(:new.con_valefrete_impresso,'N') = 'N' and nvl(:old.con_valefrete_impresso,'N') <> 'N' Then
        :new.con_valefrete_dthoraimpressao := null;

     -- Impressao feita
     ElsIf nvl(:new.con_valefrete_impresso,'N') <> 'N' and nvl(:old.con_valefrete_impresso,'N') = 'N' Then
        :new.con_valefrete_dthoraimpressao := sysdate;
     End If;
    ...
    -- 21/04/2022 Sirlano
    -- Retirado devido ao default ficar como zero
--    IF (nvl(v_AgregadoPerc,0) = 0) AND (v_AgregadoPercPlaca = -1)THEN
--       v_AgregadoPerc := 5;
--    END IF;   
...
-- Retirei a CHUMBADA abaixo e coloquei a funcao acima
    If :new.glb_rota_codigo = '060'  Then
      If nvl(:new.con_valefrete_descbonus,'N') <> 'S' Then
          ...
          If :new.con_valefrete_descbonus = 'S' Then
