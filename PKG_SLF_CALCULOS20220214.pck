...
-- 16404287044870
  ...
  function fn_Troca_tags(pConConhecimento IN T_CON_CONHECIMENTO.CON_CONHECIMENTO_CODIGO%TYPE,
   ...
   PROCEDURE SP_VERIFICA_ISENCAO_SUBST(P_SOLFRETE_CODIGO    IN CHAR, -- PODE SE UMA SOL OU TAB
                                       ...
                                       P_LEI                IN OUT tdvadm.t_con_conhecimento.con_conhecimento_obslei%TYPE,
...
CREATE OR REPLACE PACKAGE BODY PKG_SLF_CALCULOS AS
  ...
  V_CLIENTE_DESTINATARIOC T_CON_CONHECIMENTO.GLB_CLIENTE_CGCCPFDESTINATARIO%TYPE := NULL;
     ...
     BEGIN
        ...
        SELECT C.GLB_CLIENTE_CGCCPFDESTINATARIO,
         ...
         WHERE C.CON_CONHECIMENTO_CODIGO = P_CTRC
           ...
           V_CLIENTE_DESTINATARIOC := V_CLIENTE_DESTINATARIO;
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
...
--        V_TABELA_TIPO := 'FOB';
  ...
  ELSE
    ...
    ELSE
      ...
      V_IMPRESSO             := nvl(V_IMPRESSO,'D');
    END IF;
    -- PROCURA OS ESTADOS DE ORIGEM E DESTINO e DA FILIAL 
    -- ORIGEM   
    BEGIN
      SELECT L.GLB_ESTADO_CODIGO
        INTO V_UFORIGEM
    ...
    -- FILIAL
    BEGIN
       SELECT R.GLB_ESTADO_CODIGO
         INTO V_UFFILIAL
       FROM TDVADM.T_GLB_ROTA R
       WHERE R.GLB_ROTA_CODIGO = P_ROTA;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
         V_UFFILIAL := V_UFORIGEM;
    END;
    ...
    ElsIf V_UFORIGEM = 'ES' and 
          ...
          P_LEI := 'SUBST. TRIBUTÁRIA ART. 185, VII, RICMS/ES';
          V_TIPO := 'S'; 
          P_TIPORETORNO := 'S';  
...
--          trim(V_CLIENTE_DESTINATARIO) in ('03543379001146') and
    ...
    ElsIf V_UFORIGEM = 'MS' and 
          trim(V_CLIENTE_SACADO) in ('03543379001146') and 
          trim(V_CLIENTE_REMETENTE) in ('03543379001146') and 
          nvl(trim(V_CLIENTE_DESTINATARIOC),'03543379001146') in ('03543379001146') and 
          P_DATAEMBARQUE >= to_date('25/06/2021','dd/mm/yyyy') 
--          P_DATAEMBARQUE <= to_date('26/06/2021','dd/mm/yyyy') 
          Then
          V_TIPO := 'S'; 
          P_TIPORETORNO := 'S';  
          P_LEI := 'TRIBUTADA E COM COBRANÇA DO ICMS POR SUBSTITUIÇÃO TRIBUTÁRIA BASE MINIMA <<S_BSMIICMS>>';
...
*/
    ...
    IF ( INSTR(UPPER(NVL(P_LEI,'SIRLANO')),'.PRES.') = 0 ) and ( vCalculo = 'C' ) THEN
       P_LEI := SUBSTR('OPT.CRED.PRES.CONV.ICMS 106/96' || '-' || TRIM(P_LEI),1,200);
    END IF;    
...
/*            IF P_CTRC = '141662' THEN
            ...
            --Vinicius Dantas / Sirlano - 12/01/2022 - alteração cst Bahia
                if trunc(P_DATAEMBARQUE) >= to_date('12/01/2022','DD/MM/YYYY') then
  ...
  -- 14/07/2010 KLAYTON/SIRLANO SOMENTE FAZ A VERIFICAÇÃO PARA SOLICITAÇÃO
   ...
   -- Sirlano 03/12/2021
   -- Verifica se a emisao e em uma UF Sem FILIAL
   If V_UFORIGEM <> V_UFFILIAL Then
      P_TIPORETORNO := 'F';
   -- Vinicius Dantas / Sirlano 12/01/2022 - Alterado OBS conforme chamado 258368           
       p_lei := ' TRANSP.INI.OUTRA UF.ICMS PG ANTEC.À UF ORIGEM.CT-e EMITIDO PARA COB';    
      -- String muito grande - retirado por Vinicius
      --P_LEI := substr(trim(P_LEI) || ' TRANSP. INICIADO EM OUTRA UF. ICMS PAGO ANTEC. À UF DE ORIGEM. CT-e EMITIDO PARA COBRANÇA',1,100);
   End IF;   
  ...
  BEGIN
    ...
    --WHO_CALLED_ME2;
     ...
     -- IF V_RPS <> 'S' THEN 
        ...
        IF V_GLB_ROTA not in ('194','571','230','000','188','011','760','261','057') THEN
      ...
      BEGIN
        ...
        INSERT INTO T_CON_CONHECIMENTO
          ...
          SELECT V_NUMCONHECAUX,
                 ...
                 CON_CONHECIMENTO_DTENVSEG,
                 CON_CONHECIMENTO_DTENVEDI,
                 decode(vCalculo,'C',decode(INSTR(UPPER(CON_CONHECIMENTO_OBSLEI),'.PRES.'),0,SUBSTR( 'OPT.CRED.PRES.CONV.ICMS 106/96' || '-' || TRIM(CON_CONHECIMENTO_OBSLEI),1,200),CON_CONHECIMENTO_OBSLEI),CON_CONHECIMENTO_OBSLEI),
                 CON_CONHECIMENTO_OBSCLIENTE,
                 GLB_ROTA_CODIGORECEITA,
                 ARM_CARREGAMENTO_CODIGO,
                 ARM_CARREGAMENTO_CODIGOPR,
 ...
 End If;              
    ...
    LOOP
      ...
      IF V_VERBA_PRINCIPAL = 'S_BSMIICMS' THEN
        BEGIN
         SELECT round(CA.CON_CALCVIAGEM_VALOR,0)
           INTO V_KM
         FROM T_CON_CALCCONHECIMENTO CA
         WHERE CA.CON_CONHECIMENTO_CODIGO = vNUMERO
           AND CA.CON_CONHECIMENTO_SERIE = vSAQUE_SERIE
        ...
        BEGIN
         ...
         BEGIN
           ...
           WHERE BM.GLB_ESTADO_CODIGOORI = V_ESTADO_ORIGEM
             ...
             AND V_KM BETWEEN BM.SLF_BASEMINIMAICMS_FAIXAKMI AND BM.SLF_BASEMINIMAICMS_FAIXAKMF
       ...
       If V_CGCSACADO <> '03543379001146' Then
          v_UFIRPERCT := 1;
          v_UFIR := 1;

          UPDATE T_CON_CALCCONHECIMENTO
             SET CON_CALCVIAGEM_VALOR = round(((V_TARIFA / ((100-V_ALIQ)/100)) * V_PESO), vPrecisaoVp)
           WHERE CON_CONHECIMENTO_CODIGO = vNUMERO
             AND CON_CONHECIMENTO_SERIE = vSAQUE_SERIE
             AND GLB_ROTA_CODIGO = vROTA_CODIGO
             AND RPAD(SLF_RECCUST_CODIGO, 10) = RPAD(V_VERBA_PRINCIPAL, 10)
       ...
       Else
          Begin
              select p.usu_perfil_paran1,p.usu_perfil_paran2
                into v_UFIR,v_UFIRPERCT
          ...
          Exception
            When NO_DATA_FOUND Then
               raise_application_error(-21000,'VALOR DA UFERMS não ENCONTRADA PARA A REFERENCIA ' || to_char(sysdate,'/mm/yyyy') || chr(10) || 'CADASTRAR NA T_USU_PERFIL parametro UFERMS');
            End;
          UPDATE T_CON_CALCCONHECIMENTO
             SET CON_CALCVIAGEM_VALOR = round( ( (V_TARIFA * V_PESO) * (v_UFIR * (v_UFIRPERCT/100)) ) , vPrecisaoVp)
           WHERE CON_CONHECIMENTO_CODIGO = vNUMERO
             AND CON_CONHECIMENTO_SERIE = vSAQUE_SERIE
             AND GLB_ROTA_CODIGO = vROTA_CODIGO
             AND RPAD(SLF_RECCUST_CODIGO, 10) = RPAD(V_VERBA_PRINCIPAL, 10)
          ...
          UPDATE T_CON_CALCCONHECIMENTO
           ...
           WHERE CON_CONHECIMENTO_CODIGO = vNUMERO
             AND CON_CONHECIMENTO_SERIE = vSAQUE_SERIE
             AND GLB_ROTA_CODIGO = vROTA_CODIGO
             AND RPAD(SLF_RECCUST_CODIGO, 10) = RPAD('I_BSCLICMS', 10)
             AND (CON_CONHECIMENTO_ALTALTMAN IS NULL OR
                 CON_CONHECIMENTO_ALTALTMAN = '' OR
                 CON_CONHECIMENTO_ALTALTMAN = ' ');
