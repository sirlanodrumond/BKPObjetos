  ...
  loop
    ...
    If nvl(c_msg.Frete,0) = 0 Then
       ...
       pMessage := pMessage || '***********   VERIFIQUE   ***********' || chr(10) ||
                               ...
                               '*************************************' || chr(10) ;
--                               ||
--                               ' Sem Frete p/ Origem  - ' || trim(c_msg.origemcalcd)  || chr(10) || 
--                               '             Destino - ' || trim(c_msg.destinocalcd) || chr(10) || 
--                               '             Pas Por - ' || trim(c_msg.passandoporD) || chr(10);      
    End If;
...
-- Verifica o Pedagio
  begin
     IF trim(vPassandoPorLOC) <> '99999' Then  
         ...
         If ( vPedEtp1 + vPedEtp2) != tpFCF_MEMO.Fcf_Fretecarmemo_Pedagio Then
              vPedEtp2 := tpFCF_MEMO.Fcf_Fretecarmemo_Pedagio - vPedEtp1;
         End If;
     Else
         vPedEtp1 := tpFCF_MEMO.Fcf_Fretecarmemo_Pedagio;
         vPedEtp2 := 0;
     End If;
  exception
    When OTHERS Then
...
--     tpValeFrete.con_valefrete_adiantamento     := tpValeFrete.con_valefrete_pedagio;   
     ...
     tpValeFrete.con_conhecimento_codigo        := tdvadm.fn_busca_buracaoVF('060');
     tpValeFrete.con_conhecimento_serie         := 'A1';
     tpValeFrete.glb_rota_codigo                := '060';
