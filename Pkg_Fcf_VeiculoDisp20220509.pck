create or replace package Pkg_Fcf_VeiculoDisp is
   ...
   Function fn_get_versaoSOLVEIC(pVeicDispCodigo    In t_Fcf_Veiculodisp.Fcf_Veiculodisp_Codigo%Type,
                                 pVeicDispSequencia In t_fcf_veiculodisp.fcf_veiculodisp_sequencia%Type) return char;
   
   Function fn_get_versaoMESA(pVeicDispCodigo    In t_Fcf_Veiculodisp.Fcf_Veiculodisp_Codigo%Type,
                              pVeicDispSequencia In t_fcf_veiculodisp.fcf_veiculodisp_sequencia%Type) return char;

   Function fn_get_versaoSOLVEIC(pSolveic     in tdvadm.t_fcf_solveic.fcf_solveic_cod%type) return char;
   
   Function fn_get_versaoMESA(pSolveic     in tdvadm.t_fcf_solveic.fcf_solveic_cod%type) return char;
...
--  function fn_valorentrega(pSolveic tdvadm.t_fcf_solveic.fcf_solveic_cod%type) return number; 

  -- Sirlano
  -- 11/06/2021
  -- usanda para retornar o Valor fechado do Frete para a nova mesa e a mesa velha
  function fn_valorFrete(pSolveic tdvadm.t_fcf_solveic.fcf_solveic_cod%type,
                         pValorFrete tdvadm.t_fcf_solveic.fcf_solveic_valorfrete%Type) return number ;
...
create or replace package body Pkg_Fcf_VeiculoDisp is
   ...
   Begin
      ...
      where sv.fcf_veiculodisp_codigo = pVeicDispCodigo
        and sv.fcf_veiculodisp_sequencia = pVeicDispSequencia
        and substr(sv.fcf_solveic_obsacrescimo,1,2) = 'SN';
      If vContador > 0 Then
         Return 'N';
      Else
         Return 'V';
   ...
   Begin
      ...
      where sv.fcf_veiculodisp_codigo = pVeicDispCodigo
        and sv.fcf_veiculodisp_sequencia = pVeicDispSequencia
        and substr(sv.fcf_solveic_obsvalefrete,1,2) = 'MN';
      If vContador > 0 Then
         Return 'N';
      Else
         Return 'V';
   ...
   Begin
      select count(*) 
        into vContador    
      from tdvadm.t_fcf_solveic sv
      where sv.fcf_solveic_cod = pSolveic
        and substr(sv.fcf_solveic_obsacrescimo,1,2) = 'SN';
      If vContador > 0 Then
         Return 'N';
      Else
  ...
  --          tpSolVeic.Fcf_Solveic_Dtcontr
     ...
     dbms_output.put_line('Dentro da Proc Origem ' || vOrigem || '- Destino ' || vDestino || '- Part ' || vPart);
           ...
           */
           ...
           tpFretecarMemo.Fcf_Solveic_Cod                   := tpSolVeic.Fcf_Solveic_Cod;
           -- Thiago - 28/05/2021 - Separação de qual processo grava na MEMO  
           tpFretecarMemo.Fcf_Tpveiculo_Codigo              := tpSolVeic.Fcf_Tpveiculo_Codigo;
           -- Fim Thiago
           ...
           If length(trim(vPart)) >= 1 Then
              ...
              loop
                ...
                -- Thiago - 28/05/2021 - Separação de qual processo grava na MEMO  
                update tdvadm.t_fcf_solveic s
                set s.fcf_solveic_valorparticilaridade = tpSolVeicPart.Fcf_Solveicpart_Valor
                where s.fcf_solveic_cod = tpSolVeic.Fcf_Solveic_Cod;
                -- Fim Thiago
   ...
   Begin     
      ...
      Begin
          ...
          --alterado no dia 28/04/2022 - o expresso esta sendo calculado erroneamente no delphy calculando
          -- frete + pedagio onde deveria ser so frete fazendo alteração temporaria. Vinicius Dantas / Sirlano
          
          if tpFreteCarMemo.Fcf_Fretecarmemo_Expresso <> 0 and tpVeicDisp.Fcf_Solveic_Tpfrete = 'E' then
             tpFreteCarMemo.Fcf_Fretecarmemo_Expresso := (tpFreteCarMemo.Fcf_Fretecarmemo_Frete * (tpFreteCarMemo.Fcf_Fretecarmemo_Percentexpr / 100));
          end if;
          
   -- Pego o Codigo do Veiculo e Data da Solicitação
   -- Neste Vinculo não vem o Veiculo e a Data, pois foram gravados na tela da SOLICITACAO
   
   select m.Fcf_Fretecarmemo_Dtsolveic,
          m.Fcf_Tpveiculo_Codigo
     into tpFreteCarMemo.Fcf_Fretecarmemo_Dtsolveic,
          tpFreteCarMemo.Fcf_Tpveiculo_Codigo
   from tdvadm.t_fcf_fretecarmemo m
   WHERE m.FCF_SOLVEIC_COD = tpVeicDisp.Fcf_Solveic_Cod;       
          
   Begin
      select nvl(p.usu_perfil_paran3,0),
             nvl(p.usu_perfil_paran4,0)
        into vValorporColeta,
             vValorporEntrega
      from tdvadm.t_usu_perfil p
      where p.usu_aplicacao_codigo = 'veicdisp'        
        and p.usu_perfil_ativo = 'S'
        and p.usu_perfil_codigo like trim('REGRAFRETE' || trim(tpFreteCarMemo.Fcf_Tpveiculo_Codigo))
        and p.usu_perfil_vigencia = (select max(p1.usu_perfil_vigencia)
                                     from tdvadm.t_usu_perfil p1    
                                     where p1.usu_aplicacao_codigo = p.usu_aplicacao_codigo
                                       and p1.usu_perfil_codigo = p.usu_perfil_codigo
                                       and p1.usu_perfil_vigencia <= tpFreteCarMemo.Fcf_Fretecarmemo_Dtsolveic);
        vValorporEntrega := nvl(vValorporEntrega,0);
        vValorporColeta  := nvl(vValorporColeta,0);
  exception
      When NO_DATA_FOUND Then
...
--          tpFreteCarMemo.Fcf_Fretecarmemo_Coleta          := TDVADM.fn_querystring(TDVADM.fn_querystring(pQuery,'VDCOLETA','=','*'), 'valor', '=', '|');
          -- Não estava calculando 
          --tpFretecarMemo.Fcf_Fretecarmemo_Entrega         := vValorporEntrega * tpFretecarMemo.Fcf_Fretecarmemo_Qtdeentrega;
          -- Thiago - 15/06/2021 - Para calcular valor entrega 
          tpFretecarMemo.Fcf_Fretecarmemo_Entrega         := vValorporEntrega * tpVeicDisp.Fcf_Solveic_Qtdeentregas;
          tpFretecarMemo.Fcf_Fretecarmemo_Coleta          := vValorporColeta * tpFretecarMemo.Fcf_Fretecarmemo_Qtdecoleta;
...
/* SIRLANO
  ...
  begin 
       select m.Fcf_Fretecarmemo_Dtsolveic,
              m.Fcf_Tpveiculo_Codigo,
              m.fcf_fretecarmemo_qtdeentrega
         into tpFreteCarMemo.Fcf_Fretecarmemo_Dtsolveic,
              tpFreteCarMemo.Fcf_Tpveiculo_Codigo,
              tpFretecarMemo.Fcf_Fretecarmemo_Qtdeentrega
       from tdvadm.t_fcf_fretecarmemo m
       WHERE m.FCF_SOLVEIC_COD = pSolveic;       
              
       Begin
          select nvl(p.usu_perfil_paran4,0)
            into vValorporEntrega
          from tdvadm.t_usu_perfil p
          where p.usu_aplicacao_codigo = 'veicdisp'        
            and p.usu_perfil_ativo = 'S'
            and p.usu_perfil_codigo like trim('REGRAFRETE' || trim(tpFreteCarMemo.Fcf_Tpveiculo_Codigo))
            ...
            vValorporEntrega := vValorporEntrega * tpFretecarMemo.Fcf_Fretecarmemo_Qtdeentrega;
      exception
          When NO_DATA_FOUND Then
            vValorporEntrega := 0;
      End;
...
*/
  ...
  function fn_valorFrete(pSolveic tdvadm.t_fcf_solveic.fcf_solveic_cod%type,
                         pValorFrete tdvadm.t_fcf_solveic.fcf_solveic_valorfrete%Type) return number 
    as
    vSolNovo  char(1) := fn_get_versaoSOLVEIC(pSolveic);
    vMesaNova char(1) := fn_get_versaoMESA(pSolveic);
    vValorFrete tdvadm.t_fcf_solveic.fcf_solveic_valorfrete%type;
    tpFreteMemo tdvadm.t_fcf_fretecarmemo%rowtype;
  Begin
     If vSolNovo = 'N' and vMesaNova = 'N' Then
        select * 
          into tpFreteMemo
        from tdvadm.t_fcf_fretecarmemo m
        where m.fcf_solveic_cod = pSolveic;
        vValorFrete := nvl(tpFreteMemo.Fcf_Fretecarmemo_Frete,0) +
                       nvl(tpFreteMemo.Fcf_Fretecarmemo_Entrega,0) +
                       nvl(tpFreteMemo.Fcf_Fretecarmemo_Coleta,0) +
                       nvl(tpFreteMemo.Fcf_Fretecarmemo_Excecao,0) +
                       nvl(tpFreteMemo.Fcf_Fretecarmemo_Acrescimo,0) +
                       -- Thiago 15/06/2021 - Incluida a particularidade
                       nvl(tpFreteMemo.Fcf_Fretecarmemo_Particularidade,0) +
                       nvl(tpFreteMemo.Fcf_Fretecarmemo_Expresso,0);

        If tpFreteMemo.Fcf_Fretecarmemo_Pedagionofrete = 'N' Then
           vValorFrete := vValorFrete + tpFreteMemo.Fcf_Fretecarmemo_Pedagio;
        End If; 
     Else
         vValorFrete := pValorFrete;
     End If;    
