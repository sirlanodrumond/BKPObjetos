...
create or replace package body pkg_arm_NotaPesagem is
  ...
  Begin
     ...
     Else
       ...
       -- 21/11/2021
       -- Sirlano
       -- Validar se foi enviado para o SAP
       select count(*)
         into vCount
       from tdvadm.t_arm_notapesagem_sap np
       where np.arm_nota_numero = pNotaNumero
         and np.glb_cliente_cgccpfremetente = pNotaCGCCPFRem
         and NVL(np.arm_nota_chavenfe,'x') = nvl(pChaveNFE,'x')  
         and np.arm_notapesagem_dtenvia is not null;
       If vCount > 0 then 
          pStatus := 'E';
          pMessage := pMessage || CHR(13) ||' - Nota Ja enviada para o SAP.';
       End if;  
     End If;
            ...
            Else          
                ...
                if vNotaPesagem.Arm_Notapesagem_Origem_Nota = 'SAP' then
                   ...
                   values (vNotaPesagem.arm_nota_numero,
                           ...
                           -- 22/11/2021
                           -- Sirlano
                           -- Coloquei o NVL quando for ambiente SAP
                           nvl(vNotaPesagem.arm_nota_serie,'000'),
...
--                       replace(tdvadm.fn_querystring(en.PAYLOAD_ENVIO,'"RealWeight"',':',','),'"','')
       ...
       Else
          select sum(pi.arm_notapesagemitem_peso)
            into vPesoAtual
          from tdvadm.t_arm_notapesagemitem_sap pi
          where pi.arm_nota_numero = vNotaPesagemItem.Arm_Notapessagem_Numero
            and pi.glb_cliente_cgccpfremetente = vNotaPesagemItem.Glb_Cliente_Cgccpfremetente;
            
          -- 21/11/2021
          -- Verificando se ja foi enviado para a Vale
          vClienteInformado := null; 
          Begin
              select np.arm_notapesagem_dtenvia
                 into vClienteInformado
              from tdvadm.t_arm_notapesagem_sap np
              where np.arm_nota_numero = vNotaPesagemItem.Arm_Notapessagem_Numero
                ...
                and NVL(np.arm_nota_chavenfe,'x') = nvl(vNotaPesagemItem.Arm_Nota_Chavenfe,'x')  
                and np.arm_notapesagem_dtenvia is not null;
          Exception
            When OTHERS Then
               vClienteInformado := null;
          ...
          If vClienteInformado is not null then 
             pStatus := 'E';
             pMessage := pMessage || CHR(13) ||' - Nota Ja enviada em '|| to_char(vClienteInformado) || ' para o SAP.';
          End if;  
       End If;
        ...
        If vClienteInformado Is not null Then
          ...
          If cAmbiente = 'TDV' Then
             If to_number(vPesoInf) > 0 Then
                pMessage := 'Cliente Informado em ' || to_char(vClienteInformado,'dd/mm/yyyy hh24:mi:ss') || ' Nota não pode ser mais pesada';          
             Else   
                pMessage := 'Coleta Fechada em ' || to_char(vClienteInformado,'dd/mm/yyyy hh24:mi:ss') || ' Nota não pode ser mais pesada';          
             End If;
              ...
              End If; 
          Else
             pMessage := 'SAP já Informado em ' || to_char(vClienteInformado,'dd/mm/yyyy hh24:mi:ss') || ' Nota não pode ser mais pesada';          
          End If;
          Return;
        End If;
       ...
       if (trim(vNotaPesagemItem.Usu_Usuario_Codigopesou) <> 'sorter') then
          ...
          Else
             ...
             WHERE I.arm_nota_numero  =  vNotaPesagemItem.Arm_Notapessagem_Numero        
              ...
              AND nvl(I.arm_nota_chavenfe ,'x') = nvl(vNotaPesagemItem.Arm_Nota_Chavenfe,'x');  
          End If;                        
           ...
           If cAmbiente = 'TDV' Then
              ...
              Insert Into tdvadm.t_Arm_Notapesagemitem_Sap values 
               ( vNotaPesagemItem.Arm_Notapessagem_Numero,
                 vNotaPesagemItem.glb_cliente_cgccpfremetente,
                 0,
                 vNotaPesagemItem.arm_notapesagemitem_seq,
                 vNotaPesagemItem.arm_notapesagemitem_dtinclusao,
                 vNotaPesagemItem.arm_notapesagemitem_peso,
                 vNotaPesagemItem.usu_usuario_codigopesou,
                 vNotaPesagemItem.arm_notapesagemitem_retorno_sap,
                 vNotaPesagemItem.arm_notapesagemitem_envio_sap,
                 vNotaPesagemItem.arm_nota_sequencia,
                 vNotaPesagemItem.arm_nota_chavenfe,
                 vNotaPesagemItem.arm_notapesagemitem_altura,
                 vNotaPesagemItem.arm_notapesagemitem_largura,
                 vNotaPesagemItem.arm_notapesagemitem_comprimento);
           ...
           if (vExistePesItem = 0) then
            ...
            -- Sp_AtualizaPeso_T_Arm_Nota(vNotaPesagemItem);
             Else
                Insert Into t_Arm_Notapesagemitem_sap 
                values 
                  (vNotaPesagemItem.Arm_Notapessagem_Numero,
                   vNotaPesagemItem.GLB_CLIENTE_CGCCPFREMETENTE,
                   0,
      ...
      End;  
      If cAmbiente = 'SAP' Then
         update tdvadm.t_Arm_Notapesagem_Sap s
            set (s.arm_notapesagem_qtdvolume,
                 s.arm_notapesagem_pesototal ) = (select count(*),sum(si.arm_notapesagemitem_peso) 
                                                  from tdvadm.t_Arm_Notapesagemitem_sap si
                                                  where si.arm_nota_numero = s.arm_nota_numero 
                                                    and si.glb_cliente_cgccpfremetente = s.glb_cliente_cgccpfremetente 
                                                    and NVL(si.arm_nota_chavenfe,'x') = nvl(s.arm_nota_chavenfe ,'x'))  
         where s.arm_nota_numero = vNotaPesagemItem.Arm_Notapessagem_Numero
           and s.glb_cliente_cgccpfremetente = vNotaPesagemItem.Glb_Cliente_Cgccpfremetente
           and NVL(s.arm_nota_chavenfe,'x') = nvl(vNotaPesagemItem.Arm_Nota_Chavenfe,'x') ; 
         commit;
      End If;           
...
begin
   Begin
      select upper(substr(p.usu_perfil_parat,1,3))
        into cAmbiente
