...
*/
  ...
  procedure  SP_GLB_RETORNAPCLDRU(P_XMLIN    In VARCHAR2,
  ...
  Function FN_RETORNAFRASEPEDAGIO(pConhecimento           in TDVADM.T_CON_CALCVALEFRETE.CON_CONHECIMENTO_CODIGO%type,
                                  pRota                   in TDVADM.T_CON_CALCVALEFRETE.GLB_ROTA_CODIGO%type,
                                  pSerie                  in TDVADM.T_CON_CALCVALEFRETE.CON_CONHECIMENTO_SERIE%type,
                                  pSaque                  in TDVADM.T_CON_CALCVALEFRETE.CON_VALEFRETE_SAQUE%type,
                                  pValeFrete_Cartao       in TDVADM.T_CON_CALCVALEFRETE.CON_CALCVALEFRETE_CARTAO%type,
                                  pValeFrete_Tipo         in TDVADM.T_CON_CALCVALEFRETE.CON_CALCVALEFRETE_TIPO%type,
                                  pCon_Valefrete_Pedagio  in TDVADM.T_CON_VALEFRETE.CON_VALEFRETE_PEDAGIO%type)
          return varchar2;                                 
     ...
     i:= 1; -- Quantidade de Paginas.
       ...
       select co.arm_armazem_codigo,
        ...
        --Caso tenha, alimento as variáveis com dados do parceiro.  
          ...
          P_LISTADADOS(i).linha('CODIGO3')      := vEndCol;      
          P_LISTADADOS(i).linha('ENDERECO3')    := vCidadeCol;   
          P_LISTADADOS(i).linha('BAIRRO3')      := vIECol;       
          P_LISTADADOS(i).linha('MUNICIPIO3')   := vCodCol;      
          P_LISTADADOS(i).linha('UF3')          := vBairroCol;   
          P_LISTADADOS(i).linha('CEP3')         := vUFCol;       
          P_LISTADADOS(i).linha('INSCEST3')     := vCEPCol;      
...
*/
 ...
 -- Declaro todas as variáveis que serão utilizadas.
  ...
  As
     ...
     vValeFrete_Cartao   TDVADM.T_CON_CALCVALEFRETE.CON_CALCVALEFRETE_CARTAO%type;    
     vValeFrete_Tipo     TDVADM.T_CON_CALCVALEFRETE.CON_CALCVALEFRETE_TIPO%type;  
  Begin
   vFrota              := 'N';
   vCodigo             := trim(Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam(P_XMLIN,'Codigo' ));
   vSerie              := trim(Glbadm.pkg_glb_WsInterfaceDB.Fn_GetParam(P_XMLIN,'Serie' ));
       ...
       loop
            ...
            for vPropVeicMot in (SELECT P.CAR_PROPRIETARIO_CGCCPFCODIGO CNPJPROP,
                      ...
                      loop
                          ...
                          -- Thiago - 30/09/2019 - 
                          -- Select para preencher parâmetros para 
                          -- montar lógica da frase do pedágio
                          SELECT F.CON_CALCVALEFRETE_CARTAO,
                                 F.CON_CALCVALEFRETE_TIPO
                            into vValeFrete_Cartao,
                                 vValeFrete_Tipo
                          FROM TDVADM.T_CON_CALCVALEFRETE F
                          WHERE F.CON_CONHECIMENTO_CODIGO    = vCodigo
                            AND F.GLB_ROTA_CODIGO            = vRota
                            AND F.CON_CONHECIMENTO_SERIE     = vSerie
                            AND F.CON_VALEFRETE_SAQUE        = vSaque
                            AND F.CON_CALCVALEFRETETP_CODIGO = '02';
                          ...
                          -- Thiago - 30/09/2019 - Chamada da função para montar frase do pedágio no quadro de Verbas
                          P_LISTADADOS(i).linha('OBSANTT3') := FN_RETORNAFRASEPEDAGIO( vCodigo,
                                                                                       vRota,
                                                                                       vSerie,
                                                                                       vSaque,
                                                                                       vValeFrete_Cartao,
                                                                                       vValeFrete_Tipo,                                                                                      
                                                                                       nvl(tpLinhaValeFrete.Con_Valefrete_Pedagio,0)
                                                                                     ); 
                          
                          P_LISTADADOS(i).linha('OBSANTT4') := '';
...
ElsIF P_XMLIN = 'GERCOLETA' Then
   ...
   vXmlin := vXmlin || '      <Coleta>015938</Coleta> ';
          ...
          loop
              ...
              for C_LINHA in (select pcl.glb_pcllinha_campo,
                                     ...
                                     (select f.pcl_fontes_comando || chr(27) || '(s' || f.pcl_fontes_codigo || 
                                                                                        ...
                                                                                        decode(nvl(ta.pcl_fontestam_codigo,  'X'),'X','','s' || trim(to_char(to_number(ta.pcl_fontestam_codigo),'90.00'))) || 
...
--                            End If;                            
 ...
 --   insert into dropme(a,l) values('FATURA', P_ARQOUT); commit;
  ...
  -- Thiago - 30/09/2019 - Função para montar frase do pedágio no quadro de Verbas
  Function FN_RETORNAFRASEPEDAGIO(pConhecimento           in TDVADM.T_CON_CALCVALEFRETE.CON_CONHECIMENTO_CODIGO%type,
                                  pRota                   in TDVADM.T_CON_CALCVALEFRETE.GLB_ROTA_CODIGO%type,
                                  pSerie                  in TDVADM.T_CON_CALCVALEFRETE.CON_CONHECIMENTO_SERIE%type,
                                  pSaque                  in TDVADM.T_CON_CALCVALEFRETE.CON_VALEFRETE_SAQUE%type,
  ...
  Begin
     ...
     /*pValefrete_Tipo pode ser:
                       (C) Cartao
                       (O) Conta
                       (H) Cheque
                       (D) Dinheiro
     */
     If (pValeFrete_Tipo = 'C') Then
       
          --If pCon_Valefrete_Pedagio > 0 Then
            
          --End If;          
       
       return 'Frase Vale Frete';
     End If;
