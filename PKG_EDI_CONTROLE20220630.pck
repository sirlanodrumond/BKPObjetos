CREATE OR REPLACE PACKAGE PKG_EDI_CONTROLE IS
/***************************************************************************************************
 * ROTINA           :
 * PROGRAMA         :
 * ANALISTA         :
...
-- MSG=PROCESSASAP;ACAO=BAIXA;PROTOCOLO=764535;LIMITE=31/01/2015
   ...
   Procedure sp_altera_pagamentoparacheque(tpRecebido In Out rmadm.t_glb_benasseREC%Rowtype,
     ...
     Procedure sp_arm_finalizarplacamobile(tpRecebido In Out rmadm.t_glb_benasseREC%Rowtype,
                                          vStatus Out Char,
                                          vMessage Out Varchar2);
  ...
  procedure sp_ativatab(pTpRecebido in out rmadm.t_glb_benasserec%rowtype,
                              pStatus Out Char,
                              pMessage out varchar2); 

 procedure sp_susptabela(pTpRecebido in out rmadm.t_glb_benasserec%rowtype,
                              pStatus Out Char,
                              pMessage out varchar2);
  ...
  Procedure SPi_INC_ACELORMITTAL(pTpRecebido in out rmadm.t_glb_benasserec%rowtype,
   ...
   procedure sp_uti_rodou_aute(pTipo in char);                                
    ...
    Procedure SP_BI_GLB_BENASSERECARQCSV;
...
/
CREATE OR REPLACE PACKAGE BODY PKG_EDI_CONTROLE AS
...
*/
   ...
   Begin
            ...
            -- 22/03/2021
            --- Sirlano e Malcar
            -- Colocamos para verificar a tabela de autorização 
            vAutorizado := 0;
            select count(*)
              into vAutorizado
            from tdvadm.t_edi_planilhaaut pa
            where pa.edi_planilhacfg_sistema = 'EXPREVIAFAT'
              and pa.edi_planilhaaut_ativo = 'S'
              and pa.edi_planilhaaut_autoriza = 'S'
              and pa.edi_planilhaaut_vigencia <= sysdate
              and pa.edi_planilhaaut_validate > sysdate
...
Procedure sp_arm_finalizarplacamobile(tpRecebido In Out rmadm.t_glb_benasseREC%Rowtype,
  ...
  ---------------------------------------------------------------------------
  ------------- CREATE BY VINICIUS DANTAS 08/07/2021 ----------------
  ---------------------------------------------------------------------------   
  Begin           
     --Populo as variáveis, para recuperar os paramentros passados.
     vQryString := tpRecebido.GLB_BENASSEREC_ASSUNTO;
     ...
     select count(*)
       into vContador
       from tdvadm.t_edi_planilhaaut p
      where p.edi_planilhacfg_codigo = rpad(UPPER(Trim(tdvadm.fn_querystring(upper(vQryString),'MSG','=',';'))),12)
        and p.edi_planilhaaut_email = tpRecebido.glb_benasserec_origem;  
     ...
     if ( vContador > 0) Then
        ...
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
           ...
           vStatus := 'E';
          When OTHERS Then  
           vMessage := 'Houve Algum problema na Rotina, Favor entrar em contato Com o Vinicius Dantas' ||
                       chr(10) || 'Placa: ' || vPlaca || chr(10);
           tpRecebido.Glb_Benasserec_Processado := Sysdate;
           tpRecebido.Glb_Benasserec_Status     := 'ER';
           vStatus := 'E';
     ...
     -- CASO O EMAIL NÃO TENHA LIBERAÇÃO
     Else 
        vMessage := 'EMAIL NÃO AUTORIZADO PARA FINALIZAR PLACA';
        tpRecebido.Glb_Benasserec_Processado := Sysdate;
        tpRecebido.Glb_Benasserec_Status     := 'ER';        
        vStatus := 'E';
     end if;
     ...
     IF(vStatus = 'N') THEN
       select count(*)
        ...
        IF(vContador > 0 or vFlagVF = 'S') THEN
          vMessage := 'Já Tem uma Vale de Frete Criado! Impossível Finalizar!';
          tpRecebido.Glb_Benasserec_Processado := Sysdate;
          tpRecebido.Glb_Benasserec_Status     := 'ER';  
          vStatus := 'E';
        -- FINALIZANDO PLACA
        ELSE
          IF (vEstagio <> 'F') THEN
            UPDATE TDVADM.T_FCF_VEICULODISP VD
               SET VD.ARM_ESTAGIOCARREGMOB_CODIGO = 'F'
             WHERE VD.FCF_VEICULODISP_CODIGO    = vVeicDisp
               AND VD.FCF_VEICULODISP_SEQUENCIA = vSeq
               AND NVL(VD.ARM_ESTAGIOCARREGMOB_CODIGO,'F') <> 'F'
               AND Vd.ARM_ESTAGIOCARREGMOB_CODIGO <> 'D';
           
            UPDATE TDVADM.T_ARM_CARREGAMENTOVEIC CA
               SET CA.ARM_CARREGAMENTOVEIC_FINALIZ = 'S'
             WHERE CA.FCF_VEICULODISP_CODIGO    = vVeicDisp
               AND CA.FCF_VEICULODISP_SEQUENCIA = vSeq
               AND CA.ARM_CARREGAMENTOVEIC_FINALIZ = 'N';    
            ...
            tpRecebido.Glb_Benasserec_Status     := 'OK';
          ELSE 
            vMessage := 'PLACA ESTAVA FINALIZADA';
            tpRecebido.Glb_Benasserec_Processado := Sysdate;
            tpRecebido.Glb_Benasserec_Status     := 'ER';        
            vStatus := 'E';
          END IF;
     ...
     IF (vStatus = 'E') THEN
       wservice.pkg_glb_email.SP_ENVIAEMAIL('ERRO AO FINALIZAR PLACA',
                                            vMessage,
                                            'tdv.producao@dellavolpe.com.br',
                                            trim(tpRecebido.glb_benasserec_origem));
     ELSE 
       wservice.pkg_glb_email.SP_ENVIAEMAIL('SUCESSO AO FINALIZAR PLACA',
                                            vMessage,
                                            'tdv.producao@dellavolpe.com.br',
                                            trim(tpRecebido.glb_benasserec_origem));  
     END IF;
     
  End sp_arm_finalizarplacamobile;
  
   /********************************************************************************************** 
    * ROTINA           : sp_glb_clientecontrato                                                  *
    * PROGRAMA         : SCH                                                                     *
    * ANALISTA         : Sirlano                                                                 *
    * DESENVOLVEDOR    : Vinicius Dantas                                                         *
    * DATA DE CRIACAO  : 18/08/2021                                                              *
    * BANCO            : ORACLE-TDP                                                              *
    * EXECUTADO POR    :                                                                         *
    * FUNCINALIDADE    : Vincular Cliente ao Contrato por via Aut-e                              *
    * PARTICULARIDADES :                                                                         *
    **********************************************************************************************/
procedure sp_glb_clientecontrato(tpRecebido In Out rmadm.t_glb_benasseREC%Rowtype,
                                                   vStatus Out Char,
                                                   vMensagem Out Varchar2)

  As                                    
    vQryString    varchar2(500);
    vCliente      tdvadm.t_glb_clientecontrato.glb_cliente_cgccpfcodigo%type;
    vContrato     tdvadm.t_glb_clientecontrato.slf_contrato_codigo%type;
    vAuxiliar     integer;
   
     
  Begin 
    --MSG=CLICONT;CLIENTE=07170938001847;CONTRATO=C2015080004;
    vStatus    := 'N';
    vMensagem := '';
    --pega o cliente e o contrato
    vQryString  := tpRecebido.GLB_BENASSEREC_ASSUNTO; 
    vCliente    :=  tdvadm.fn_querystring(tpRecebido.Glb_Benasserec_Assunto, 'CLIENTE','=',';'); 
    vContrato   :=  tdvadm.fn_querystring(tpRecebido.Glb_Benasserec_Assunto, 'CONTRATO','=',';');   
    
    --verifico se o usuario tem permissão
    select count(*)
      into vAuxiliar
      from tdvadm.t_edi_planilhaaut p
     where p.edi_planilhacfg_codigo = rpad(UPPER(Trim(tdvadm.fn_querystring(upper(vQryString),'MSG','=',';'))),12)
     ...
     if (vAuxiliar <> 0 ) then
      -- verifico se o cliente e o cnpj existe
       begin
         select cl.glb_cliente_cgccpfcodigo
           into vCliente
           from tdvadm.t_glb_cliente cl
          where cl.glb_cliente_cgccpfcodigo = vCliente;
       exception 
         when NO_DATA_FOUND then
           vCliente := null;
       end; 
       begin
       select co.slf_contrato_codigo
         into vContrato
         from tdvadm.t_slf_contrato co
        where co.slf_contrato_codigo = vContrato;
       exception 
         when NO_DATA_FOUND then
           vContrato := null;
       end;
       --validando variaveis
       if nvl(vCliente,'N') = 'N' then
        vStatus   := 'E';
        vMensagem := 'Cliente não encontrado';
        tpRecebido.Glb_Benasserec_Processado := Sysdate;
        tpRecebido.Glb_Benasserec_Status     := 'ER';   
       end if;
       
       if nvl(vContrato,'N') = 'N' then
         vStatus   := 'E';
         vMensagem := 'Contrato não encontrado';  
         tpRecebido.Glb_Benasserec_Processado := Sysdate;
         tpRecebido.Glb_Benasserec_Status     := 'ER';  
       end if;
       ...
       vStatus   := 'E';
       vMensagem := 'Email nao tem autorização para vincular cliente no contrato';
        tpRecebido.Glb_Benasserec_Processado := Sysdate;
        tpRecebido.Glb_Benasserec_Status     := 'ER';  
     end if;  
     
     IF (vStatus = 'E') THEN
       wservice.pkg_glb_email.SP_ENVIAEMAIL('ERRO AO CADASTRAR CLIENTE NO CONTRATO',
                                            vMensagem,
                                            'tdv.producao@dellavolpe.com.br',
                                            trim(tpRecebido.glb_benasserec_origem));
     ELSE 
       wservice.pkg_glb_email.SP_ENVIAEMAIL('SUCESSO AO CADASTRAR CLIENTE NO CONTRATO',
                                            vMensagem,
                                            'tdv.producao@dellavolpe.com.br',
                                            trim(tpRecebido.glb_benasserec_origem));   
     END IF; 
     
End sp_glb_clientecontrato; 
...
*/
 ...
 vAcao         varchar2(50);
  ...
  vAcao := tdvadm.fn_querystring(TpRecebido.Glb_Benasserec_Assunto, 'ACAO', '=', ';');
  --vReferencia := tdvadm.fn_querystring(TpRecebido.Glb_Benasserec_Assunto, 'REFERENCIA', '=', ';');
  vAcao := nvl(vAcao,'ATUALIZA'); 
  If vAcao not in ('ATUALIZA','CARGATOTAL') Then 
     pStatus := 'E';
     pMessage := 'Ação [' || vAcao || '] Invalida, Valores Validos ATUALIZA ou CARGATOTAL';
  End If;
  ...
  if (vContador > 0) then
     ...
     delete tdvipf.t_ipf_sparar;
     commit;
     
     If ( vAcao = 'CARGATOTAL' ) Then
        delete tdvipf.t_ipf_sparar x
        where x.ipf_sparar_referencia = vReferencia;
        vContador := 0;
     End If;
     ...
     for c_msg in  (select it.edi_integra_protocolo,
                           ...
                           it.edi_integra_col11 referencia,
                           it.rowid
     ...
     loop
      ...
      --Verifico se já existe o registro na tabela do sem parar do IPF
        ...
        where sp.ipf_sparar_placa = c_msg.placa
          ...
          and sp.ipf_sparar_referencia = c_msg.referencia;
        ...
        values(c_msg.placa,
               ...
               c_msg.valor,
               c_msg.referencia,
               tdvadm.fn_mudaplaca(c_msg.placa,'MS'),
               pMessage);
        commit; 
       ...
       --Atualizo a benasserec conforme o processo executado.
        ...
        update tdvadm.t_edi_integra i
           set i.edi_integra_critica = pMessage
  ...
  select x.ipf_sparar_placa,
         x.ipf_sparar_tag,
         x.ipf_sparar_prefixo,
         x.ipf_sparar_marca,
         x.ipf_sparar_categ,
         x.ipf_sparar_data,
         x.ipf_sparar_hora,
         x.ipf_sparar_rodovia,
         x.ipf_sparar_praca,
         x.ipf_sparar_valor,
         x.ipf_sparar_referencia,
         x.ipf_sparar_placaunificada,
         x.ipf_sparar_obs
  from tdvipf.t_ipf_sparartmp x;
  commit;
  
  begin
  
  select distinct a.ipf_sparar_referencia
    into vReferencia
    from tdvipf.t_ipf_sparar a;
    
  exception when others then
    
    pMessage := 'Planilha com mais de uma referência!!!' ||chr(10);
    
  end;

  tdvipf.pkg_reccust_diretos2019.sp_carregaSemParar(vReferencia);
  
     pMessage := pMessage || ' Registros inseridos com sucesso!!!' ||chr(10);
     
  exception when others then
    
     pMessage := 'ERRO ORACLE - ' || SQLERRM;
...
procedure sp_bloqueia_vfcaixa(pTpRecebido in out rmadm.t_glb_benasserec%rowtype,
                              pStatus Out Char,
...
* PROGRAMA    : Movimento WEB - VF | Caixa
* ANALISTA    : Rafael Aiti
* PROGRAMADOR : Rafael Aiti
* CRIACAO     : 28/04/2020
* BANCO       : ORACLE
* SIGLAS      : ARM, CON
* DESCRICAO   : Bloqueia parcelas de VF impedindo o lançamento no caixa.
* OBSERVACAO  : O bloqueio está na triger tg_bloq_pagvfeletronico.
---------------------------------------------------------------------------------------------------
*/
...
begin
  ...
  vValeFrete := tdvadm.fn_querystring(pTpRecebido.Glb_Benasserec_Assunto, 'VF','=',';');  
  vVFSerie   := tdvadm.fn_querystring(pTpRecebido.Glb_Benasserec_Assunto, 'SERIEVF','=',';');
  vVFRota    := tdvadm.fn_querystring(pTpRecebido.Glb_Benasserec_Assunto, 'ROTAVF', '=', ';');
  vVFSaque   := tdvadm.fn_querystring(pTpRecebido.Glb_Benasserec_Assunto, 'SAQUEVF','=',';');   
  vTipo      := tdvadm.fn_querystring(pTpRecebido.Glb_Benasserec_Assunto, 'TIPO','=',';');                         
  
  vTipo := trim (vTipo);
  --MSG=;VF=067443;SERIEVF=A1;ROTAVF=620;SAQUEVF=1;TIPO=AMBOS;
  
  
  vValeFrete := trim(vValeFrete);
  vVFSerie   := trim(vVFSerie); 
  vVFRota    := trim(vVFRota);
  vVFSaque   := trim(vVFSaque);
  vTipo      := trim(vTipo);
  
  -- Alimento váriavel para ver se usuário tem permissão.
   select count(*)
       into vContador
       from tdvadm.t_edi_planilhaaut p
      where p.edi_planilhacfg_codigo = 'BLOQVFCAIXA'
        and p.edi_planilhaaut_email = pTpRecebido.glb_benasserec_origem;
    
   -- Caso tenha permissão, incio as próximas validações    
    if (vContador > 0) Then
      
      select count (*)
        into vExisteVF
        from tdvadm.t_con_calcvalefrete ca
       where ca.con_conhecimento_codigo = vValeFrete
         and ca.con_conhecimento_serie  = vVFSerie
         and ca.glb_rota_codigo         = vVFRota
         and ca.con_valefrete_saque     = vVFSaque;
         
        --Se o VF existir, prossigo. 
         if vExisteVF > 0 then
           ...
           --Bloqueia adiantamento.
             ...
             update tdvadm.t_con_calcvalefrete ca
                set ca.usu_usuario_bloqueou         = 'sistema',
                    ca.con_calcvalefrete_dtbloqueio = sysdate,
                    ca.glb_rota_codigobloqueou      = '010'
              where ca.con_conhecimento_codigo    = vValeFrete
                and ca.con_conhecimento_serie     = vVFSerie
                and ca.glb_rota_codigo            = vVFRota
                and ca.con_valefrete_saque        = vVFSaque
                ...
                pMessage := 'Adiantamento do VF bloqueado com sucesso';
                             
           elsif vTipo = 'SALDO' then
           ...
           --Bloqueia Saldo.
              ...
              update tdvadm.t_con_calcvalefrete ca
               ...
               where ca.con_conhecimento_codigo    = vValeFrete
                 ...
                 pMessage := 'Saldo do VF bloqueado com sucesso';
             
           elsif vTipo = 'AMBOS' then
              ...
              update  tdvadm.t_con_calcvalefrete ca
               ...
               where ca.con_conhecimento_codigo = vValeFrete
                 ...
                 and ca.con_calcvalefretetp_codigo in ('01','20');
                  ...
                  pMessage := 'Saldo e Adiantamento do VF bloqueado com sucesso';
                 
            end if;       
         else
         
            pMessage:= 'VF ' || vValeFrete ||'-'|| vVFSerie ||'-'|| vVFRota ||'-'|| vVFSaque||' não existe';
            pStatus := 'E';
         end if;
    
    -- Caso não tenha permissão, apresento a mensagem e paro a execução da procedure.
    else
      
      pMessage := 'Usuário não tem permissão para realizar processo, entre em contato com o Service Desk';
      pStatus := 'E';
      return;
        ...
        if pStatus = 'N' then
        wservice.pkg_glb_email.SP_ENVIAEMAIL('EXECUTADO COM SUCESSO',
                                            pMessage,
                                            'grp.hd@dellavolpe.com.br',
                                            trim(ptpRecebido.glb_benasserec_origem));
        else
        wservice.pkg_glb_email.SP_ENVIAEMAIL('ERRO AO REALIZAR PROCESSO',
                                            pMessage,
                                            'grp.hd@dellavolpe.com.br',
                                            trim(ptpRecebido.glb_benasserec_origem));
        end if;
       ...
       exception when others then
        pStatus  := 'E';
        pMessage := 'Erro ao realizar processo.';
        return;
        
       end;
  
end sp_bloqueia_vfcaixa;
...
*------------------------------------------------------------------------------------------------
* SISTEMA     : Faturamento
* PROGRAMA    : Ativar Tabela
* ANALISTA    : Rafael Aiti
* PROGRAMADOR : Rafael Aiti
* CRIACAO     : 19/10/2020
* BANCO       : ORACLE
* SIGLAS      : SLF, EDI
* DESCRICAO   : Processo de ativar tabela através de código de contrato e vigência
---------------------------------------------------------------------------------------------------
*/
as                           

    vContrato tdvadm.t_slf_tabela.slf_tabela_contrato%type;
    vVigencia tdvadm.t_slf_tabela.slf_tabela_vigencia%type;
    vContador number;
    
Begin    
--Exemplo de QueryString: --MSG=ATIVATAB;CONTRATO=C2017060100;VIGENCIA=19/06/2017;
  
       vContrato := tdvadm.fn_querystring(pTpRecebido.Glb_Benasserec_Assunto, 'CONTRATO','=', ';');
       vVigencia := tdvadm.fn_querystring(pTpRecebido.Glb_Benasserec_Assunto, 'VIGENCIA', '=', ';');
       pStatus := 'N';
       
  select count(*)
       into vContador
       from tdvadm.t_edi_planilhaaut p
      where p.edi_planilhacfg_codigo = 'ATIVATAB'
        and p.edi_planilhaaut_email = pTpRecebido.glb_benasserec_origem; 
        
        pMessage := 'Foi possível ativar as tabela do contrato '
        || vContrato || chr(10) || chr(10) || 'Segue número das tabelas:' 
        || chr(10);
     ...
     if vContador > 0 then
        ...
        for c_msg in (select ta.slf_tabela_codigo tabela,
                             ta.slf_tabela_saque  saque
                        from tdvadm.t_slf_tabela ta
                       where ta.slf_tabela_contrato = vContrato
                         and ta.slf_tabela_vigencia = vVigencia)
                         
        loop
          
           update tdvadm.t_slf_tabela sl
              set sl.slf_tabela_status = 'N'
            where sl.slf_tabela_codigo = c_msg.tabela
              and sl.slf_tabela_saque  = c_msg.saque;
              
        pMessage :=  pMessage || c_msg.tabela || '-' || c_msg.saque || chr(10);
      ...
      pStatus := 'E';
      pMessage := 'Você não tem permissão para esse tipo de envio!';
     ...
     exception
       when others then
        ...
        pStatus  := 'E';
        pMessage :=  'Erro ao rodar processo: ' || sqlerrm;
        
               wservice.pkg_glb_email.SP_ENVIAEMAIL('ATIVAR TABELA',
                                                    pMessage,
                                                    'rafael.aiti@dellavolpe.com.br',
                                                    trim(ptpRecebido.glb_benasserec_origem));
     end;
     ...
     pMessage := pMessage || chr(10) || 'Alterado por: ' || ptpRecebido.glb_benasserec_origem;
...
*------------------------------------------------------------------------------------------------
* SISTEMA     : Faturamento
* PROGRAMA    : Suspender Tabela
* ANALISTA    : Rafael Aiti
* PROGRAMADOR : Rafael Aiti
* CRIACAO     : 19/10/2020
* BANCO       : ORACLE
* SIGLAS      : SLF, EDI
* DESCRICAO   : Processo de suspender tabela através de código de contrato e vigência
---------------------------------------------------------------------------------------------------
*/
as                           
...
Begin    
--Exemplo de QueryString: --MSG=SUSPTAB;CONTRATO=C2017060100;VIGENCIA=19/06/2017;
  ...
  select count(*)
       into vContador
       from tdvadm.t_edi_planilhaaut p
      where p.edi_planilhacfg_codigo = 'SUSPTAB'
        and p.edi_planilhaaut_email = pTpRecebido.glb_benasserec_origem; 
        
        pMessage := 'Foi possível suspender as tabela do contrato '
        || vContrato || chr(10) || chr(10) || 'Segue número das tabelas:' 
        || chr(10);
     ...
     exception
       when others then
        ...
        pMessage :=  'Erro ao rodar processo: ' || sqlerrm;
               ...
               wservice.pkg_glb_email.SP_ENVIAEMAIL('SUSPENDER TABELA',
                                                    pMessage,
                                                    'rafael.aiti@dellavolpe.com.br',
                                                    trim(ptpRecebido.glb_benasserec_origem));
     end;
...
--          commit;
          ...
          vParamsEntradaB := glbadm.pkg_glb_blob.f_clob2blob(vParamsEntrada,vParamsEntradaB);
          tdvadm.pkg_fifo_recebimento.sp_insertnota(pparamsentrada => vParamsEntradaB,
                                                    pstatus => vStatus,
                                                    pmessage => vMessage2,
...
*/
   ...
   procedure SPI_ConfirmaNotasEmbarcadas( pTpRecebido in out rmadm.t_glb_benasserec%rowtype,
     ...
     vColunaseml   varchar2(200)   := 'SELECT DISTINCT E.SLF_CLIREGRASEMAIL_EMAIL EMAIL,E.SLF_CLIREGRASEMAIL_EMAILCC EMAILCC,E.SLF_CLIREGRASEMAIL_ENVIOINDIVIDUAL ENVIOINDIVIDUAL' || chr(10);
     ...
     vAchou        integer;
     v_Email       tdvadm.t_slf_cliregrasemail.slf_cliregrasemail_email%type;
     v_EmailCC     tdvadm.t_slf_cliregrasemail.slf_cliregrasemail_emailcc%type;
     v_EmailIndividual tdvadm.t_slf_cliregrasemail.slf_cliregrasemail_envioindividual%type;
     c_email       T_CURSOR;
     vContador2 number;
...
--       Exemplo MSG=LISTNOTA;MINUTA=9876459871;GRUPO=0535
        ...
        vGrupo      := nvl(Trim(tdvadm.fn_querystring(vQryString,'GRUPO','=',';')),'9999');
        vCnpj       := nvl(Trim(tdvadm.fn_querystring(vQryString,'CNPJ','=',';')),'99999999999999');
        vContrato   := nvl(Trim(tdvadm.fn_querystring(vQryString,'CONTRATO','=',';')),'999999999999999');
      ...
      End If;
       ...
       Begin -- INICIO DA MONTAGEM DO EMAIL
     ...
     --Exemplo de QryString = MSG=LISTNOTA;MINUTA=0549686301
        ...
        if vPlaca <> 'VFERRADO' Then
           ...
           vMessage := vMessage || 'https://extranet.dellavolpe.com.br/gerplanilha/get/result?codigoPlanilha=romValeSAC' || 
         ...
         group by tdvadm.fn_busca_codigoibge(c.con_conhecimento_localentrega,'IBD');
           ...
           open vCursor FOR SELECT DISTINCT RT.GLB_ROTA_CODIGO || '-' || TRIM(RT.GLB_ROTA_DESCRICAO) ROTA,
                                            ...
                                            CO.ARM_COLETA_DTSOLICITACAO DTSOLICITACAO,
                                            ...
                                            tdvadm.Fn_Con_RomaneioVfEntrega(VFC.CON_CONHECIMENTO_CODIGO, VFC.CON_CONHECIMENTO_SERIE, VFC.GLB_ROTA_CODIGO) PREVISAO,
     ...
     End If;
      ...
      End; -- FIM DA MONTAGEM DO EMAIL

       -- Muda a regra para envio, quando e para 
       -- 13/07/2021 Sirlano / Dantas
       -- qualquer GRUPO
       -- qualquer CNPJ
       -- qualquer CONTRATO
       for vLoop in 1..8 -- INICIO da variação as opcoes
       loop

           BEGIN -- INICIO DA SELECAO DOS EMAIL´s    
             vWhere := vWhereFixo;    
              If ( vLoop = 1 ) and 
                 ( vGrupo    <> '9999' ) and 
                 ( vCnpj     <> '99999999999999' ) and 
                 ( vContrato <> '999999999999999' ) Then
              ...
              ElsIf vLoop = 2  and 
                 ...
                 -- 13/07/2021 Sirlano / Dantas
                 --and ( vContrato = '999999999999999' ) 
                 Then
                 vWhere := vWhere || replace(vWhereGRP,'<<GRP>>',vGrupo);
                 vWhere := vWhere || replace(vWhereCNPJ,'<<vCNPJ>>',vCnpj);
                 vWhere := vWhere || replace(vWhereCTO,'<<vCONTRATO>>','999999999999999');
              ElsIf vLoop = 3  and 
                 ( vGrupo    <> '9999' ) and 
                -- 13/07/2021 Sirlano / Dantas
                -- ( vCnpj     = '99999999999999' ) and 
                 ( vContrato <> '999999999999999' ) Then
                 vWhere := vWhere || replace(vWhereGRP,'<<GRP>>',vGrupo);
                 vWhere := vWhere || replace(vWhereCNPJ,'<<vCNPJ>>','99999999999999');
                 vWhere := vWhere || replace(vWhereCTO,'<<vCONTRATO>>',vContrato);
              ElsIf vLoop = 4  and 
                 -- 13/07/2021 Sirlano / Dantas
                 --( vGrupo    = '9999' ) and 
                 ( vCnpj     <> '99999999999999' ) and 
                 ( vContrato <> '999999999999999' ) Then
                 vWhere := vWhere || replace(vWhereGRP,'<<GRP>>','9999');
                 vWhere := vWhere || replace(vWhereCNPJ,'<<vCNPJ>>',vCnpj);
                 vWhere := vWhere || replace(vWhereCTO,'<<vCONTRATO>>',vContrato);
              ElsIf vLoop = 5  and 
                 ( vGrupo    <> '9999' )  
                -- 13/07/2021 Sirlano / Dantas
                -- and ( vCnpj     = '99999999999999' ) and 
                -- ( vContrato = '999999999999999' ) 
                 Then
                 vWhere := vWhere || replace(vWhereGRP,'<<GRP>>',vGrupo);
                 vWhere := vWhere || replace(vWhereCNPJ,'<<vCNPJ>>','99999999999999');
                 vWhere := vWhere || replace(vWhereCTO,'<<vCONTRATO>>','999999999999999');
             ...
             /* ElsIf vLoop = 8  and
                 ...
                 vWhere := vWhere || replace(vWhereCTO,'<<vCONTRATO>>',999999999999999);*/
              End If;
              -- Se nao entrou em nenhuma condicao anteriror
              If vWhere <> vWhereFixo Then
            ...
            END; -- Fim da SELECAO DOS EMAIL
           
            If vAchou > 0 Then -- INICIO DO ACHOU ALGUM EMAIL
              
                Begin -- Inicio do disparo dos EMAIL´s
                 If ( vAchou > 0 ) and ( nvl(vEmail,'a') <> 'sdrumond@dellavolpe.com.br' ) Then
                    vEmail := ''; 
                    open c_email FOR vSql;
                    loop
                       fetch c_email into v_Email,v_EmailCC,v_EmailIndividual;
                       exit when c_email%notfound;               
                       -- Verifica se existe um " ; " para concatenar com o email defaul
                       If substr(nvl(v_EmailCC,';'),1,1) <> ';' Then
                          v_EmailCC := ';' || v_EmailCC;
                       End If;
                       If v_EmailIndividual = 'N' Then
                          vEmail := vEmail || v_Email || ';';     
                       End If;
                       vEmailCopia := 'romaneiolog@dellavolpe.com.br' || v_EmailCC;
                       ...
                       if length(trim(nvl(vEmail,'a'))) = 1 Then
                          wservice.pkg_glb_email.SP_ENVIAEMAIL('ROMANEIO - ' || vValeFrete || ' PLACA - ' || vPlaca || ' EMISSAO - ' || TO_CHAR(vEmissao,'DD/MM/YYYY') || ' ENTREGA - ' || vEntrega ,
                                                              pMessage,
                                                               'aut-e.romaneio@dellavolpe.com.br',
                                                               v_Email,
                                                               vEmailCopia);
                       End If;
                    ...
                    if length(trim(nvl(vEmail,'a'))) <> 1 Then
                       vEmailCopia := nvl(vEmailCopia,'romaneiolog@dellavolpe.com.br');
             ...
             exception
               When OTHERS Then
                   ...
                   pMessage := 'Erro Rodando Confirmação de Comprovantes . ' || chr(10) || 
                               ...
                               '********************************************************************************' || chr(10);
                   vEmail := 'sdrumond@dellavolpe.com.br';
               End ; -- FIM do disparo dos email´s

            End If; -- FIM DO ACHOU ALGUM EMAIL
       end loop; -- FIM  da variação as opcoes
  EXCEPTION
    when others then
       pStatus := pkg_glb_common.Status_Erro;
       pMessage := 'Erro Rodando Confirmação de Comprovantes . ' || chr(10) || 
   ...
   if pStatus = pkg_glb_common.Status_Nomal Then
       ...
       LOOP
          ...
          where f.glb_rota_codigofilialimp = '013'
            and f.con_fatura_ciclo = (select max(f1.con_fatura_ciclo)
                                        from tdvadm.t_con_fatura f1
                                       where 0 = 0
--                                         and f1.con_fatura_codigo = f.con_fatura_codigo
                                         and f1.glb_rota_codigofilialimp = f.glb_rota_codigofilialimp);
          ...
          LOOP
            ...
            If vCnpjPagante <> C_FATURA.Sacado Then
               ...
               -- 26/01/2022
               -- Sirlano
               -- Exception para pegar qual fatura esta com erro
               Begin
                  insert into t_con_fatura values tpFatura;
               exception
                  When OTHERS Then
                     raise_application_error(-20111,'Inserindo Fatura ' || tpFatura.Con_Fatura_Codigo || tpFatura.Glb_Rota_Codigofilialimp || chr(10) || sqlerrm);
                  End;
            End If;
...
Is
  ...
  vErro        clob;
  vCiclo       tdvadm.t_con_fatura.con_fatura_ciclo%type;
  vSaldo       number;
  vDtBaixa     date;
  vDtLimite    date;
  vContador    number;
...
Begin
   ...
   LOOP
      ...
      begin
          ...
          -- Vinicius Dantas -- alteração de ciclo para pegar o menor de uma fatura nao paga
          
          select min(f.con_fatura_ciclo)
            into vCiclo
            from tdvadm.t_con_fatura f
           where f.con_fatura_codigo = vFatura
             ...
             and 0 < (select count(*)
                       ...
                       where t.con_fatura_codigo = f.con_fatura_codigo
                         and t.glb_rota_codigofilialimp = f.glb_rota_codigofilialimp
                         and t.con_fatura_ciclo = f.con_fatura_ciclo
                         and nvl(t.crp_titreceber_pago,'N') = 'N');
          ...
          if length(trim(c_titulos.doccomptdv)) <= 12 Then
             tpTitEvento.Crp_Titrecevento_Nrodoc      := c_titulos.doccomptdv;
          Else
             tpTitEvento.Crp_Titrecevento_Nrodoc      := substr(trim(c_titulos.doccomptdv),-12,12);
          end If;
    ...
    --              tpplanilhaaut.edi_planilhaaut_permiteanexo := 'S';
              ...
              Begin
                  If vAnexo <> 'A'  and tpBenasserec.Glb_Benasserec_Status <> 'ER' Then
                      ...
                      elsif (UPPER(Trim(tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'MSG','=',';'))) = 'SUSPTAB') then    
                         --Exemplo de QryString =MSG=SUSPTAB;CONTRATO=C2017060100;VIGENCIA=02/10/2017;
                           sp_susptabela(tpBenasserec, vStatus, vMessage);
                           vAchouTarefa := 'S';
                         elsif (UPPER(Trim(tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'MSG','=',';'))) = 'ATIVATAB') then    
                         --Exemplo de QryString =MSG=ATIVATAB;CONTRATO=C2017060100;VIGENCIA=02/10/2017;
                           sp_ativatab(tpBenasserec, vStatus, vMessage);
                           vAchouTarefa := 'S';
                      ...
                      elsif (UPPER(Trim(tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'MSG','=',';'))) = 'BLOQVFCAIXA') then
                        --Exemplo de QryString = --MSG=;VF=067443;SERIEVF=A1;ROTAVF=620;SAQUEVF=1;TIPO=AMBOS;
                          sp_bloqueia_vfcaixa( tpBenasserec, vStatus, vMessage);
                          vAchouTarefa := 'S';    
                     ...
                     --Exemplo de QryString = MSG=REABREPLACA;PLACA=SLT0001;
                      ...
                      /* INICIO DO BLOCO PARA FINALIZAR O VEÍCULO DO COLETOR *** VINICIUS DANTAS 09/07/2021 ***/
                     Elsif (UPPER(Trim(tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'MSG','=',';'))) = 'FINALPLACA') then
                     --Exemplo de QryString = MSG=FINALPLACA;PLACA=SLT0001;
                        sp_arm_finalizarplacamobile(tpBenasserec,vStatus,vMessage);
                        vAchouTarefa := 'S'; 
                        
                      /* INICIO DO BLOCO PARA VINCULAR CNPJ NO CONTRATO *** VINICIUS DANTAS 20/09/2021 ***/
                     Elsif (UPPER(Trim(tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'MSG','=',';'))) = 'CLICONT') then
                     --Exemplo de QryString = MSG=CLICONT;CLIENTE=07170938001847;CONTRATO=C2015080004;
                        sp_glb_clientecontrato(tpBenasserec,vStatus,vMessage);
                        vAchouTarefa := 'S';                         
...
--       Exemplo MSG=NIQUEL;CODIGOBARRA=15170933592510007590550020000157331889371401;KM=1334;PEDIDO=123456778;PEDIDOVZ=23323345;RETORNOVAZIO=S
                     ...
                     ElsIf tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'MSG','=',';') = 'PROCINCTAB' Then
                          ...
                          select tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'PROTOCOLO','=',';')
                           ...
                           SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECARQCSV',Arqcsv);
                           SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECPORCCSV','CRIANDO TABELA');
                          ...
                          SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECPORCCSV','CRIANDO VALORES');
                          ...
                          SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECARQCSV','');
                          SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECPORCCSV','');
                          
                          vMessage := vMessage || chr(10) || ' Anexo Original - ' || trim(tpbenasserec.glb_benasserec_fileanexoorig);
                          
                          if vStatus = 'N' then                                                   
                                                     
                                wservice.pkg_glb_email.SP_ENVIAEMAIL('TABELA SUBIU COM SUCESSO' ||'-'|| tpbenasserec.glb_benasserec_origem || '-' || trim(tpbenasserec.glb_benasserec_fileanexoorig),
                                           vMessage,
                                           'aut-e@dellavolpe.com.br',
                                           tpbenasserec.glb_benasserec_origem,
                                           'sdrumond@dellavolpe.com.br');
                          Else                            
                                wservice.pkg_glb_email.SP_ENVIAEMAIL('ERRO SUBINDO TABLEA' ||'-'|| tpbenasserec.glb_benasserec_origem || '-' || trim(tpbenasserec.glb_benasserec_fileanexoorig),
                                           vMessage,
                                           'aut-e@dellavolpe.com.br',
                                           tpbenasserec.glb_benasserec_origem,
                    ...
                    /*      tdvadm.pkg_slf_tabelas.sp_CriaAtulizaTabela(tdvadm.fn_querystring(tpBenasserec.GLB_BENASSEREC_ASSUNTO,'PROTOCOLO','=',';'),
                               ...
                               Else
                                  tpBenasserec.Glb_Benasserec_Status     := vStatus;
                               End If;
                     ...
                     ElsIf tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'MSG','=',';') = 'INCTAB' and 
                           tdvadm.fn_querystring(upper(tpBenasserec.GLB_BENASSEREC_ASSUNTO),'ARQ','=',';') is not null then
                        
                        tpBenasserec.Glb_Benasserec_Fileanexo     := tdvadm.fn_querystring(tpBenasserec.GLB_BENASSEREC_ASSUNTO,'ARQ','=',';');
                        tpBenasserec.Glb_Benasserec_Fileanexoorig := tpBenasserec.Glb_Benasserec_Fileanexo;
                        tpBenasserec.Glb_Benasserec_Status        := 'MO';
                        update rmadm.t_glb_benasserec r
                           set r.glb_benasserec_fileanexo     = tpBenasserec.Glb_Benasserec_Fileanexo,
                               r.glb_benasserec_fileanexoorig = tpBenasserec.Glb_Benasserec_Fileanexoorig,
                               r.glb_benasserec_status        = tpBenasserec.Glb_Benasserec_Status
                        where r.glb_benasserec_chave = tpBenasserec.Glb_Benasserec_Chave;
                        
                        tdvadm.pkg_slf_tabelas .SP_CRITICAPLANILHA(tpBenasserec.Glb_Benasserec_Chave,
                                                                  vStatus,
                                                                  vMessage);
                     End if;
...
*/      
    ...
    begin
        ...
        -- 13/01/2021 - Sirlano
        -- Mudei o Filtro para NAO pegar somente os processos DO NIQUEL
        -- Mudei o Filtro para NAO pegar somente os processos DO CARVAO
        -- Mudei o Filtro para NAO pegar somente os processos DO CSV
          vSql := 'Select *
                   From rmadm.t_glb_benasserec b
                   Where b.glb_benasserec_status in (''IN'',''AP'')
                     ...
                     and NVL(b.glb_benasserec_assunto,''X'') not like ''%MSG=PROCINCTAB;PROTOCOLO=%'' -- Processa Tabelas CSV
...
--                                              'Chave ' || tpBenasserec.glb_benasserec_chave ||
   ...
   Procedure SP_BI_GLB_BENASSERECNIQUEL as
    ...
    begin
        ...
        -- 13/01/2021 - Sirlano
        -- Mudei o Filtro para pegar somente os processos DO NIQUEL
          vSql := 'Select *
                   From rmadm.t_glb_benasserec b
...
--                                              'Chave ' || tpBenasserec.glb_benasserec_chave ||
   ...
   Procedure SP_BI_GLB_BENASSERECCARVAO as
    ...
    begin
        -- 13/01/2021 - Sirlano
        -- Mudei o Filtro para pegar somente os processos DO CARVAO
        vSql := 'Select *
                 From rmadm.t_glb_benasserec b
                 Where b.glb_benasserec_status in (''IN'')
                   and b.glb_benasserec_processado is null
...
--                                              'Chave ' || tpBenasserec.glb_benasserec_chave ||
   ...
   Procedure SP_BI_GLB_BENASSERECARQCSV as
    ...
    begin
        -- 13/01/2021 - Sirlano
        -- Mudei o Filtro para pegar somente os processos CSV
        vSql := 'Select *
                 From rmadm.t_glb_benasserec b
                 Where b.glb_benasserec_status in (''IN'')
                   and NVL(b.glb_benasserec_assunto,''X'') like ''%MSG=PROCINCTAB;PROTOCOLO=%''
                 Order by b.glb_benasserec_chave';
          
          vProcesso := SYS_CONTEXT('PROCESSOUNICO','SP_BI_GLB_BENASSERECCSV') ;
          -- se ja estiver rodando não deixa rodar novamente
          If vProcesso = 'S' Then
             Return;
          End If;    
       ...
       -- para voltar o contexto para N
       begin
          SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECCSV','S');
          SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECINICSV',TO_CHAR(SYSDATE,'DD/MM/YYYY HH24:MI:SS'));

          sp_bi_glb_processa(vSql);

      SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECCSV','N');
      SYSTEM.pkg_glb_context.sp_set_vlr_PROCESSOUNICO('SP_BI_GLB_BENASSERECFIMCSV',TO_CHAR(SYSDATE,'DD/MM/YYYY HH24:MI:SS'));
...
--                                              'Chave ' || tpBenasserec.glb_benasserec_chave ||
    ...
    end SP_BI_GLB_BENASSERECARQCSV;
