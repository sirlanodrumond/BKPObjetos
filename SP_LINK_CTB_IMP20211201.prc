...
BEGIN
     ...
     FOR R_LAN IN C_LAN 
        LOOP
           ...
           If r_lan.refr < vRefCTB Then
              vMovimentoOK := 'N';
              cCritica := cCritica || 'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Referencia ' || r_lan.refr || ' Ja Fechada' || tdvadm.pkg_glb_html.PulaLinha;
           End If;
           ...
           If vAuxiliar = 0 Then
              vMovimentoOK := 'N';
              cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Referencia ' || r_lan.refr || ' nao Exite no movimento' || tdvadm.pkg_glb_html.PulaLinha;
           End If;
           ...
           If vAuxiliar = 0 Then
              vMovimentoOK := 'N';
              cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Conta ' || r_lan.debito || ' nao Exite no Plano de Contas ou nao e do QUINTO GRAU' || tdvadm.pkg_glb_html.PulaLinha;
           End If;
           ...
           If vAuxiliar = 0 Then
              vMovimentoOK := 'N';
              cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Conta ' || r_lan.credito || ' nao Exite no Plano de Contas ou nao e do QUINTO GRAU' || tdvadm.pkg_glb_html.PulaLinha;
           End If;

           If r_lan.valor <= 0 Then
              vMovimentoOK := 'N';
              cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Valor ' || r_lan.valor || ' Tem que ser maior que ZERO' || tdvadm.pkg_glb_html.PulaLinha;
           End If;
           
           If substr(r_lan.credito,1,1) in ('3','4') Then
              If vCC is null Then
                 vMovimentoOK := 'N';
                 cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Conta Credito ' || r_lan.credito || ' Exige Centro de Custo' || tdvadm.pkg_glb_html.PulaLinha;
              End If;
           End If;
           ...
           If substr(r_lan.debito,1,1) in ('3','4') Then 
              If vCC is null Then
                 vMovimentoOK := 'N';
                 cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Conta Debito ' || r_lan.debito || ' Exige Centro de Custo' || tdvadm.pkg_glb_html.PulaLinha;
              End If;
           End If;
           ...
           If vCC is not null Then
              ...
              If vAuxiliar = 0 Then
                 vMovimentoOK := 'N';
                 cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Centro Custo ' || vCC || ' nao Exite ' || tdvadm.pkg_glb_html.PulaLinha;
              End If;
           End If;
           ...
           If vCCC is not null Then
              ...
              If vAuxiliar = 0 Then
                 vMovimentoOK := 'N';
                 cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Centro Custo Contrato ' || vCCC || ' nao Exite ' || tdvadm.pkg_glb_html.PulaLinha;
              End If;
           End If;  
           ...
           If length(trim(r_lan.descricao)) > 60 then
              vMovimentoOK := 'N';
              cCritica := cCritica ||  'Documento ' || TRIM(TO_CHAR(R_LAN.VOUCHER,'0000000')) || ' Historico com mais de 60 Digitos, tamanho ' || length(trim(r_lan.descricao)) || tdvadm.pkg_glb_html.PulaLinha;
           End If;
  ...
  End If; --
    ...
    */
    ...
    IF vErro > 0 Then
       cCritica := 'INTEGRACAO NAO FOI GRAVADA| PROBLEMAS ' || tdvadm.pkg_glb_html.PulaLinha || tdvadm.pkg_glb_html.PulaLinha || 
                   vOk || ' Registros OK' || tdvadm.pkg_glb_html.PulaLinha || 
                   vErro || ' Registros Com erro,   ' || tdvadm.pkg_glb_html.PulaLinha || tdvadm.pkg_glb_html.PulaLinha || 
                   cCritica;
    Else
       cCritica := vOk || ' Registros Gravados' || tdvadm.pkg_glb_html.PulaLinha || tdvadm.pkg_glb_html.PulaLinha || 
                   cCritica;
    End If;
    If TpBenesseRec.Glb_Benasserec_Chave is not null Then
       cCritica :=  'PROTOCOLO - ' || to_char(TpBenesseRec.Glb_Benasserec_Chave) || tdvadm.pkg_glb_html.PulaLinha ||
                    'Remtente  - ' || TpBenesseRec.Glb_Benasserec_Origem || tdvadm.pkg_glb_html.PulaLinha ||
                    'Anexo     - ' || TpBenesseRec.Glb_Benasserec_Fileanexoorig || tdvadm.pkg_glb_html.PulaLinha ||
                    '*****************************************************************************' || tdvadm.pkg_glb_html.PulaLinha ||
                    cCritica;
