 ...
 **********************************************************************************************/
  ...
  v_bonus              Number(14,2);
  V_CONDESPECIAIS      tdvadm.t_con_valefrete.con_valefrete_condespeciais%type;  
  V_QTDIMG             Integer;
  ...
  /* Variaveis criadas para controlar a exigencia de imagem para o CTRC e/ou NF, na criac?o do valefrete. */
  ...
  CURSOR C_CTRCVEIC IS
    /* SELECT CONSIDERA TODOS OS CONHECIMENTOS QUE EST?O VINCULADO AO VEICULO DISPONIVEL PELO CARREGAMENTO*/
...
--       AND CA.ARM_CARREGAMENTO_CODIGO        = NT.ARM_CARREGAMENTO_CODIGO
       ...
       -- Depois que comecou a cobrar coleta em CTe separado 
    ...
    UNION
       ...
       WHERE 0 = 0
         ...
         AND CA.FCF_VEICULODISP_SEQUENCIA = P_FCF_VEICSEQ
    UNION
    /* SELECT CONSIDERA TODOS OS CONHECIMENTOS QUE EST?O VINCULADOS AO VEICULO DISPONIVEL( CONHECIMENTOS N?O FORAM FEITOS PELO O FIFO)*/
  ...
  begin
    ...
    IF V_CONTADOR = 0 THEN
      RAISE_APPLICATION_ERROR(-20001,
                              CHR(10) ||
                              'N?O FOI DEFINIDO OS DESTINOS NO CARREGAMENTO!' ||
                              CHR(10) ||
                              ' VERIFIQUE OS DESTINOS NA MESA DE CONTRATAC?O');
    END IF;
    ...
    LOOP
      ...
      /* Alimento a variavel com o Parametro de excess?o para Imagem da CTRC  { 'S' ou 'N' }*/ 
      elsif Trim(V_PARAMETRO) = 'ROTAEXIMGCTRC' THEN    V_EXCESSAO_CTRC := V_PARAMTEXTO;
      /* Alimento a variavel com o Parametro de excess?o para Imagem da Nota Fiscal { 'S' ou 'N' }*/
...
/*             RAISE_APPLICATION_ERROR(-20001, CHR(10) ||
    ...
    /** SE N?O ENCONTRAR NUMERO DO VALE DE FRETE        **/
    IF (V_MAXCTRC = 0) OR (V_VERIFICAMAXCTRC is null) THEN
  ...
  begin
    ...
    IF V_CTRCSERIE = NULL THEN
      RAISE_APPLICATION_ERROR(-20001,'N?O EXISTE NOS CARREGAMENTOS A ROTA DA FILIAL! VERIFIQUE OS CONHECIMENTOS!');
    END IF;
...
--            END IF;
      ...
      IF ( V_VERIFICACTRC IS NOT NULL ) and ( vAuxiliar > 1 ) THEN
      ...
      BEGIN
        ...
        FOR R_NOTAS IN (SELECT lpad(NT.ARM_NOTA_NUMERO, 6, '0')                      ARM_NOTA_NUMERO,
                          ...
                          FROM 
                           ...
                           T_GLB_CLIENTE           CLd
                         WHERE 
                           --Passo o numero do carregamento recuperado no cursor de carregamento.
...
--              END IF;
        ...
        --NOTAS FISCAIS QU8E S?O PROVENIENTES DE CTRC'S DA DIG MANUAL. 
  ...
  begin
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        BEGIN
          IF P_RETORNOPROCESSAMENTO IS NULL THEN
            P_RETORNOPROCESSAMENTO := 'CAT VF n?o encontrada -> ' ||
                                      P_PLACA;
          ELSE
            P_RETORNOPROCESSAMENTO := substr(P_RETORNOPROCESSAMENTO ||
                                             CHR(10) ||
...
--                  END IF;
                  ...
                  IF P_RETORNOPROCESSAMENTO IS NULL THEN
                    P_RETORNOPROCESSAMENTO := 'KM1 n?o encontrado -> Origem: ' ||
                                              R_KM.ORIGEM || ' Destino: ' ||
                                              R_KM.DESTINO || '-' || v_km;
                  ELSE
                    P_RETORNOPROCESSAMENTO := substr(P_RETORNOPROCESSAMENTO ||
...
--                  END IF;
                  ...
                  IF P_RETORNOPROCESSAMENTO IS NULL THEN
                    ...
                    P_RETORNOPROCESSAMENTO := ' KM2 n?o encontrado -> Origem: ' ||
                                              V_PROXIMOORIGEM ||
...
--          END IF;
          ...
          IF P_RETORNOPROCESSAMENTO IS NULL THEN
             P_RETORNOPROCESSAMENTO := ' KM4 n?o encontrado -> Origem: ' ||
                                       R_KM.ORIGEM || ' Destino: ' ||
                                       R_KM.DESTINO || '- KM: ' || v_km || '- PROXORIGEM: ' || V_PROXIMOORIGEM || '- ORDEM: ' || R_KM.ORDEM;
          ELSE
            P_RETORNOPROCESSAMENTO := substr(P_RETORNOPROCESSAMENTO ||
                                             CHR(10) ||
                                             ' KM5 n?o encontrado -> Origem: ' ||
                                             R_KM.ORIGEM || ' Destino: ' ||
                                             R_KM.DESTINO || '-' || v_km,
                                             1,
                                             200);
...
--         AND ER.GLB_LOCALIDADE_CODIGO = V_MAIORORIGEM
    ...
    BEGIN
          ...
          IF V_CIDDEST IS NULL THEN
            BEGIN
              IF P_RETORNOPROCESSAMENTO IS NULL THEN
                 P_RETORNOPROCESSAMENTO := 'Cidade Destinatario ' || V_CIDDEST || ' n?o encontrado. Verifique na Mesa de Contratac?o';
             ELSE
                P_RETORNOPROCESSAMENTO := substr(P_RETORNOPROCESSAMENTO ||
                                                 CHR(10) ||
                                                 'Cidade Destinatario ' || V_CIDDEST || ' n?o encontrado.',
                                                 1,
                                                 200);
              END IF;
            END;
          ...
          IF V_RAZAOREM IS NULL THEN
            BEGIN
              IF P_RETORNOPROCESSAMENTO IS NULL THEN
                P_RETORNOPROCESSAMENTO := 'Raz?o Social do Remetente ' || V_RAZAOREM  || ' n?o encotrado.';
              ELSE
                P_RETORNOPROCESSAMENTO := substr(P_RETORNOPROCESSAMENTO ||
                                                 CHR(10) ||
                                                 'Raz?o Social do Remetente ' || V_RAZAOREM  || ' n?o encotrado.',
                                                 1,
                                                 200);
              END IF;
              V_RAZAOREM := '';
          ...
          IF V_CIDDEST IS NULL THEN
            BEGIN
              IF P_RETORNOPROCESSAMENTO IS NULL THEN
                P_RETORNOPROCESSAMENTO := 'Cidade do Destinatario ' || V_CIDDEST  || ' n?o localizado.';
              ELSE
                P_RETORNOPROCESSAMENTO := substr(P_RETORNOPROCESSAMENTO ||
                                                 CHR(10) ||
                                                 'Cidade do Destinatario ' || V_CIDDEST  || ' n?o localizado.',
                                                 1,
                                                 200);
              END IF;
            END;
          ...
          IF V_RAZAODEST IS NULL THEN
            BEGIN
              IF P_RETORNOPROCESSAMENTO IS NULL THEN
                P_RETORNOPROCESSAMENTO := 'Raz?o Social do Destinatario ' || V_RAZAODEST || ' n?o encontrado.';
              ELSE
                P_RETORNOPROCESSAMENTO := substr(to_char(P_RETORNOPROCESSAMENTO ||
                                                         CHR(10) ||
                                                         'Raz?o Social do Destinatario ' || V_RAZAODEST || ' n?o encontrado.'),
                                                 1,
                                                 200);
              END IF;
            END;
        ...
        If vProcessoSV = 'N' and vProcessoMS = 'N' Then
          ...
          exception
            WHEN TOO_MANY_ROWS  THEN
               RAISE_APPLICATION_ERROR(-20001,'VEICULO DISP [' || P_FCF_VEICULOCOD || '] em mais de uma SOLICITACAO');     
            WHEN NO_DATA_FOUND Then
              -- Sirlano
              -- 12/08/2021
              -- Quando veiculo vinculado pelo coletor
              -- deixou de colocar na MEMO o VeiculoDisp desde o dia 10/08/2021
                update tdvadm.t_fcf_fretecarmemo m
                   set m.fcf_veiculodisp_codigo = P_FCF_VEICULOCOD,
                       m.fcf_veiculodisp_sequencia = P_FCF_VEICSEQ,
                       m.fcf_fretecarmemo_acrescimo = nvl(m.fcf_fretecarmemo_acrescimo,0)
                where m.fcf_veiculodisp_codigo is null
                  and m.fcf_solveic_cod = (select sv.fcf_solveic_cod
                                           from tdvadm.t_fcf_solveic sv 
                                           where sv.fcf_veiculodisp_codigo = P_FCF_VEICULOCOD 
  ...
  --          AND VD.ARM_CARREGAMENTO_CODIGO IS Not Null 
    ...
    --                                      AND D1.ARM_CARREGAMENTO_CODIGO IS Not NULL 
      ...
      EXCEPTION WHEN NO_DATA_FOUND THEN
        ...
        RAISE_APPLICATION_ERROR(-20431,chr(10) || 
                                       '**********************************************************************' || chr(10) || 
                                       'Verificar Veiculo - FRETE NÃO FOI VINCULADO NA MESA - ' || P_FCF_VEICULOCOD  || ' SEQ. - ' || P_FCF_VEICSEQ || chr(10) || 
                                       V_ROTATRAVADA||' - '||P_PLACA||' - '||P_TPMOTORISTA || ' - ' || v_diasfretemesa || chr(10) ||
                                       '**********************************************************************' || chr(10));
        V_FRETEFECHADO    := 0;
        V_FRETEEXCECAO    := 0;
        v_PARTICULARIDADE := 0;
  ...
  begin
    If vProcessoSV = 'N' and vProcessoMS = 'N' Then
      V_CUSTOCARRETEIRO := tpFreteMemo.Fcf_Fretecarmemo_Frete +
                           tpFreteMemo.Fcf_Fretecarmemo_Entrega +
                           tpFreteMemo.Fcf_Fretecarmemo_Coleta +
      ...
      V_PEDAGIO :=  tpFreteMemo.Fcf_Fretecarmemo_Pedagio;                     
...
--           V_CUSTOCARRETEIRO := V_FRETEFECHADO +  V_ACRESCIMO;
      ...
      tpValeFrete.CON_VALEFRETE_PESOINDICADO    := TO_NUMBER(REPLACE(NVL(V_PESOREALNEW / 1000, 0), ',', '.'));
      tpValeFrete.CON_VALEFRETE_LOTACAO         := TO_NUMBER(REPLACE(NVL(V_PESOCOBRADONEW / 1000, 0), ',', '.'));
      tpValeFrete.CON_VALEFRETE_PESOCOBRADO     := TO_NUMBER(REPLACE(NVL(V_PESOCOBRADONEW / 1000, 0), ',', '.'));
    ...
    --select fn_retornavalorfretebonus(V_CUSTOCARRETEIRO, 15, 'B')
        ...
        IF v_Bonus > 0 Then
           -- 07/06/2021
           -- Sirlano 
           -- Inclui o Desconta bonus
--           tpValeFrete.con_valefrete_descbonus        := nvl(V_CALCBONUS,'S'); -- tdvadm.PKG_CON_VALEFRETE.Fn_VerificaDescBonus(vCONTRATO,tpValeFrete.Glb_Tpmotorista_Codigo);
 
--           tpValeFrete.con_valefrete_descbonus := nvl(tpValeFrete.con_valefrete_descbonus,'S');

--           If nvl(tpValeFrete.con_valefrete_descbonus,'X') = 'S' Then
--              V_CONDESPECIAIS := 'RECEBER COMISS?O S/BONUS DE R$ ' || TRIM(TO_CHAR(v_Bonus,'999,999,990.00')) || ' NA PONTA SE ENTR.NO DIA COMBINADO, PAGAMENTO AUTORIZADO SOMENTE NAS FILIAIS';
...
--           End If;
        Else    
           V_CUSTOCARRETEIRO := 0;
           V_BONUS := 0;
           tpValeFrete.con_valefrete_descbonus := null ; --'N';
           V_CONDESPECIAIS := '';    
        End if;
    End IF;
    V_OBRIGACOES := trim(V_OBRIGACOES) ;
    
    IF LENGTH(TRIM(V_OBRIGACOES)) > 0 THEN
       V_OBRIGACOES := SUBSTR(V_OBRIGACOES || ' ',1,70);
    END IF; 
    ...
    End IF;
      ...
      tpValeFrete.Con_Valefrete_Freteoriginal   := tpValeFrete.Con_Valefrete_Custocarreteiro;
      tpValeFrete.Con_Valefrete_Pedagiooriginal := tpValeFrete.Con_Valefrete_Pedagio;
...
/*
      If sql%rowcount > 0 Then
         If tpValeFrete.Con_Valefrete_Saque = '1' Then
            update tdvadm.t_fcf_fretecarmemo m
...
--               And dt.fcf_veiculodispdest_dtentrega Is Not Null
  ...
  begin
    ...
    IF nvl(V_ERRO_PROCESSAMENTO, 'N') <> 'E' THEN
      ...
      P_RETORNOPROCESSAMENTO := P_RETORNOPROCESSAMENTO || chr(10) || 'Problemas na criac?o do VALE DE FRETE.: '||chr(10) || 'erro: ' || sqlerrm;
  ...
  exception
    When OTHERS Then
      ...
      -- 17/06/2021
      -- Sirlano
      -- Caso tenha que deletar o Vale de Frete tira a Chave da MEMO
      If tpValeFrete.Con_Valefrete_Saque = '1' Then
         update tdvadm.t_fcf_fretecarmemo m
           set m.con_valefrete_codigo = null,
               m.con_valefrete_serie  = null,
               m.glb_rota_codigo      = null,
               m.con_valefrete_saque  = null
         where m.fcf_veiculodisp_codigo = P_FCF_VEICULOCOD
           and m.fcf_veiculodisp_sequencia = P_FCF_VEICSEQ;
      Else
         update tdvadm.t_fcf_fretecarmemo m
           set m.con_valefrete_codigo2 = null,
               m.con_valefrete_serie2  = null,
               m.glb_rota_codigo2      = null,
               m.con_valefrete_saque2  = null
         where m.fcf_veiculodisp_codigo = P_FCF_VEICULOCOD
           and m.fcf_veiculodisp_sequencia = P_FCF_VEICSEQ;
      End If;
      ...
      commit;
      P_RETORNOPROCESSAMENTO := 'Problemas na criac?o do VALE DE FRETE.: '||chr(10) || 'erro: ' || sqlerrm;
