create or replace package pkg_migracao_sap is
...
/
CREATE OR REPLACE PACKAGE BODY pkg_migracao_sap IS
...
vCliRed  TpCliRed;                            
begin    
    FOR vCursor IN (SELECT distinct s.tab_cidade,
                                    s.tab_estado,
                                    s.tab_pais,
  ...
  /*  
    ...
    LOOP
     ...
     IF (NVL(PIDCLIENTE, 'X') <> 'X') AND (vCount <> 0) THEN
        ...
        tpCUSTOMER_S_CUST_COMPANY.aKONT    := 'teste'; --Obrigatório(S) - Conta de reconciliação*
        Begin                        
...
begin
    ...
    FOR vCursor IN (   
      SELECT DECODE(M.FRT_MARCMODAPELIDO_DESCRICAO, 'NAO DEFINIDO', '', M.FRT_MARCMODAPELIDO_DESCRICAO) || ' - ' || A.FRT_MARMODVEIC_DESCRICAO DESCRICAO,
             ...
             DECODE(M.FRT_MARCMODAPELIDO_DESCRICAO, 'NAO DEFINIDO', 'ND', 
                                                    ...
                                                    'GUINCHO','GG',
                                                    'MOTO','MM',
                                                    M.FRT_MARCMODAPELIDO_DESCRICAO) TIPO, 
             'KG' UNIDADE,
    ...
    LOOP
        ...
        tpEQUIPMENT_S_EQUI.EQTYP     := vCursor.TIPO;  -- Obrigatório(S) - Categoria de equipamento *
        ...
        tpEQUIPMENT_S_EQUI.MATNR     := vCursor.PLACA; -- Obrigatório(N) - Identificador de Material
        tpEQUIPMENT_S_EQUI.GERNR     := NVL(SUBSTR(vCursor.CHASSI, 1, 18), ' '); -- Obrigatório(N) - Número de série
                                         
        IF vCursor.TRACAO = 'NAO' THEN
         -- Select para encontrar veículo de tracao que está engatado
           FOR vCursorEngate IN(
               ...
               where 0 = 0
                 ...
                 AND ce.frt_conjveiculo_codigo = ( select ce1.frt_conjveiculo_codigo
                                                   from tdvadm.t_frt_conteng ce1
                                                   where ce1.frt_veiculo_codigo = vCursor.VEIC_CODIGO) -- código do veículo
                                )
           LOOP
               tpEQUIPMENT_S_EQUI.TPLNR := vCursorEngate.PLACA;
           End Loop;
        End If;
...
--                                                  'SP_SAP_INSERE_EQUIPAMENTO - Placa - ' || tpEQUIPMENT_S_EQUI.TPLNR);
        ...
        tpFUNC_LOC_S_FUN_LOCATION.EXTERNAL_NUMBER := vCursor.PLACA; -- Obrigatório(S) - Identificador do local de instalação (sequencial ou identificador do legado)
        ...
        */
        ...
        tpFUNC_LOC_S_FUN_LOCATION.FLTYP      := 'NAO SEI'; -- Obrigatório(S) - Categoria do local de instalação
        ...
        tpFUNC_LOC_S_FUN_LOCATION.EQART      := 'NAO SEI'; -- Obrigatório(N) - Tipo de objeto técnico
        tpFUNC_LOC_S_FUN_LOCATION.INVNR      := ''; -- Obrigatório(N) - Número de inventário
        tpFUNC_LOC_S_FUN_LOCATION.BRGEW      := 0;--NVL(vCursor.PESO_VEICULO, 0); -- Obrigatório(N) - Peso do objeto
        ...
        tpFUNC_LOC_S_FUN_LOCATION.GSBER      := 'NAO SEI'; -- Obrigatório(N) - Área de trabalho
        tpFUNC_LOC_S_FUN_LOCATION.KOSTL      := ' '; -- Obrigatório(N) - Centro de custo
...
begin  
    ...
    LOOP
        ...
        tpMATERIAL_S_MARA.GEWEI    := 'KG'; -- Obrigatório(N) - Unidade de Peso
        ...
        tpMATERIAL_S_MARM.BRGEW := 0; -- Obrigatório(N) - Peso bruto
        tpMATERIAL_S_MARM.GEWEI := 'M'; -- Obrigatório(N) - Unidade de Peso
...
begin
    ...
    LOOP
        ...
        for vCursor2 in (
            ...
            FROM MIGDV.OPEN_ITEM_AP_S_BSIK C
        ...
        tpOPEN_ITEM_AP_S_BSIK.LIFNR := vCursor.LIFNR ; -- Obrigatório(S) - Identificador do Fornecedor*
        ...
        tpOPEN_ITEM_AP_S_BSIK.ZTERM := vCursor.ZTERM ; -- Obrigatório(N) - Termos de pagamento
        tpOPEN_ITEM_AP_S_BSIK.ZFBDT := vCursor.ZFBDT ; -- Obrigatório(N) - Data base
...
begin
    ...
    LOOP
        ...
        tpOPEN_ITEM_AR_S_BSID.KUNNR := vCursor.KUNNR ; -- Obrigatório(S) - Identificador do Cliente
        ...
        tpOPEN_ITEM_AR_S_BSID.ZTERM := vCursor.ZTERM ; -- Obrigatório(N) - Termos de pagamento
        tpOPEN_ITEM_AR_S_BSID.ZFBDT := vCursor.ZFBDT ; -- Obrigatório(N) - Data base
...
--                                            'SP_SAP_INSERE_CONTAS_RECEBER_DOCUMENTOS - Titulo ' || vCursor.BLART || '-' || vCursor.XBLNR);
        ...
        tpGL_BALANCES_S_BSIS.HKONT := ' ';            -- Obrigatório(S) - Conta GL*
        ...
        tpGL_BALANCES_S_BSIS.FWBAS := ' ';            -- Obrigatório(N) - Montante base do imposto na moeda do documento
        tpGL_BALANCES_S_BSIS.HWBAS := ' ';            -- Obrigatório(N) - Montante base do imposto na moeda local
