CREATE OR REPLACE PROCEDURE sp_cax_movimento_acc_NOVO
 ...
 IS
    SELECT a.acc_acontas_numero, 
     ...
     WHERE a.frt_motorista_codigo = m.frt_motorista_codigo
       AND TRUNC (a.acc_acontas_data) BETWEEN p_datainicial AND p_datafinal
       AND a.acc_acontas_datatransf = v_caixa ; -- IS NULL;
...
---------     DELETE t_cax_movimento;
  ...
  select max(b.CAX_BOLETIM_DATA) BOLETIM
   ...
   WHERE b.GLB_ROTA_CODIGO = '015'
     ...
     and b.CAX_BOLETIM_DATA >= TRUNC(last_day(add_months(SYSDATE,-1))+1)
...
--               v_motorista := 'EDUARDO SOARES'; 
          ...
          IF V_VAZIO > 0 THEN
            ...
            INSERT INTO t_cax_movimento (glb_rota_codigo,
                                             cax_boletim_data,
