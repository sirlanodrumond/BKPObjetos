...
CREATE OR REPLACE PACKAGE BODY PKG_SLF_CONTRATO AS
    ...
    if vXMLColeta is null Then
      ...
      Exception
        When NO_DATA_FOUND Then
          ...
          vEntCol      := '';
      End;
      If vCodTpcarga in ('01', '11') Then
        vLotacao := 'S';
      ElsIf vCodTpcarga in ('02', '12', '10') Then
        vFracionado := 'S';
  ...
  Begin
    ...
    exception
      When NO_DATA_FOUND Then
        pStatus  := 'E';
        pMessage := 'Nota não Existe ou não esta vinculada a uma coleta. ';
        return;
    End;
    ...
    If (vInformaCli = 'S') and (vAuxiliar = 0) Then
      ...
      If vStatus = 'N' Then
        ...
        Else
          ...
          -------------------------------------------------------------------------
          -----------------------Thiago 03/10/2019---------------------------------
          --Inclusão de log para identificar erro de criação de Janela Vale      --
          -------------------------------------------------------------------------   
    ...
    If pStatus = 'N' then
      ...
      ElsiF (vContrato in ('C5900011012', -- 0020 - MRO-VALE-0314-CS2781910
        ...
        If vContrato in
          ...
          -- Verificação para a vale, validar nfe
          IF (vGrupo = '0020') THEN
...
/*          update t_arm_coleta co
      ...
      ElsIF (vContrato in ('COC91566000')) Then
        ...
        ----------Conforme solicitação do faturamento-----------------
      ...
      ElsIf (vContrato in ('C4600005946', -- MRO-ALUNORTE-1116-XXX
        ...
        ---------Negado conforme solicitação do faturamento----
      ...
      ElsIF (vContrato in ('C2013060061', -- LOT-VOTORANTIM-0613-SULFATO
                           ...
                           'C2016020026', -- MRO-VOTORANTIM-0216-ENERGIA
      ...
      -- Condições abaixo negadas conforme ticket 114835 a pedido do faturamento.
...
*/      ElsIF (vContrato in ('C2017020107')) Then
      ...
      ElsIf (vContrato = 'C2017110116') Then
        ...
        --Teste será realizado para entrar em produção dia 06/01/2020
...
/*      ElsIf (vContrato = 'C2018010117') Then
      ...
      ElsIf (vContrato = 'C2018010117/2') Then
        ...
        *\
        ...
        Else
          ...
          exception
            When NO_DATA_FOUND Then
              -- pode não achar o carregamento durante a digitacao da nota
              vCarregamento := '';
          end;
        End If;
      ...
      ElsIf vContrato = 'C4600005631' then  -- MRO-PARANAPANEMA-0820-XXX
       ...
       If nvl(vPedido,'X') = 'X' Then       
          pStatus := 'E';
          pMessage := pMessage || chr(10) || 'Pedido Obrigatorio para este Contrato. Não informado na COLETA [' || vColeta || '-' || vCiclo || '] !';
       End If;      
      ...
      ElsIf (vContrato in ('C4600006653','C7000067442') -- MRO-USI MECANICA-0318-XXX
        ...
        IF (vGrupoRem = vGrupo) and (nvl(vCentroCusto, 'X') = 'X') Then
          pStatus  := 'E';
          pMessage := pMessage || chr(10) ||
                      'Codigo da DT no campo CENTRO DE CUSTO, n?o informado na COLETA [' ||
                      vColeta || '-' || vCiclo || '] !';
        Else
          update tdvadm.t_arm_nota an
             set an.arm_nota_codclicoleta = vCentroCusto
        ...
        End If;
          ...
          -- Sirlano
          -- 14/12/2020 - Para não rodar sem querer no TDP
      ...
      Elsif vContrato in ('C2019121134') -- MRO-PETROPOLIS-1219-XXX
        then
       ...
       --Particularidade adicionada dia 11/02 a pedido do faturamento, chamado (122101) -- Rafael Aiti
       
      ElsIf vContrato in ('C2020020004') -- MRO-VALMET-0220-XXX
        then
      ...
      ElsIf vContrato in ('C2020050006','C2019120027') --ANGLO
       then
        If nvl(vPedido,'X') = 'X' Then
      ...
      ElsIf vContrato in ('C2018010119') Then
        -- MRO-RAIZEN-0119-XXX
...
/*      Elsif vContrato in ('C2020070004') Then --LOT-GE-0720-XXX
      ...
      Elsif vContrato in ('C2020110001') -- NOURYON Adicionado conforme chamado 172932 - Alexandre Malçar 09/11/2020
        Then      
           If nvl(vPedido,'X') = 'X' Then
      ...
      ElsIf vContrato in ('C2020020002') --PRADA - PARTICULARIDADE CRIADA DE ACORDO COM O CHAMADO 177105 - ALEXANDRE
       then
        If nvl(vPedido,'X') = 'X' Then
       ...
       End If;
       /*
       Elsif vContrato in ('C2020120001') --  Adicionado conforme chamado 184953 
                                           -- Alexandre Malçar 04/01/2021
                                           --Retirado conforme pedido 187133 (18/01/2021)
        Then      
           If nvl(vPedido,'X') = 'X' Then
           ...
           End If;
      */
      Elsif vContrato in ('C2021020001') --  Adicionado conforme chamado 187683 
                                           -- Alexandre Malçar 18/01/2021
  ...
  Begin
    ...
    Begin
      ...
      If vPesoNota > 3000 Then 
       ...
       --Se foi finalizada pesagem, não usa data etiqueta e sim da finalização--
       -------------------------------------------------------------------------
    ...
    select an.arm_coleta_ncompra,
     ...
     select cp.arm_coletapart_dtenvcli
      ...
      where cp.arm_coleta_ncompra = vColeta
       ...
       -------------------------------------------------------
       ---------------Rafael Aiti 07/10/2019------------------
       --Se o Cliente já foi informado, não cria mais janela--
       -------------------------------------------------------
      ...
      Begin
        ...
        Begin
          ...
          where cp.arm_coletapart_remetente = cl.glb_cliente_cgccpfcodigo
            ...
            and cl.glb_ramoatividade_codigo = '50'; -- AQUAVIÁRIO, TRANSPORTE
