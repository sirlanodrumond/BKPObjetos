                         ...
                         When OTHERS Then
                            P_STATUS   := PKG_GLB_COMMON.Status_Erro;
                             P_MESSAGE :=  P_MESSAGE || chr(10) ||
                                                        ...
                                                        'Venciamento - '  || tpContaCorrente.CAR_CONTACORRENTE_DTVENC      || chr(10) ||
                                                        'erro-' || sqlerrm;
     ...
     exception
       ...
       Begin
          ...
          if  vAcao = 'Salvar' then
           ...
           ELSE
              if nvl(vValeFrete.Con_Valefrete_Impresso,'N') <> 'S' Then
...
--         and x.con_valefrete_saque = pVfreteSaque
  ...
  for c_Cursor in (select distinct
                               ...
                               to_date(to_char(a.con_valefrete_dataprazomax,'dd/mm/yyyy') || ' ' || to_char(a.con_valefrete_horaprazomax,'hh24:mi'),'dd/mm/yyyy hh24:mi') dtentrega,
...
*/                            group by a.con_valefrete_saque,
                                     ...
                                     ce.glb_tpcliend_codigo,
                                     ce.glb_cliend_codcliente,
                                     to_date(to_char(a.con_valefrete_dataprazomax,'dd/mm/yyyy') || ' ' || to_char(a.con_valefrete_horaprazomax,'hh24:mi'),'dd/mm/yyyy hh24:mi'),
    ...
    begin
        if P_QRYSTR is null Then
           ...
           vC_QRYSTR := '<Parametros>    <Input>     <VFNumero>083341</VFNumero>      <VFSerie>A1</VFSerie>     <VFRota>185</VFRota>     <VFSaque>1</VFSaque>     <VFAcao>Salvar</VFAcao>     <VFUsuarioTDV>jsantos</VFUsuarioTDV>     <VFRotaUsuarioTDV>185</VFRotaUsuarioTDV>     <VFAplicacaoTDV>comvlfrete</VFAplicacaoTDV>     <VFVersaoAplicao>16.5.23.0</VFVersaoAplicao>     <VFProcedureDelphi></VFProcedureDelphi>     <CON_CATVALEFRETE_CODIGO>01</CON_CATVALEFRETE_CODIGO>     <CON_SUBCATVALEFRETE_CODIGO></CON_SUBCATVALEFRETE_CODIGO>     <CON_VALEFRETE_PERCETDES>20</CON_VALEFRETE_PERCETDES>     <CON_VALEFRETE_MULTA>0</CON_VALEFRETE_MULTA>     <CON_VALEFRETE_PLACA>MPF7989</CON_VALEFRETE_PLACA>     <CON_VALEFRETE_PLACASAQUE>0004</CON_VALEFRETE_PLACASAQUE>     <CON_VALEFRETE_CUSTOCARRETEIRO>499</CON_VALEFRETE_CUSTOCARRETEIRO>     <CON_VALEFRETE_TIPOCUSTO>U</CON_VALEFRETE_TIPOCUSTO>     <FCF_FRETECAR_ROWID>AAAReDACMAAAM7SAA1</FCF_FRETECAR_ROWID>     <FCF_VEICULODISP_CODIGO>2403064</FCF_VEICULODISP_CODIGO>     <FCF_VEICULODISP_SEQUENCIA>0</FCF_VEICULODISP_SEQUENCIA>     <GLB_CLIENTE_CGCCPFCODIGO></GLB_CLIENTE_CGCCPFCODIGO>   </Input></Parametros>';
               ...
               where trim(d.glb_sql_programa) = 'teste13/08'
         ...
         when OTHERS Then
              begin
               select d.glb_sql_instrucao
                 into vC_QRYSTR
               from tdvadm.t_glb_sql d
               where trim(d.glb_sql_programa) = 'teste22/08'
                 and d.glb_sql_observacao like '%Sirlano%'
                 and rownum = 1;
              Exception
                When OTHERS Then
...
--                tpVf.vAuxiliarTexto := 'R';
           ...
           where vfc.con_valefrete_codigo     = tpVF.VFNumero
             ...
             and pkg_slf_utilitarios.fn_retorna_contratoCod(vfc.CON_CONHECIMENTO_CODIGO,
                                                            vfc.CON_CONHECIMENTO_SERIE,
                                                            vfc.GLB_ROTA_CODIGO) in ('C5900010792');  -- TCN


           begin

               if ( tpVF.vPrimeiroSaque = tpvf.VFSaque )  and  -- Primeiro Saque Valido
                  ( tpVF.vAuxiliar = 0 ) and
                  ( tpVf.VFRota <> '026' ) and
                  ( tpVF.VFRota < '900' ) Then
                  If (tpVF.v_Proprietario = '61139432000172') and tpvf.vfCARGO_MOTORISTA <> 'UTL'  then -- É Frota
          ...
          begin
              ...
              Begin
                 Select CI.con_vfreteciot_numero,
                        NVL(CI.CON_VFRETECIOT_FLAGIMPRIME,'N')
                      ...
                      where vfe.con_conhecimento_codigo = plistaVFEntregas(tpVF.vAuxiliar).con_conhecimento_codigo
                       ...
                       If ( nvl(tpVfreteentregas.Con_Vfreteentrega_Dtentrega,'01/01/1900') <> plistaVFEntregas(tpVF.vAuxiliar).con_vfreteentrega_dtentrega ) or
                        ...
                        Elsif tpVfreteentregas.Con_Vfreteentrega_Confirmado is null and
                              ( ( tpVfreteentregas.Con_Vfreteentrega_Dtentrega  <> plistaVFEntregas(tpVF.vAuxiliar).con_vfreteentrega_dtentrega ) or 
                                ( tpVfreteentregas.Con_Vfreteentrega_Codigo  <> plistaVFEntregas(tpVF.vAuxiliar).Con_Vfreteentrega_Codigo ) or
                                ( tpVfreteentregas.Con_Vfreteentregaobservacao  <> plistaVFEntregas(tpVF.vAuxiliar).Con_Vfreteentregaobservacao ) or
                                ( tpVfreteentregas.Usu_Usuario_Codigo  <> plistaVFEntregas(tpVF.vAuxiliar).Usu_Usuario_Codigo ) ) Then
...
--                    P_STATUS := pkg_glb_common.Status_Erro ;
         ...
         End If;
          ...
          If ( tpVF.tpValeFrete.glb_tpmotorista_codigo <> 'F' ) Then
            ...
            End If;
             ...
             select sum(cc.car_contacorrente_valor)
               into tpvf.vAuxiliar
               from v_car_contacorrente cc
              where trim(cc.docref) = tpVF.VFNumero || tpVF.VFRota || tpVF.VFSaque
...
-- CAD - Cadastrado
 ...
 --         P_MESSAGE := P_MESSAGE || 'Rota Travada ' || tpVf.vFCF_VEICULODISP_ROTATRAVAS || chr(10);
      ...
      End if;
           ...
           End if;
             ...
             If tpvf.vCON_VALEFRETE_TIPOCUSTO <> substr(tpvf.vFCF_FRETECAR_DESINENCIA,1,1) Then
                If plistaparams('ALTERA_TPCUSTO').texto = 'N' Then
                  P_STATUS := pkg_glb_common.Status_Erro;
                  P_MESSAGE := P_MESSAGE || 'ERROTA TRAVADA - Voce não pode Mudar o Tipo de Custo do Frete'  || chr(10);
                end if;
             End If;
           End if;
         ...
         If tpvf.tpValeFrete.con_valefrete_valorliquido < 0 Then
            P_STATUS := pkg_glb_common.Status_Erro;
            P_MESSAGE := P_MESSAGE || 'ERSALDO ERRADO - Saldo não pode ser negativo'  || chr(10);
         End If;
 ...
 --                             tpVF.tpContaCorrente.car_contacorrente_docorigem := tpVF.VFNumero || tpVF.VFSerie || tpVF.VFRota || tpVF.VFSaque;
                              ...
                              tpVF.tpContaCorrente.Car_Contacorrente_Obs := 'Desconto automatico feito no VF ' || tpVF.VFNumero || tpVF.VFSaque  || tpVF.VFRota || tpVF.VFSaque;
                                 ...
                                 When OTHERS Then
                                    P_STATUS   := PKG_GLB_COMMON.Status_Erro;
                                     P_MESSAGE :=  P_MESSAGE || chr(10) ||
                                                                ...
                                                                'Venciamento - '  || tpVF.tpContaCorrente.CAR_CONTACORRENTE_DTVENC      || chr(10) ||
                                                                'erro-' || sqlerrm;
 ...
 */
 ...
 --         P_STATUS := pkg_glb_common.Status_Erro;
      ...
      Loop
          ...
          vXmlEntregaDet := vXmlEntregaDet || '<CHAVENF>'       || R_CURSOR.CHAVENF                                   || '</CHAVENF>';
          vXmlEntregaDet := vXmlEntregaDet || '<CHAVECTE>'      || R_CURSOR.CHAVECTE                                  || '</CHAVECTE>';
    ...
    vXmlImagem := Replace( vXmlImagem, '#', '''' );
     ...
     P_XMLOUT := fn_limpa_campoxml(tpVF.vXmlTab);
     ...
     Begin
       ...
       update t_con_valefrete vf
         set vf.con_valefrete_frete = vValorFrete,
             vf.con_valefrete_valorcomdesconto = vValorconDesc
       ...
       If vTpMotorista = 'A' Then
          vValorLiquido := vValorconDesc - nvl(vTpValefrete.Con_Valefrete_Pedagio,0);
          vValorLiquido := vValorconDesc - nvl(vTpValefrete.Con_Valefrete_Adiantamento,0);
          vValorLiquido := vValorconDesc - nvl(vTpValefrete.Con_Valefrete_Multa,0);
          vValorLiquido := vValorconDesc - nvl(vTpValefrete.Con_Valefrete_Irrf,0);
          vValorLiquido := vValorconDesc - nvl(vTpValefrete.Con_Valefrete_Inss,0);
          vValorLiquido := vValorconDesc - nvl(vTpValefrete.Con_Valefrete_Valorestiva,0);
       Else
          vValorLiquido := vValorFrete - nvl(vTpValefrete.Con_Valefrete_Pedagio,0);
          vValorLiquido := vValorFrete - nvl(vTpValefrete.Con_Valefrete_Adiantamento,0);
          vValorLiquido := vValorFrete - nvl(vTpValefrete.Con_Valefrete_Multa,0);
          vValorLiquido := vValorFrete - nvl(vTpValefrete.Con_Valefrete_Irrf,0);
          vValorLiquido := vValorFrete - nvl(vTpValefrete.Con_Valefrete_Inss,0);
          vValorLiquido := vValorFrete - nvl(vTpValefrete.Con_Valefrete_Valorestiva,0);
       End If;

       update t_con_valefrete vf
         set vf.Con_Valefrete_Inss = vTpValefrete.Con_Valefrete_Inss,
             vf.Con_Valefrete_Sestsenat = vTpValefrete.Con_Valefrete_Sestsenat,
             vf.Con_Valefrete_Irrf = vTpValefrete.Con_Valefrete_Irrf,
    ...
    for c_msg in (select p.usu_perfil_codigo,
      ...
      Loop
          ...
          ElsIf c_msg.usu_perfil_codigo = 'SESTSENAT' Then
             vAliqSESTSENAT := c_msg.usu_perfil_paran1;
      ...
      if vVlrRetidoINSS = vLimiteDesINSS Then
         p_INSS := 0;
      Else
         p_INSS := nvl( ( vBaseINSS * vPrecBaseINSS ) * vBaseINSS ,0);
      End If;
      
      -- Se ultrapassar o Limite pega so a diferenca
      If ( vVlrRetidoINSS + p_INSS ) > vLimiteDesINSS Then
         p_INSS := nvl(( vVlrRetidoINSS + p_INSS ) - vLimiteDesINSS,0);
      End If;

      p_SESTSENAT := nvl(( vBaseINSS * vPrecBaseSESTSENAT ) * vBaseSESTSENAT,0);
      ...
      vBaseIRRF_PF := vBaseIRRF_PF - p_INSS - p_SESTSENAT ;
