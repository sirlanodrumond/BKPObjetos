...
SELECT T.GLB_FORNECEDOR_CGCCPF   FORN,
       ...
       DECODE(D.CTB_HISTORICO_CODIGODEB,'993',F.GLB_FORNECEDOR_NOME,(SELECT SUBSTR(REPLACE(REPLACE(REPLACE(REPLACE(HD.CTB_HISTORICO_DESCRICAO,'#TIT#',TRIM(lpad(T.CPG_TITULOS_NUMTIT,9,'0'))),'#VC#',NULL),'#NOME#',F.GLB_FORNECEDOR_NOME),'#DOC#',TRIM(lpad(T.CPG_TITULOS_NUMTIT,9,'0'))),1,50)
                                                                       FROM TDVADM.T_CTB_HISTORICO HD
                                                                      WHERE HD.CTB_HISTORICO_CODIGO = D.CTB_HISTORICO_CODIGODEB)) HDD,
       D.CTB_HISTORICO_CODIGOCRED HTC,
       DECODE(D.CTB_HISTORICO_CODIGOCRED,'993',F.GLB_FORNECEDOR_NOME,(SELECT SUBSTR(REPLACE(REPLACE(REPLACE(REPLACE(HC.CTB_HISTORICO_DESCRICAO,'#TIT#',TRIM(lpad(T.CPG_TITULOS_NUMTIT,9,'0'))),'#VC#',NULL),'#NOME#',F.GLB_FORNECEDOR_NOME),'#DOC#',TRIM(lpad(T.CPG_TITULOS_NUMTIT,9,'0'))),1,50)
...
SELECT DISTINCT 
       ...
       E.GLB_EVENTO_CODIGO,
       SUBSTR(TRIM(replace(REPLACE((SELECT HD.CTB_HISTORICO_DESCRICAO FROM TDVADM.T_CTB_HISTORICO HD WHERE CTB_HISTORICO_CODIGO = E.CTB_HISTORICO_CODIGODEB),
                            '#DOC#',lpad(CP.CPG_TITULOS_NUMTIT,9,'0')),
                            '#NOME#',DECODE(nvl(TCC.CPG_DESPESAS_CODIGO,'0000'),'0277',C.CPG_CHEQUEBANCO_NOMINAL, F.GLB_FORNECEDOR_NOME))
                  ),1,50) HDD,
       E.CTB_HISTORICO_CODIGOCRED      HTC,
       SUBSTR(TRIM(replace(replace(REPLACE(
                                   (SELECT HC.CTB_HISTORICO_DESCRICAO FROM TDVADM.T_CTB_HISTORICO HC WHERE CTB_HISTORICO_CODIGO = E.CTB_HISTORICO_CODIGOCRED),
                                   '#DOC#',lpad(C.CPG_CHEQUEBANCO_NUMERO,6,'0')),
                                   '#TIT#',lpad(CP.CPG_TITULOS_NUMTIT,9,'0')),
                                   '#NOME#',DECODE(nvl(TCC.CPG_DESPESAS_CODIGO,'0000'),'0277',C.CPG_CHEQUEBANCO_NOMINAL, F.GLB_FORNECEDOR_NOME))
                   ),1,50) HDC,
