...
Begin
  ...
  If vLinha > 0 Then
        ...
        vRetorno.Xml := vRetorno.Xml || '<coleta_ciclo>'           ||  Trim( pDadosNota.coleta_ciclo )                                                || '</coleta_ciclo>'          ;
    ...
    Else
        ...
        vRetorno.Xml := vRetorno.Xml || '<coleta_ciclo></coleta_ciclo>'          ;
  ...
  begin
    ...
    for vCursor in ( Select
                      ...
                      --Função que devolve o tipo de Sacado "0 = Remetente; 1=Destinatario; 2=Outros
                       ...
                       -- 22/10/2020
                       -- Alterado para definicaçao d quatidade limitada, nulo sera considerado nao limitada
                       -- Thiago Soares 
                       nvl(nota.arm_nota_qtdelimit,'N')        nota_qtdelimitada
  ...
  Begin
    ...
    For vCursor In ( Select 
                     ...
                     Where 
                       ...
                       And w.arm_nota_sequencia = pDadosNota.nota_Sequencia
                    )
                    Loop
                      --Incremento a variável de controle
                      vRetorno.Controle := vRetorno.Controle + 1;
    ...
    Select 
      ...
      ( Select Count(*) From t_arm_notafichaemerg fe
        Where fe.arm_nota_numero = nota.arm_nota_numero
          ...
          and fe.arm_nota_sequencia = nota.arm_nota_sequencia
      ) FichaEmerg,
...
--    insert into t_glb_sql(glb_sql_instrucao,glb_sql_programa) values(vDetalhes,'CARGA_DET_20150622');      
    ...
    --deleta da sorter
     DELETE TDVADM.T_ARM_NOTASORTER NS
     WHERE NS.ARM_NOTA_SEQUENCIA = pSEQUENCIA;
...
Begin
  ...
  Select Count(*) 
   ...
   Where w.arm_nota_numero                   = pNota_numero
     ...
     And w.arm_nota_sequencia = pNota_sequencia;
  ...
  If vCount > 0 Then
   ...
   select Count(*) 
    ...
    Where 0 = 0
      ...
      and fe.arm_nota_sequencia = pNota_sequencia;
...
-- 21/03/2014
  Begin
   ...
   if (vNumeroAsn is not null) and (vOrigemCol = '8') then   
      ...
      select distinct nvl(asn.col_asn_numeroonu,'0')
        into vNumeroOnu
...
*************************************************/
  ...
  pStatus := pkg_fifo.Status_Normal;
  -- 21/10/2020
  -- Colocado para auxiliar no DEBUG da aplicacao
  -- Sirlano / Matheus
  If vUsuario = 'jsantos' then
     insert into tdvadm.t_glb_sql values (pParamsSaida,sysdate,'NOTA-RETORNO DELPHI',vUsuario||'-NOTA');
  End If;
 commit ;
--  pMessage := '';
End sp_InsertNota;                  
