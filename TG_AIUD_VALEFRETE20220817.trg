...
BEGIN
   ...
   Begin
      Select substr(P.USU_PERFIL_PARAT,1,6)   
         into vRefCTB                                  
      FROM T_USU_PERFIL P
   ...
   Exception
     When OTHERS Then
        vRefCTB := to_char(add_months(sysdate,-1),'YYYYMM');
     End;
     
    
   IF ( ( inserting ) or ( updating('con_valefrete_impresso') ) )  THEN
  ...
  if updating('CON_VALEFRETE_STATUS') then
    ...
    If to_char(nvl(:NEW.Con_Valefrete_Datacadastro, sysdate),'yyyymm') <= vRefCTB Then
      raise_application_error(-20001,'MODULO DE VF-CTB FECHADO ATE ' || vRefCTB );
    End If;
...
/*     If :new.fcf_fretecar_rowid is null Then
  ...
  if updating('con_catvalefrete_codigo')    or
     ...
     inserting                              then
     -- Sirlano 03/11/2021
     -- Trava para pegar se o SestSEnat esta ZERADO  
     If ( :new.con_valefrete_impresso = 'S' )          and 
        ( :new.glb_tpmotorista_codigo <> 'F' )         and 
        ( :new.con_valefrete_frete > 1 )               and 
        ( nvl(:new.con_valefrete_sestsenat,0) = 0 )    and 
        ( :new.con_catvalefrete_codigo not in ('10') ) and  
        ( length(trim(nvl(sProprietario,'.'))) = 11 )  Then
        
        Raise_application_error(-20111,'SEST SENAT não pode SER 0 (ZERO)!!' || ' VFRETE ' || :NEW.CON_CONHECIMENTO_CODIGO ||:NEW.GLB_ROTA_CODIGO || :NEW.CON_VALEFRETE_SAQUE);
